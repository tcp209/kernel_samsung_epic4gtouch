...
void fusb_rdcxf(struct fusb300 *fusb300,
...
void fusb_fill_idma_prdtbl(struct fusb300_ep *ep,
			struct fusb300_request *req)
{
	u32 value;
	u32 reg;
...
static void fusb300_ep0out(struct fusb300 *fusb300)
{
	...
	if (!list_empty(&ep->queue)) {
		...
		req = list_first_entry(&ep->queue,
			struct fusb300_request, queue);
		if (req->req.length)
			fusb_rdcxf(ep->fusb300, req->req.buf,
				req->req.length);
		done(ep, req, 0);
...
int fusb300_gadget_probe_driver(struct usb_gadget_driver *driver,
...
EXPORT_SYMBOL(fusb300_gadget_probe_driver);

int fusb300_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	struct fusb300 *fusb300 = the_controller;
...
EXPORT_SYMBOL(fusb300_gadget_unregister_driver);
/*--------------------------------------------------------------------------*/
