# 1 "../../drivers/power/sec_battery.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../drivers/power/sec_battery.c"
# 14 "../../drivers/power/sec_battery.c"
...
# 15 "../../drivers/power/sec_battery.c" 2
...
# 16 "../../drivers/power/sec_battery.c" 2
...
# 17 "../../drivers/power/sec_battery.c" 2
...
# 18 "../../drivers/power/sec_battery.c" 2
...
# 20 "../../drivers/power/sec_battery.c" 2
...
# 21 "../../drivers/power/sec_battery.c" 2
...
# 23 "../../drivers/power/sec_battery.c" 2
# 1 "../../include/linux/power_supply.h" 1
# 17 "../../include/linux/power_supply.h"
# 1 "../../include/linux/wakelock.h" 1
...
# 18 "../../include/linux/power_supply.h" 2
...
# 20 "../../include/linux/power_supply.h" 2
...
# 188 "../../include/linux/power_supply.h"
...
# 24 "../../drivers/power/sec_battery.c" 2
...
# 25 "../../drivers/power/sec_battery.c" 2
...
# 28 "../../drivers/power/sec_battery.c" 2
# 1 "../../include/linux/android_alarm.h" 1
...
# 60 "../../include/linux/android_alarm.h"
struct alarm {
 struct rb_node node;
...
# 29 "../../drivers/power/sec_battery.c" 2
# 1 "../../arch/arm/plat-samsung/include/plat/adc.h" 1
# 17 "../../arch/arm/plat-samsung/include/plat/adc.h"
...
extern void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) s3c_adc_set_platdata(struct s3c_adc_mach_info *pd);
# 30 "../../drivers/power/sec_battery.c" 2
# 1 "../../arch/arm/mach-s5pv310/include/mach/sec_battery.h" 1
# 39 "../../arch/arm/mach-s5pv310/include/mach/sec_battery.h"
struct sec_bat_adc_table_data {
 int adc;
 int temperature;
};
# 54 "../../arch/arm/mach-s5pv310/include/mach/sec_battery.h"
...
# 31 "../../drivers/power/sec_battery.c" 2
# 209 "../../drivers/power/sec_battery.c"
enum tmu_status_t {
 TMU_STATUS_NORMAL = 0,
 TMU_STATUS_TRIPPED,
...
# 241 "../../drivers/power/sec_battery.c"
...
static struct sec_bat_info *pchg = ((void *)0) ;


struct power_supply *get_power_supply_by_name(char *name)
{
 if (!name)
  return (struct power_supply *)((void *)0);
 else
     ...
     struct adc_sample *sample, int adc)
{
 ...
 if (adc < 0 || adc == 0) {
  printk("<3>" "%s %s: " "%s: invalid adc : %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, adc);
  return 0;
 }
 ...
 if (!average_adc) {
  ...
  sample->index = ++index >= 5 ? 0 : index;
  sample->adc_arr[sample->index] = adc;
  for (i = 0; i < 5; i++)
   total_adc += sample->adc_arr[i];

  average_adc = total_adc / 5;
 }

 sample->average_adc = average_adc;
 ({ if (0) printk("<7>" "%s %s: " "%s: i(%d) adc=%d, avg_adc=%d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, sample->index, adc, average_adc); 0; });
...
static int sec_bat_get_fuelgauge_data(struct sec_bat_info *info, int type)
{
 ...
 if (!psy) {
  printk("<3>" "%s %s: " "%s: fail to get fuel gauge ps\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return -19;
 }
...
static int sec_bat_get_property(struct power_supply *ps,
    ...
    union power_supply_propval *val)
{
 struct sec_bat_info *info = ({ const typeof( ((struct sec_bat_info *)0)->psy_bat ) *__mptr = (ps); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,psy_bat) );});
...
{
# 539 "../../drivers/power/sec_battery.c"
 if (!sec_bat_enable_charging(info, false)) {
  ...
  printk("<6>" "%s %s: " "%s: Charging Top-off\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
...
# 605 "../../drivers/power/sec_battery.c"
  break;
 case POWER_SUPPLY_PROP_CAPACITY:
  printk("<6>" "%s %s: " "%s: refresh battery data\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 case POWER_SUPPLY_PROP_ONLINE:
  ...
  printk("<6>" "%s %s: " "%s: cable was changed(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, val->intval);
 ...
 case POWER_SUPPLY_PROP_HEALTH:
  ...
  printk("<6>" "%s %s: " "%s: TMU status has been changed(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_tmu_status);


  break;
# 669 "../../drivers/power/sec_battery.c"
 default:
  return -22;
 }
 return 0;
...
static int sec_usb_get_property(struct power_supply *ps,
    ...
    union power_supply_propval *val)
{
 struct sec_bat_info *info = ({ const typeof( ((struct sec_bat_info *)0)->psy_usb ) *__mptr = (ps); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,psy_usb) );});
...
static int sec_ac_get_property(struct power_supply *ps,
          ...
          union power_supply_propval *val)
{
 struct sec_bat_info *info = ({ const typeof( ((struct sec_bat_info *)0)->psy_ac ) *__mptr = (ps); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,psy_ac) );});
...
static int sec_bat_get_adc_data(struct sec_bat_info *info, int adc_ch)
{
 ...
 for (i = 0; i < 7; i++) {
  ...
  if (adc_data < 0) {
   printk("<3>" "%s %s: " "%s : err(%d) returned, skip read\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, adc_data);
 ...
 return (adc_total - adc_max - adc_min) / (7 - 2);
err:
 return err_value;
}


static inline __attribute__((always_inline)) int s3c_read_temper_adc(struct sec_bat_info *info)
{
 int adc;
...
# 804 "../../drivers/power/sec_battery.c"
static int is_event_end_timer_running(struct sec_bat_info *info)
{
 unsigned long passed_time = 0;

 if (info->event_expired_time== 0xFFFFFFFF)
  return false;
 ...
 if ((({ unsigned long __dummy; typeof(passed_time) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof((unsigned long)( 10 * 60 * 200)) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)((unsigned long)( 10 * 60 * 200)) - (long)(passed_time) < 0))) {
  info->event_expired_time = 0xFFFFFFFF;
  printk("<6>" "%s %s: " "[SPR_NA] %s: Event timer is over 10 min\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return false;
 } else {
  printk("<6>" "%s %s: " "[SPR_NA] %s: Event timer is running(%u s)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, jiffies_to_msecs(passed_time) / 1000);
...
static int sec_bat_check_temper(struct sec_bat_info *info)
{
 ...
 printk("<6>" "%s %s: " "[test] temp_radc_value : %d \n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, temp_radc);
 ...
 if (info->get_lpcharging_state) {
  if (info->get_lpcharging_state()) {
   if (temp_radc >= 710) {
    ...
    if (info->batt_temp_high_cnt < 3)
     info->batt_temp_high_cnt++;
    printk("<6>" "%s %s: " "%s: high count = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_temp_high_cnt);

   } else if (temp_radc <= 702 && temp_radc >= 512) {
    if (health == POWER_SUPPLY_HEALTH_OVERHEAT ||
    health == POWER_SUPPLY_HEALTH_COLD){
     info->batt_temp_high_cnt = 0;
     info->batt_temp_low_cnt = 0;

    if (info->batt_temp_recover_cnt < 3)
      info->batt_temp_recover_cnt++;
     printk("<6>" "%s %s: " "%s: recovery count = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_temp_recover_cnt);

    }
   } else if (temp_radc <= 506) {
    if (health != POWER_SUPPLY_HEALTH_COLD &&
    health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE)
    if (info->batt_temp_low_cnt < 3)
     info->batt_temp_low_cnt++;
    printk("<6>" "%s %s: " "%s: low count = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_temp_low_cnt);
   ...
   if((info->batt_event_status) || (is_event_end_timer_running(info))){
    printk("<6>" "%s %s: " "[NA_SPR] Changed Put off Current" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
    if (temp_radc >= 777) {
     if (health != POWER_SUPPLY_HEALTH_OVERHEAT &&
     health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE)
     if (info->batt_temp_high_cnt < 3)
    ...
    } else if (temp_radc <= 699 && temp_radc >= 516) {
     if (health == POWER_SUPPLY_HEALTH_OVERHEAT ||
     health == POWER_SUPPLY_HEALTH_COLD){
 ...
 else if (info->batt_temp_low_cnt >= 3)
  info->batt_health = POWER_SUPPLY_HEALTH_COLD;
 else if (info->batt_temp_recover_cnt >= 3)
  info->batt_health = POWER_SUPPLY_HEALTH_GOOD;
...
skip:
 ...
 if (info->fuel_gauge_name) {
  ...
  if (ret) {
   printk("<3>" "%s %s: " "%s: fail to set temperature(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);

  }
 }

 printk("<6>" "%s %s: " "%s: temp=%d, adc=%d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, temp, temp_adc);
...
# 1290 "../../drivers/power/sec_battery.c"
static void check_chgcurrent(struct sec_bat_info *info)
{
 unsigned long chg_current_adc = 0;

 mutex_lock(&info->adclock);
 chg_current_adc = sec_bat_get_adc_data(info, 1);
 mutex_unlock(&info->adclock);
 if (chg_current_adc < 0)
  chg_current_adc = info->batt_current_adc;
 info->batt_current_adc = chg_current_adc;

 ({ if (0) printk("<7>" "%s %s: " "[battery] charging current = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , info->batt_current_adc); 0; });
 ...
 case POWER_SUPPLY_STATUS_CHARGING:
  if (info->batt_vcell >= (4000 * 1000)) {
   ...
   if (info->batt_current_adc <= 350) {
    ...
    printk("<6>" "%s %s: " "%s : ADC full cnt = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->charging_adc_full_count);
...
{
 info->batt_raw_soc = sec_bat_get_fuelgauge_data(info, 3);
 info->batt_soc = sec_bat_get_fuelgauge_data(info, 0);
 info->batt_vcell = sec_bat_get_fuelgauge_data(info, 1);
 info->batt_vfocv = sec_bat_get_fuelgauge_data(info, 4);
}
...
static int sec_bat_enable_charging_main(struct sec_bat_info *info, bool enable)
{
 ...
 if (!psy) {
  printk("<3>" "%s %s: " "%s: fail to get charger ps\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return -19;
 }
 ...
 if (enable) {
  ...
  case CABLE_TYPE_MISC:
   ...
   break;
  default:
   printk("<3>" "%s %s: " "%s: Invalid func use\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
   return -22;
  }
  ...
  ret = psy->set_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW,
     &val_chg_current);
  if (ret) {
   printk("<3>" "%s %s: " "%s: fail to set charging cur(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);
  ...
  ret = psy->set_property(psy, POWER_SUPPLY_PROP_CHARGE_FULL,
     &val_topoff);
  if (ret) {
   printk("<3>" "%s %s: " "%s: fail to set topoff cur(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);
  ...
  if (info->charging_start_time == 0) {
   ...
   info->charging_next_time = (10 * 60 * 200);
  }
 } else {
 ...
 if (ret) {
  printk("<3>" "%s %s: " "%s: fail to set charging status(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);
...
static int sec_bat_enable_charging_sub(struct sec_bat_info *info, bool enable)
{
 ...
 if (enable) {
  ...
  ret = psy_main->set_property(psy_main, POWER_SUPPLY_PROP_STATUS,
          &val_type);
  if (ret) {
   printk("<3>" "%s %s: " "%s: fail to set charging" " status-main(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);
...
static int sec_bat_enable_charging(struct sec_bat_info *info, bool enable)
{
 if (enable && (info->batt_health != POWER_SUPPLY_HEALTH_GOOD)) {
  ...
  printk("<6>" "%s %s: " "%s: Battery is NOT good!!!\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return -1;
 }
...
{
 struct sec_bat_info *info = ({ const typeof( ((struct sec_bat_info *)0)->cable_work ) *__mptr = (work); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,cable_work) );});
 ...
 case CABLE_TYPE_NONE:
  if (!sec_bat_enable_charging(info, false)) {
   ...
   wake_lock_timeout(&info->vbus_wake_lock, 200 * 5);
  }
  break;
 case CABLE_TYPE_USB:
 case CABLE_TYPE_AC:
 case CABLE_TYPE_MISC:
  ...
  break;
 default:
  printk("<3>" "%s %s: " "%s: Invalid cable type\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  break;;
 }
...
{
 if (info->charging_start_time == 0) {
  printk("<6>" "%s %s: " "%s: charging_start_time has never			 been used since initializing\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 case POWER_SUPPLY_STATUS_FULL:
  if (info->recharging_status == true &&
   (({ unsigned long __dummy; typeof(info->charging_passed_time) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof((unsigned long)(2 * 60 * 60 * 200)) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)((unsigned long)(2 * 60 * 60 * 200)) - (long)(info->charging_passed_time) < 0))) {

   if (!sec_bat_enable_charging(info, false)) {
    info->recharging_status = false;
    printk("<6>" "%s %s: " "%s: Recharging timer expired\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 case POWER_SUPPLY_STATUS_CHARGING:
   ...
   if ((({ unsigned long __dummy; typeof(info->charging_passed_time) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof((unsigned long)(6 * 60 * 60 * 200)) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)((unsigned long)(6 * 60 * 60 * 200)) - (long)(info->charging_passed_time) < 0))) {
    ...
    if (!sec_bat_enable_charging(info, false)) {
     ...
     printk("<6>" "%s %s: " "%s: Charging timer expired\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  ...
  if ((({ unsigned long __dummy; typeof(info->charging_passed_time) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(info->charging_next_time) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(info->charging_next_time) - (long)(info->charging_passed_time) < 0))) {


   if (!sec_bat_enable_charging(info, true)) {
    info->charging_next_time =
     info->charging_passed_time + (10 * 60 * 200);

    printk("<6>" "%s %s: " "%s: Reset charging current\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  ...
  break;
 default:
  printk("<6>" "%s %s: " "%s: Undefine Battery Status\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return false;
 }

 printk("<6>" "%s %s: " "Time past : %u secs\n" , dev_driver_string(info->dev) , dev_name(info->dev) , jiffies_to_msecs(info->charging_passed_time) / 1000);


 return false;
}
# 1705 "../../drivers/power/sec_battery.c"
static void sec_bat_check_vf(struct sec_bat_info *info)
{
 struct power_supply *psy = get_power_supply_by_name(info->charger_name);
 union power_supply_propval value;
 int ret;
# 1727 "../../drivers/power/sec_battery.c"
  ret = psy->get_property(psy, POWER_SUPPLY_PROP_PRESENT, &value);

  if (ret < 0) {
   printk("<3>" "%s %s: " "%s: fail to get status(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, ret);
 ...
 printk("<6>" "%s %s: " "%s: Battery Health (%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_health);


 return;
}
# 1781 "../../drivers/power/sec_battery.c"
static bool sec_bat_check_ing_level_trigger(struct sec_bat_info *info)
{
 ...
 if (info->use_sub_charger && info->sub_charger_name) {
  ...
  case POWER_SUPPLY_STATUS_CHARGING:
   switch (value.intval) {
# 1833 "../../drivers/power/sec_battery.c"
   ...
   case POWER_SUPPLY_STATUS_NOT_CHARGING:
    ...
    if (info->cable_type == CABLE_TYPE_USB ||
     ...
     if (info->batt_vcell >=
      ...
      printk("<6>" "%s %s: " "%s : full interrupt cnt = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->charging_int_full_count);
   ...
   break;
  case POWER_SUPPLY_STATUS_NOT_CHARGING:
# 1888 "../../drivers/power/sec_battery.c"
...
{
 struct sec_bat_info *info = ({ const typeof( ((struct sec_bat_info *)0)->monitor_work ) *__mptr = (work); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,monitor_work) );});
 ...
 if (info->cable_type == CABLE_TYPE_NONE &&
     info->charging_status == POWER_SUPPLY_STATUS_FULL) {
  printk("<3>" "%s %s: " "invalid full state\n" , dev_driver_string(info->dev) , dev_name(info->dev));
  info->charging_status = POWER_SUPPLY_STATUS_DISCHARGING;
 }
 ...
 if (info->batt_vcell < (4130 * 1000) &&
  ...
  if (info->recharging_int_threshold_count
   ...
   if (!sec_bat_enable_charging(info, true)) {
    ...
    printk("<6>" "%s %s: " "%s: Start Recharging, Vcell = %d\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, info->batt_vcell);
...
# 2046 "../../drivers/power/sec_battery.c"
  break;
 case POWER_SUPPLY_STATUS_CHARGING:
  switch (info->batt_health) {
# 2062 "../../drivers/power/sec_battery.c"
  ...
  break;
 case POWER_SUPPLY_STATUS_DISCHARGING:
  ({ if (0) printk("<7>" "%s %s: " "%s: Discharging\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__); 0; });
  break;
 case POWER_SUPPLY_STATUS_NOT_CHARGING:
  if (info->batt_health == POWER_SUPPLY_HEALTH_GOOD) {
   printk("<6>" "%s %s: " "%s: recover health state\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  ...
  break;
 default:
  printk("<6>" "%s %s: " "%s: Undefined Battery Status\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return;
 }
...
full_charged:
 ...
 printk("<6>" "%s %s: " "soc(%d), vfocv(%d), vcell(%d), temp(%d), charging(%d), health(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , info->batt_soc, info->batt_vfocv, info->batt_vcell / 1000, info->batt_temp / 10, info->charging_status, info->batt_health);
...
static void sec_bat_vf_check_work(struct work_struct *work)
{
 ...
 info = ({ const typeof( ((struct sec_bat_info *)0)->vf_check_work.work ) *__mptr = (work); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,vf_check_work.work) );});

 sec_bat_check_vf(info);
# 2141 "../../drivers/power/sec_battery.c"
 if (info->batt_health == POWER_SUPPLY_HEALTH_UNSPEC_FAILURE) {
  printk("<6>" "%s %s: " "%s: Battery Disconnected\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
...
{
 struct sec_bat_info *info;
 info = ({ const typeof( ((struct sec_bat_info *)0)->polling_work.work ) *__mptr = (work); (struct sec_bat_info *)( (char *)__mptr - __builtin_offsetof(struct sec_bat_info,polling_work.work) );});

 wake_lock(&info->monitor_wake_lock);
 queue_work(info->monitor_wqueue, &info->monitor_work);

 if (info->initial_check_count) {
  schedule_delayed_work(&info->polling_work, 200);
  info->initial_check_count--;
 } else
 schedule_delayed_work(&info->polling_work,
         msecs_to_jiffies(info->polling_interval));
}
# 2178 "../../drivers/power/sec_battery.c"
static struct device_attribute sec_battery_attrs[] = {
 { .attr = { .name = "batt_vol", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_soc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_vfocv", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_temp", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_temp_adc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },

 { .attr = { .name = "batt_temp_radc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },

 { .attr = { .name = "batt_temp_adc_avg", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "charging_source", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_lp_charging", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "video", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "mp3", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_type", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_full_check", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_temp_check", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_temp_adc_spec", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_test_value", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_current_now", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_current_adc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "system_rev", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },

 { .attr = { .name = "fg_soc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },



 { .attr = { .name = "batt_lpm_state", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "batt_tmu_status", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },

 { .attr = { .name = "slate_test_mode", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
# 2219 "../../drivers/power/sec_battery.c"
 { .attr = { .name = "call", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },


 { .attr = { .name = "browser", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "hotspot", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "camera", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "data_call", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
 { .attr = { .name = "wimax", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_bat_show_property, .store = sec_bat_store, },
...
enum {
 ...
 SLATE_TEST_MODE,
# 2268 "../../drivers/power/sec_battery.c"
 BATT_CALL,
 BATT_BROWSER,
 BATT_HOTSOPT,
 BATT_CAMERA,
...
# 2304 "../../drivers/power/sec_battery.c"
  ...
  struct sec_bat_info *info, int mode, int offset)
{
 ...
 printk("<7>" "[%s] current batt_event_status = 0x%x\n", __func__,
...
int sec_bat_use_wimax(int onoff)
{
 ...
 sec_bat_check_event_status(info, onoff, ( 0x1 << 7)) ;
}
extern typeof(sec_bat_use_wimax) sec_bat_use_wimax; static const char __kstrtab_sec_bat_use_wimax[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "sec_bat_use_wimax"; static const struct kernel_symbol __ksymtab_sec_bat_use_wimax __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&sec_bat_use_wimax, __kstrtab_sec_bat_use_wimax } ;
         ...
         struct device_attribute *attr, char *buf)
{
 ...
 switch (off) {
 case BATT_VOL:
  val = sec_bat_get_fuelgauge_data(info, 5);
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", val);
  break;
 case BATT_SOC:
  val = sec_bat_get_fuelgauge_data(info, 0);
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", val);
  break;
 case BATT_VFOCV:
  val = sec_bat_get_fuelgauge_data(info, 4);
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", val);
  break;
 case BATT_TEMP:
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", info->batt_temp);
 ...
 case BATT_TEST_VALUE:
  i += scnprintf(buf + i, ((1UL) << 12) - i, "0-normal, 1-full, 2-low, 3-high, 4-over, 5-cf, 6-trip (%d)\n",
   info->test_value);
  break;
 case BATT_CURRENT_NOW:
  {
   ...
   if (!psy) {
    printk("<3>" "%s: fail to get battery ps\n", __func__);
    return -19;
   }
 ...
 case BATT_SYSTEM_REV:
  ...
  break;
 case BATT_FG_PSOC:
  val = sec_bat_get_fuelgauge_data(info, 3);
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", val);
  break;
 case BATT_LPM_STATE:
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n",
 ...
 case SLATE_TEST_MODE:
  ...
  break;
# 2508 "../../drivers/power/sec_battery.c"
 case BATT_CALL:
  i += scnprintf(buf + i, ((1UL) << 12) - i, "%d\n", info->use_call);
  break;
 case BATT_BROWSER:
        ...
        const char *buf, size_t count)
{
 ...
 case BATT_CURRENT_NOW:
  if (sscanf(buf, "%d\n", &x) == 1) {
   ...
   if ((info->charging_status == POWER_SUPPLY_STATUS_CHARGING) ||
    ...
    printk("<3>" "%s: NOT charging status\n", __func__);
  }
  break;
 case BATT_VIDEO:

  if (sscanf(buf, "%d\n", &x) == 1) {
   printk("<6>" "%s %s: " "%s: video(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;



   info->use_video = x;
   sec_bat_check_event_status(info, x, ( 0x1 << 1));
 ...
 case BATT_MP3:
  ...
  if (sscanf(buf, "%d\n", &x) == 1) {
   printk("<6>" "%s %s: " "%s: mp3(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;



   info->use_music = x;
   sec_bat_check_event_status(info, x, ( 0x1 << 2));
 ...
 case SLATE_TEST_MODE:
  ...
  break;
# 2671 "../../drivers/power/sec_battery.c"
 case BATT_CALL:

  if (sscanf(buf, "%d\n", &x) == 1) {
   info->use_call = x ;
   printk("<6>" "%s %s: " "[NA_SPR]%s: call (%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 0));
  }
  break;
 case BATT_BROWSER:
  ...
  if (sscanf(buf, "%d\n", &x) == 1) {
   info->use_browser = x ;
   printk("<6>" "%s %s: " "[NA_SPR]%s: BROWSER(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 3));
  }
  break;
 case BATT_HOTSOPT:
  ...
  if (sscanf(buf, "%d\n", &x) == 1) {
   info->use_hotspot = x;
   printk("<6>" "%s %s: " "[NA_SPR]%s: HOTSPOT(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 4));
  }
  break;
 case BATT_CAMERA:

  if (sscanf(buf, "%d\n", &x) == 1) {
   printk("<6>" "%s %s: " "[NA_SPR]%s: CAMERA(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 5));
  }
  break;
 case BATT_DATA_CALL:
  ...
  if (sscanf(buf, "%d\n", &x) == 1) {
   ...
   printk("<6>" "%s %s: " "[NA_SPR]%s: DATA CALL(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 6));
  }
  break;
 case BATT_WIMAX:
  ...
  if (sscanf(buf, "%d\n", &x) == 1) {
   info->use_wimax =x;
   printk("<6>" "%s %s: " "[NA_SPR]%s: WIMAX(%d)\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__, x);
   ret = count;
   sec_bat_check_event_status(info, x, ( 0x1 << 7));
  }
  break;
...
static int sec_bat_create_attrs(struct device *dev)
{
 ...
 for (i = 0; i < (sizeof(sec_battery_attrs) / sizeof((sec_battery_attrs)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sec_battery_attrs), typeof(&sec_battery_attrs[0]))); }))); i++) {
...
# 2836 "../../drivers/power/sec_battery.c"
static __attribute__ ((__section__(".devinit.text"))) __attribute__((__cold__)) int sec_bat_probe(struct platform_device *pdev)
{
 ...
 printk("<6>" "%s %s: " "%s: SEC Battery Driver Loading\n" , dev_driver_string(&pdev->dev) , dev_name(&pdev->dev) , __func__);

 info = kzalloc(sizeof(*info), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!info)
  return -12;
 ...
 if (!pdata->fuel_gauge_name || !pdata->charger_name) {
  printk("<3>" "%s %s: " "%s: no fuel gauge or charger name\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 if (system_rev >= 0x08) {
# 2877 "../../drivers/power/sec_battery.c"
  printk("<6>" "%s %s: " "%s: use sub-charger\n" , dev_driver_string(&pdev->dev) , dev_name(&pdev->dev) , __func__);
  ...
  printk("<6>" "%s %s: " "%s: use main charger\n" , dev_driver_string(&pdev->dev) , dev_name(&pdev->dev) , __func__);
 ...
 info->psy_bat.name = "battery",
     ...
     info->psy_bat.num_properties = (sizeof(sec_battery_props) / sizeof((sec_battery_props)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sec_battery_props), typeof(&sec_battery_props[0]))); }))),
     ...
     info->psy_usb.num_supplicants = (sizeof(supply_list) / sizeof((supply_list)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(supply_list), typeof(&supply_list[0]))); }))),
     info->psy_usb.properties = sec_power_props,
     info->psy_usb.num_properties = (sizeof(sec_power_props) / sizeof((sec_power_props)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sec_power_props), typeof(&sec_power_props[0]))); }))),
     ...
     info->psy_ac.num_supplicants = (sizeof(supply_list) / sizeof((supply_list)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(supply_list), typeof(&supply_list[0]))); }))),
     info->psy_ac.properties = sec_power_props,
     info->psy_ac.num_properties = (sizeof(sec_power_props) / sizeof((sec_power_props)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sec_power_props), typeof(&sec_power_props[0]))); }))),
 ...
 if (!psy) {
  printk("<3>" "%s %s: " "%s: fail to get charger\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  return -19;
 }
 ...
 info->initial_check_count = 4;

 do { static struct lock_class_key __key; __mutex_init((&info->adclock), "&info->adclock", &__key); } while (0);

 info->padc = s3c_adc_register(pdev, ((void *)0), ((void *)0), 0);
 info->charging_start_time = 0;
 ...
 if (info->get_lpcharging_state) {
  if (info->get_lpcharging_state())
   info->polling_interval = (10 * 1000) / 4;
  else
   info->polling_interval = (10 * 1000);
 }
 ...
 if (ret) {
  printk("<3>" "%s %s: " "%s: failed to register psy_bat\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 if (ret) {
  printk("<3>" "%s %s: " "%s: failed to register psy_usb\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 if (ret) {
  printk("<3>" "%s %s: " "%s: failed to register psy_ac\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 info->entry = create_proc_entry("batt_info_proc", (00400|00040|00004), ((void *)0));
 if (!info->entry)
  printk("<3>" "%s %s: " "%s: failed to create proc_entry\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 ...
 info->monitor_wqueue =
     __create_workqueue_key(((dev_name(&pdev->dev))), (1), (1), (0), ((void *)0), ((void *)0));
 if (!info->monitor_wqueue) {
  printk("<3>" "%s %s: " "%s: fail to create workqueue\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
  goto err_supply_unreg_ac;
 }

 do { do { __init_work(((&info->monitor_work)), 0); ((&info->monitor_work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&info->monitor_work))->entry); do { (((&info->monitor_work)))->func = (((sec_bat_monitor_work))); } while (0); } while (0); } while (0);
 do { do { __init_work(((&info->cable_work)), 0); ((&info->cable_work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&info->cable_work))->entry); do { (((&info->cable_work)))->func = (((sec_bat_cable_work))); } while (0); } while (0); } while (0);

 do { do { do { __init_work(((&(&info->polling_work)->work)), 0); ((&(&info->polling_work)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&info->polling_work)->work))->entry); do { (((&(&info->polling_work)->work)))->func = ((((sec_bat_polling_work)))); } while (0); } while (0); } while (0); init_timer_deferrable_key((&(&info->polling_work)->timer), ((void *)0), ((void *)0)); } while (0);
 ...
 do { do { do { __init_work(((&(&info->vf_check_work)->work)), 0); ((&(&info->vf_check_work)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&info->vf_check_work)->work))->entry); do { (((&(&info->vf_check_work)->work)))->func = ((((sec_bat_vf_check_work)))); } while (0); } while (0); } while (0); init_timer_deferrable_key((&(&info->vf_check_work)->timer), ((void *)0), ((void *)0)); } while (0);
 ...
 printk("<6>" "%s %s: " "%s: SEC Battery Driver Loaded\n" , dev_driver_string(info->dev) , dev_name(info->dev) , __func__);
 return 0;
...
static int __attribute__ ((__section__(".devexit.text"))) __attribute__((__used__)) __attribute__((__cold__)) sec_bat_remove(struct platform_device *pdev)
{
 struct sec_bat_info *info = dev_get_drvdata(&(pdev)->dev);

 remove_proc_entry("batt_info_proc", ((void *)0));
...
# 3202 "../../drivers/power/sec_battery.c"
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) sec_bat_init(void)
{
 return platform_driver_register(&sec_bat_driver);
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) sec_bat_exit(void)
{
 platform_driver_unregister(&sec_bat_driver);
}

static initcall_t __initcall_sec_bat_init7 __attribute__((__used__)) __attribute__((__section__(".initcall" "7" ".init"))) = sec_bat_init;
static exitcall_t __exitcall_sec_bat_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = sec_bat_exit;;
