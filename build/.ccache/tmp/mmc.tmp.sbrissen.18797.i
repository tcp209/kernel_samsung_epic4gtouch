...
# 17 "../../drivers/mmc/core/mmc.c" 2
# 1 "../../include/linux/mmc/card.h" 1
...
# 171 "../../include/linux/mmc/card.h"
static inline __attribute__((always_inline)) int mmc_card_lenient_fn0(const struct mmc_card *c)
{
 return c->quirks & (1<<0);
}

static inline __attribute__((always_inline)) int mmc_blksz_for_byte_mode(const struct mmc_card *c)
{
 return c->quirks & (1<<1);
}
# 191 "../../include/linux/mmc/card.h"
struct mmc_driver {
...
# 18 "../../drivers/mmc/core/mmc.c" 2
# 1 "../../include/linux/mmc/mmc.h" 1
# 159 "../../include/linux/mmc/mmc.h"
struct _mmc_csd {
 u8 csd_structure;
 u8 spec_vers;
 u8 taac;
...
# 19 "../../drivers/mmc/core/mmc.c" 2

# 1 "../../drivers/mmc/core/core.h" 1
# 14 "../../drivers/mmc/core/core.h"
...
static inline __attribute__((always_inline)) void mmc_delay(unsigned int ms)
{
 if (ms < 1000 / 200) {
  ({ __might_sleep("../../drivers/mmc/core/core.h", 44, 0); _cond_resched(); });
  ( (__builtin_constant_p(ms) && (ms)<=2) ? (__builtin_constant_p((ms)*1000) ? (((ms)*1000) > (2 * 1000) ? __bad_udelay() : __const_udelay(((ms)*1000) * ((2199023U*200)>>11))) : __udelay((ms)*1000)) : ({unsigned long __ms=(ms); while (__ms--) (__builtin_constant_p(1000) ? ((1000) > (2 * 1000) ? __bad_udelay() : __const_udelay((1000) * ((2199023U*200)>>11))) : __udelay(1000));}));
 } else {
  msleep(ms);
 }
}
...
# 1 "../../drivers/mmc/core/bus.h" 1
# 22 "../../drivers/mmc/core/bus.h"
...
# 22 "../../drivers/mmc/core/mmc.c" 2
# 1 "../../drivers/mmc/core/mmc_ops.h" 1
# 15 "../../drivers/mmc/core/mmc_ops.h"
...
static int mmc_decode_cid(struct mmc_card *card)
{
 ...
 case 0:
 case 1:
  card->cid.manfid = ({ const int __size = 24; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((104) / 32); const int __shft = (104) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[0] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((96) / 32); const int __shft = (96) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[1] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((88) / 32); const int __shft = (88) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[2] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((80) / 32); const int __shft = (80) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[3] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((72) / 32); const int __shft = (72) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[4] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((64) / 32); const int __shft = (64) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[5] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((56) / 32); const int __shft = (56) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[6] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((48) / 32); const int __shft = (48) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.hwrev = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((44) / 32); const int __shft = (44) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.fwrev = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((40) / 32); const int __shft = (40) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.serial = ({ const int __size = 24; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((16) / 32); const int __shft = (16) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.month = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((12) / 32); const int __shft = (12) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.year = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((8) / 32); const int __shft = (8) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; }) + 1997;
  break;

 case 2:
 case 3:
 case 4:
  card->cid.manfid = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((120) / 32); const int __shft = (120) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.oemid = ({ const int __size = 16; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((104) / 32); const int __shft = (104) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[0] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((96) / 32); const int __shft = (96) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[1] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((88) / 32); const int __shft = (88) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[2] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((80) / 32); const int __shft = (80) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[3] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((72) / 32); const int __shft = (72) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[4] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((64) / 32); const int __shft = (64) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.prod_name[5] = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((56) / 32); const int __shft = (56) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.serial = ({ const int __size = 32; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((16) / 32); const int __shft = (16) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.month = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((12) / 32); const int __shft = (12) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
  card->cid.year = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((8) / 32); const int __shft = (8) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; }) + 1997;
  break;

 default:
  printk("<3>" "%s: card has unknown MMCA version %d\n",
   (dev_name(&(card->host)->class_dev)), card->csd.mmca_vsn);
  return -22;
 }

 return 0;
}
# 131 "../../drivers/mmc/core/mmc.c"
static int mmc_decode_csd(struct mmc_card *card)
{
 ...
 csd->structure = ({ const int __size = 2; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((126) / 32); const int __shft = (126) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 if (csd->structure == 0) {
  printk("<3>" "%s: unrecognised CSD structure version %d\n",
   (dev_name(&(card->host)->class_dev)), csd->structure);
  return -22;
 }

 csd->mmca_vsn = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((122) / 32); const int __shft = (122) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 m = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((115) / 32); const int __shft = (115) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 e = ({ const int __size = 3; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((112) / 32); const int __shft = (112) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->tacc_ns = (tacc_exp[e] * tacc_mant[m] + 9) / 10;
 csd->tacc_clks = ({ const int __size = 8; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((104) / 32); const int __shft = (104) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; }) * 100;

 m = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((99) / 32); const int __shft = (99) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 e = ({ const int __size = 3; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((96) / 32); const int __shft = (96) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->max_dtr = tran_exp[e] * tran_mant[m];
 csd->cmdclass = ({ const int __size = 12; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((84) / 32); const int __shft = (84) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });

 e = ({ const int __size = 3; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((47) / 32); const int __shft = (47) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 m = ({ const int __size = 12; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((62) / 32); const int __shft = (62) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->capacity = (1 + m) << (e + 2);

 csd->read_blkbits = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((80) / 32); const int __shft = (80) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->read_partial = ({ const int __size = 1; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((79) / 32); const int __shft = (79) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->write_misalign = ({ const int __size = 1; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((78) / 32); const int __shft = (78) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->read_misalign = ({ const int __size = 1; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((77) / 32); const int __shft = (77) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->r2w_factor = ({ const int __size = 3; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((26) / 32); const int __shft = (26) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->write_blkbits = ({ const int __size = 4; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((22) / 32); const int __shft = (22) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
 csd->write_partial = ({ const int __size = 1; const u32 __mask = (__size < 32 ? 1 << __size : 0) - 1; const int __off = 3 - ((21) / 32); const int __shft = (21) & 31; u32 __res; __res = resp[__off] >> __shft; if (__size + __shft > 32) __res |= resp[__off-1] << ((32 - __shft) % 32); __res & __mask; });
# 184 "../../drivers/mmc/core/mmc.c"
 return 0;
}
...
static int mmc_read_ext_csd(struct mmc_card *card)
{
 ...
 do { if (__builtin_expect(!!(!card), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 ext_csd = kmalloc(512, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!ext_csd) {
  printk("<3>" "%s: could not allocate a buffer to "
   "receive the ext_csd.\n", (dev_name(&(card->host)->class_dev)));
  return -12;
 }
 ...
 if (err) {
  ...
  if (card->csd.capacity == (4096 * 512)) {
   printk("<3>" "%s: unable to read EXT_CSD "
    "on a possible high capacity card. "
    "Card will be ignored.\n",
    (dev_name(&(card->host)->class_dev)));
  } else {
   printk("<4>" "%s: unable to read "
 ...
 if (card->csd.structure == 3) {
  int ext_csd_struct = ext_csd[194];
  if (ext_csd_struct > 5) {
   printk("<3>" "%s: unrecognised EXT_CSD structure "
    "version %d\n", (dev_name(&(card->host)->class_dev)),
     ext_csd_struct);
   err = -22;
   goto out;
 ...
 card->ext_csd.rev = ext_csd[192];
 if (card->ext_csd.rev > 5) {
  printk("<3>" "%s: unrecognised EXT_CSD revision %d\n",
   (dev_name(&(card->host)->class_dev)), card->ext_csd.rev);
  err = -22;
  goto out;
 }
 ...
 if (card->ext_csd.rev >= 2) {
  ...
  if (card->ext_csd.sectors) {
   ...
   boot_sectors = ext_csd[226] * 512;
   card->ext_csd.sectors -= boot_sectors;
   ((card)->state |= (1<<3));
  }
 }

 switch (ext_csd[196] & 0xF) {
 ...
 default:
  ...
  printk("<4>" "%s: card is mmc v4 but doesn't "
   "support any high-speed modes.\n",
   (dev_name(&(card->host)->class_dev)));
 }

 if (card->ext_csd.rev >= 3) {
  u8 sa_shift = ext_csd[217];


  if (sa_shift > 0 && sa_shift <= 0x17)
   card->ext_csd.sa_timeout =
     1 << ext_csd[217];
# 314 "../../drivers/mmc/core/mmc.c"
 }
# 329 "../../drivers/mmc/core/mmc.c"
out:
 kfree(ext_csd);
...
static ssize_t mmc_cid_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "%08x%08x%08x%08x\n", card->raw_cid[0], card->raw_cid[1], card->raw_cid[2], card->raw_cid[3]); } static struct device_attribute dev_attr_cid = { .attr = {.name = "cid", .mode = (00400|00040|00004) }, .show = mmc_cid_show, .store = ((void *)0), };

static ssize_t mmc_csd_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "%08x%08x%08x%08x\n", card->raw_csd[0], card->raw_csd[1], card->raw_csd[2], card->raw_csd[3]); } static struct device_attribute dev_attr_csd = { .attr = {.name = "csd", .mode = (00400|00040|00004) }, .show = mmc_csd_show, .store = ((void *)0), };

static ssize_t mmc_date_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "%02d/%04d\n", card->cid.month, card->cid.year); } static struct device_attribute dev_attr_date = { .attr = {.name = "date", .mode = (00400|00040|00004) }, .show = mmc_date_show, .store = ((void *)0), };
...
static ssize_t mmc_fwrev_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "0x%x\n", card->cid.fwrev); } static struct device_attribute dev_attr_fwrev = { .attr = {.name = "fwrev", .mode = (00400|00040|00004) }, .show = mmc_fwrev_show, .store = ((void *)0), };
static ssize_t mmc_hwrev_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "0x%x\n", card->cid.hwrev); } static struct device_attribute dev_attr_hwrev = { .attr = {.name = "hwrev", .mode = (00400|00040|00004) }, .show = mmc_hwrev_show, .store = ((void *)0), };
static ssize_t mmc_manfid_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "0x%06x\n", card->cid.manfid); } static struct device_attribute dev_attr_manfid = { .attr = {.name = "manfid", .mode = (00400|00040|00004) }, .show = mmc_manfid_show, .store = ((void *)0), };
static ssize_t mmc_name_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "%s\n", card->cid.prod_name); } static struct device_attribute dev_attr_name = { .attr = {.name = "name", .mode = (00400|00040|00004) }, .show = mmc_name_show, .store = ((void *)0), };
static ssize_t mmc_oemid_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "0x%04x\n", card->cid.oemid); } static struct device_attribute dev_attr_oemid = { .attr = {.name = "oemid", .mode = (00400|00040|00004) }, .show = mmc_oemid_show, .store = ((void *)0), };
static ssize_t mmc_serial_show (struct device *dev, struct device_attribute *attr, char *buf) { struct mmc_card *card = ({ const typeof( ((struct mmc_card *)0)->dev ) *__mptr = (dev); (struct mmc_card *)( (char *)__mptr - __builtin_offsetof(struct mmc_card,dev) );}); return sprintf(buf, "0x%08x\n", card->cid.serial); } static struct device_attribute dev_attr_serial = { .attr = {.name = "serial", .mode = (00400|00040|00004) }, .show = mmc_serial_show, .store = ((void *)0), };
 ...
 struct mmc_card *oldcard)
{
 ...
 do { if (__builtin_expect(!!(!host), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ({ int __ret_warn_on = !!(!host->claimed); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/core/mmc.c", 402); __builtin_expect(!!(__ret_warn_on), 0); });
 ...
 err = mmc_send_op_cond(host, ocr | (1 << 30), ((void *)0));
 if (err)
  goto err;
 ...
 if ((card->ext_csd.hs_max_dtr != 0) &&
  (host->caps & (1 << 1))) {
  err = mmc_switch(card, (1<<0),
   185, 1);
  if (err && err != -74)
   goto free_card;

  if (err) {
   printk("<4>" "%s: switch to highspeed failed\n",
          (dev_name(&(card->host)->class_dev)));
   err = 0;
  } else {
   ((card)->state |= (1<<2));
   mmc_set_timing(card->host, 1);
  }
 }
 ...
 if ((card->csd.mmca_vsn >= 4) &&
  ...
  if (err) {
   printk("<4>" "%s: switch to bus width %d "
          "failed\n", (dev_name(&(card->host)->class_dev)),
          1 << bus_width);
   err = 0;
  } else {
...
{
 do { if (__builtin_expect(!!(!host), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 do { if (__builtin_expect(!!(!host->card), 0)) do { *(int *)0 = 0; } while (1); } while(0);
...
static int mmc_sleep(struct mmc_host *host)
{
 ...
 if (card && card->ext_csd.rev >= 3) {
  ...
  if (err < 0)
   ({ if (0) printk("<7>" "%s: Error %d while putting card into sleep", (dev_name(&(host)->class_dev)), err); 0; });
...
static int mmc_awake(struct mmc_host *host)
{
 ...
 if (card && card->ext_csd.rev >= 3) {
  ...
  if (err < 0)
   ({ if (0) printk("<7>" "%s: Error %d while awaking sleeping card", (dev_name(&(host)->class_dev)), err); 0; });
...
static void mmc_attach_bus_ops(struct mmc_host *host)
{
 ...
 if (host->caps & (1 << 8) || !mmc_assume_removable)
...
int mmc_attach_mmc(struct mmc_host *host, u32 ocr)
{
 ...
 do { if (__builtin_expect(!!(!host), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ({ int __ret_warn_on = !!(!host->claimed); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/core/mmc.c", 752); __builtin_expect(!!(__ret_warn_on), 0); });
 ...
 if (ocr & 0x7F) {
  printk("<4>" "%s: card claims to support voltages "
 ...
 err = mmc_init_card(host, host->ocr, ((void *)0));
 if (err)
  goto err;
...
err:
 ...
 printk("<3>" "%s: error %d whilst initialising MMC card\n",
  (dev_name(&(host)->class_dev)), err);
