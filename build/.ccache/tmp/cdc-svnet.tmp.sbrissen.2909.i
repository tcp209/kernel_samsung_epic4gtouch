# 1 "../../drivers/net/usb/cdc-svnet.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../drivers/net/usb/cdc-svnet.c"
# 15 "../../drivers/net/usb/cdc-svnet.c"
...
# 16 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 17 "../../drivers/net/usb/cdc-svnet.c" 2
...
{
 return usb_register_driver(driver, ((struct module *)0), "cdc_svnet");
}
...
# 18 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 19 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 20 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 21 "../../drivers/net/usb/cdc-svnet.c" 2
# 1 "../../include/linux/if_phonet.h" 1
# 16 "../../include/linux/if_phonet.h"
extern struct header_ops phonet_header_ops;
# 22 "../../drivers/net/usb/cdc-svnet.c" 2
# 1 "../../include/linux/phonet.h" 1
# 52 "../../include/linux/phonet.h"
struct phonethdr {
 __u8 pn_rdev;
 __u8 pn_sdev;
 __u8 pn_res;
...
static inline __attribute__((always_inline)) __u16 pn_object(__u8 addr, __u16 port)
{
 return (addr << 8) | (port & 0x3ff);
}

static inline __attribute__((always_inline)) __u8 pn_obj(__u16 handle)
{
 return handle & 0xff;
}

static inline __attribute__((always_inline)) __u8 pn_dev(__u16 handle)
{
 return handle >> 8;
}

static inline __attribute__((always_inline)) __u16 pn_port(__u16 handle)
{
 return handle & 0x3ff;
}

static inline __attribute__((always_inline)) __u8 pn_addr(__u16 handle)
{
 return (handle >> 8) & 0xfc;
}

static inline __attribute__((always_inline)) void pn_sockaddr_set_addr(struct sockaddr_pn *spn, __u8 addr)
{
 spn->spn_dev &= 0x03;
 spn->spn_dev |= addr & 0xfc;
}

static inline __attribute__((always_inline)) void pn_sockaddr_set_port(struct sockaddr_pn *spn, __u16 port)
{
 spn->spn_dev &= 0xfc;
...
static inline __attribute__((always_inline)) void pn_sockaddr_set_object(struct sockaddr_pn *spn,
      __u16 handle)
{
...
static inline __attribute__((always_inline)) void pn_sockaddr_set_resource(struct sockaddr_pn *spn,
      __u8 resource)
{
 spn->spn_resource = resource;
}

static inline __attribute__((always_inline)) __u8 pn_sockaddr_get_addr(const struct sockaddr_pn *spn)
{
 return spn->spn_dev & 0xfc;
}

static inline __attribute__((always_inline)) __u16 pn_sockaddr_get_port(const struct sockaddr_pn *spn)
{
 return ((spn->spn_dev & 0x03) << 8) | spn->spn_obj;
}

static inline __attribute__((always_inline)) __u16 pn_sockaddr_get_object(const struct sockaddr_pn *spn)
{
 return pn_object(spn->spn_dev, spn->spn_obj);
}

static inline __attribute__((always_inline)) __u8 pn_sockaddr_get_resource(const struct sockaddr_pn *spn)
{
...
# 23 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 24 "../../drivers/net/usb/cdc-svnet.c" 2
...
# 25 "../../drivers/net/usb/cdc-svnet.c" 2
# 1 "../../include/linux/modemctl.h" 1
# 13 "../../include/linux/modemctl.h"
...
# 14 "../../include/linux/modemctl.h" 2
# 23 "../../include/linux/modemctl.h"
struct modemctl;
struct modemctl_ops {
...
# 26 "../../drivers/net/usb/cdc-svnet.c" 2

# 1 "../../drivers/net/usb/sipc4.h" 1
# 21 "../../drivers/net/usb/sipc4.h"
enum sipc4_format {
 SIPC4_FMT,
 SIPC4_RAW,
 SIPC4_RFS,
...
extern int pdp_netif_rx(struct net_device *parent_ndev, struct sk_buff *skb,
   int svnet_ch);
# 28 "../../drivers/net/usb/cdc-svnet.c" 2
# 1 "../../drivers/net/usb/pdp.h" 1
# 19 "../../drivers/net/usb/pdp.h"
struct pdp_parent {
...
struct pdp_device {
 ...
 struct net_device *pdp_devs[(15 - 1 + 1)];
 int pdp_cnt;
...
# 29 "../../drivers/net/usb/cdc-svnet.c" 2
# 44 "../../drivers/net/usb/cdc-svnet.c"
...
struct usbsvn {
 ...
 struct usbsvn_devdata devdata[4];
...
static inline __attribute__((always_inline)) void _wake_lock_init(struct usbsvn *sn)
{
...
static inline __attribute__((always_inline)) void _wake_lock_destroy(struct usbsvn *sn)
{
 wake_lock_destroy(&sn->wlock);
 wake_lock_destroy(&sn->dormancy_lock);
}

static inline __attribute__((always_inline)) void _wake_lock(struct usbsvn *sn, int type)
{
...
static inline __attribute__((always_inline)) void _wake_unlock(struct usbsvn *sn, int type)
{
 if (sn)
  switch (type) {
  case SVNET_WLOCK_DORMANCY:
...
static inline __attribute__((always_inline)) void _wake_lock_timeout(struct usbsvn *sn, int type)
{
 switch (type) {
 case SVNET_WLOCK_DORMANCY:
 ...
 case SVNET_WLOCK_RUNTIME:
 default:
  wake_lock_timeout(&sn->wlock, msecs_to_jiffies(20));
 }
}

static inline __attribute__((always_inline)) void _wake_lock_settime(struct usbsvn *sn, long time)
{
 if (sn)
  sn->wake_time = time;
}

static inline __attribute__((always_inline)) long _wake_lock_gettime(struct usbsvn *sn)
{
 return sn ? sn->wake_time : (6*200);
}
# 180 "../../drivers/net/usb/cdc-svnet.c"
int usbsvn_request_resume(void)
{
 ...
 if (share_svn->dpm_suspending) {
  ...
  printk("<7>" "%s: suspending skip host wakeup\n",
   __func__);
  return 0;
 }
 usb_mark_last_busy(share_svn->usbdev);
 if (share_svn->resume_debug >= 1) {
  printk("<7>" "%s: resumeing, return\n", __func__);
  return 0;
 }

 if (dev->power.status != DPM_OFF) {
  _wake_lock(share_svn, SVNET_WLOCK_RUNTIME);
  printk("<7>" "%s:run time resume\n", __func__);
  share_svn->resume_debug = 1;
  err = pm_runtime_resume(dev);
  if (!err && dev->power.timer_expires == 0
   && dev->power.request_pending == false) {
   printk("<7>" "%s:run time idle\n", __func__);
   pm_runtime_idle(dev);
  }
...
extern typeof(usbsvn_request_resume) usbsvn_request_resume; static const char __kstrtab_usbsvn_request_resume[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "usbsvn_request_resume"; static const struct kernel_symbol __ksymtab_usbsvn_request_resume __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&usbsvn_request_resume, __kstrtab_usbsvn_request_resume };

static void usbsvn_runtime_start(struct work_struct *work)
{
 struct usbsvn *svn =
  ({ const typeof( ((struct usbsvn *)0)->pm_runtime_work.work ) *__mptr = (work); (struct usbsvn *)( (char *)__mptr - __builtin_offsetof(struct usbsvn,pm_runtime_work.work) );});
 ...
 if (svn->usbdev && dev->parent) {
  ...
  printk("<7>" "%s %s: " "usbsvn Runtime PM Start!!\n" , dev_driver_string(dev) , dev_name(dev));
...
{
 struct usbsvn *svn =
  ({ const typeof( ((struct usbsvn *)0)->try_reconnect_work.work ) *__mptr = (work); (struct usbsvn *)( (char *)__mptr - __builtin_offsetof(struct usbsvn,try_reconnect_work.work) );});


 if (svn->usbsvn_connected) {
  _wake_unlock(svn, SVNET_WLOCK_RUNTIME);
  printk("<6>" "svn re-connected\n");
  goto out;
 }
 if (svn->dpm_suspending) {
  _wake_lock(svn, SVNET_WLOCK_RUNTIME);
  printk("<6>" "svn suspending, delay reconnection\n");
  goto retry;
 }
 if (!svn->reconnect_cnt--) {
  _wake_unlock(svn, SVNET_WLOCK_RUNTIME);
# 265 "../../drivers/net/usb/cdc-svnet.c"
  goto out;
 }
...
{
 struct usbsvn *svn =
  ({ const typeof( ((struct usbsvn *)0)->post_resume_work ) *__mptr = (work); (struct usbsvn *)( (char *)__mptr - __builtin_offsetof(struct usbsvn,post_resume_work) );});
 struct device *dev = &svn->usbdev->dev;
 int spin = 10;
 int err;
# 322 "../../drivers/net/usb/cdc-svnet.c"
}
...
static struct device_attribute dev_attr_rx_debug = { .attr = {.name = "rx_debug", .mode = 0664 }, .show = usbsvn_rx_debug_show, .store = usbsvn_rx_debug_store, };
static struct device_attribute dev_attr_tx_debug = { .attr = {.name = "tx_debug", .mode = 0664 }, .show = usbsvn_tx_debug_show, .store = usbsvn_tx_debug_store, };
static struct device_attribute dev_attr_connected = { .attr = {.name = "connected", .mode = (00400|00040|00004) | 00400 }, .show = usbsvn_connection_show, .store = ((void *)0), };
static struct device_attribute dev_attr_waketime = { .attr = {.name = "waketime", .mode = 0664 }, .show = show_waketime, .store = store_waketime, };
...
retry:
  ...
  case RPM_SUSPENDED:
   if (share_svn->dpm_suspending || host_wakeup_done) {
    printk("<7>" "%s %s: " "DPM Suspending, spin:%d\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev) , spin2);

    if (spin2-- == 0) {
     printk("<3>" "%s %s: " "dpm resume timeout\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev));
...
# 480 "../../drivers/net/usb/cdc-svnet.c"
   if (spin2-- == 0) {
    printk("<3>" "%s %s: " "svn initiated resume, RPM_SUSPEND timeout\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev));
# 495 "../../drivers/net/usb/cdc-svnet.c"
    return -110;
   }
   msleep(20);
   goto retry;
  ...
  case RPM_SUSPENDING:
   printk("<7>" "%s %s: " "RPM Suspending, spin:%d\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev) , spin);

   if (spin-- == 0) {
    printk("<3>" "%s %s: " "Modem suspending timeout\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev));
   ...
   goto retry;
  case RPM_RESUMING:
   printk("<7>" "%s %s: " "RPM Resuming, spin:%d\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev) , spin2);

   if (spin2-- == 0) {
    printk("<3>" "%s %s: " "Modem resume timeout\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev));
...
static int usbsvn_write(struct net_device *dev, struct sipc4_tx_data *tx_data)
{
 ...
 if (err < 0) {
  printk("<3>" "%s: usbsvn_initated_resume fail\n", __func__);
  goto exit;
 }

 skb = tx_data->skb;
 dev_id = (((tx_data->res) >> 5) & 0x3);
 devdata = &svn->devdata[dev_id];

 req = usb_alloc_urb(0, ((( gfp_t)0x20u)));
 if (!req) {
  printk("<3>" "%s: can't get urb\n", __func__);
  err = -12;
  goto exit;
 }
 ...
 req->transfer_flags = 0x0040;
 err = usb_submit_urb(req, ((( gfp_t)0x20u)));
 if (err < 0) {
  printk("<3>" "%s:usb_submit_urb fail\n", __func__);
  usb_free_urb(req);
  goto exit;
 }
 usb_mark_last_busy(svn->usbdev);
 if (dev_id == SIPC4_RAW)
  _wake_lock_timeout(svn, SVNET_WLOCK_DORMANCY);
...
static netdev_tx_t usbsvn_xmit(struct sk_buff *skb, struct net_device *dev)
{
 ...
 if(!test_bit(0, ((unsigned long *)(&(&svn->tx_work)->data))))
...
{
 struct usbsvn *svn = ({ const typeof( ((struct usbsvn *)0)->tx_work ) *__mptr = (work); (struct usbsvn *)( (char *)__mptr - __builtin_offsetof(struct usbsvn,tx_work) );});
 ...
 while (skb) {
  if (skb->protocol != (( __be16)(__builtin_constant_p((__u16)((0x00F5))) ? ((__u16)( (((__u16)((0x00F5)) & (__u16)0x00ffU) << 8) | (((__u16)((0x00F5)) & (__u16)0xff00U) >> 8))) : __fswab16((0x00F5)))))
   goto drop;
...
static void tx_complete(struct urb *req)
{
 ...
 default:
  dev->stats.tx_errors++;
  printk("<3>" "%s %s: " "TX error (%d)\n" , dev_driver_string(&dev->dev) , dev_name(&dev->dev) , req->status);
 }
  ...
  case (0x80|0x40|0xB):
   ...
   for (i=0 ; i<15; i++) {
    if(pdp->pdp_devs[i]) {
     ...
     printk("<7>" "%s %s: " "svn flowctl wake queue i=%d\n" , dev_driver_string(&ndev->dev) , dev_name(&ndev->dev) , i);
  ...
  gfp_t gfp_flags)
{
 ...
 usb_fill_bulk_urb(req, svn->usbdev, devdata->rx_pipe,
   page_address(page), ((1UL) << 12), rx_complete, svn_rx);
 req->transfer_flags = 0;

 err = usb_submit_urb(req, gfp_flags);
 if (__builtin_expect(!!(err), 0)) {
  printk("<3>" "%s %s: " "RX submit error (%d)\n" , dev_driver_string(&dev->dev) , dev_name(&dev->dev) , err);
  kfree(svn_rx);
...
static void rx_complete(struct urb *req)
{
 ...
 struct usbsvn *svn = netdev_priv(dev);
 struct page *page = ({ unsigned long __pfn = ((((unsigned long)(req->transfer_buffer)) - (0xC0000000UL) + (0x40000000UL)) >> 12); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; });
 ...
 case -2:
  ...
  printk("<7>" "%s: Rx ENOENT", __func__);

 case 0:
  if (!svn->driver_info)
   flags |= (1 << 0);
  if (req->actual_length < ((1UL) << 12))
   flags |= (1 << 1);
  ...
  if (err < 0) {
   svn->devdata[dev_id].rx_skb = ((void *)0);
   break;
  }
 ...
 case -108:
  if (!svn->suspended)
   printk("<7>" "%s: RX complete Status(%d)\n",
    __func__, req->status);
  req = ((void *)0);
  break;
...
resubmit:
 ...
 if (page)
  netdev_free_page(dev, page);
 if (req && req->status != -2) {
  rx_submit(svn, dev_id, req, ((( gfp_t)0x20u)));
 }
}
...
static int usbsvn_start(struct usbsvn *svn)
{
 ...
 for (dev_id = 0; dev_id < svn->dev_count; dev_id++) {
  for (i = 0; i < rxq_size; i++) {
   int index = dev_id * rxq_size + i;
   struct urb *req = usb_alloc_urb(0, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));

   if (!req || rx_submit(svn, dev_id, req, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)))) {
    usbsvn_stop(svn);
    return -12;
   }
   svn->urbs[index] = req;
...
static int usbsvn_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
 ...
 switch (cmd) {
 case (0x89F0 + 0):
  req->ifr_ifru.ifru_phonet_autoconf.device = 0xa0;
  return 0;
 }
 return -515;
}
...
static struct usb_device_id usbsvn_ids[] = {
 { .match_flags = (0x0001 | 0x0002), .idVendor = (0x058b), .idProduct = (0x0041),
 .driver_info = 1,
 },
 { .match_flags = (0x0001 | 0x0002), .idVendor = (0x058b), .idProduct = (0x0015),
 .driver_info = 1,
 },
 { .match_flags = (0x0001 | 0x0002), .idVendor = (0x1519), .idProduct = (0x0020), },
 { },
};
...
int usbsvn_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
 ...
 if (!share_svn) {
  printk("<3>" "%s: netdev not registed\n", __func__);
  return -22;
 }
 svn = share_svn;
 ...
 if (!union_header) {
  printk("<3>" "%s %s: " "USB CDC isn't union type\n" , dev_driver_string(&intf->dev) , dev_name(&intf->dev));
  return -22;
 }
 ...
 if (dev_id >= 4) {
  printk("<3>" "%s %s: " "Device id %d cannot support\n" , dev_driver_string(&intf->dev) , dev_name(&intf->dev) , dev_id);
  return -22;
 }

 printk("<3>" "%s: probe dev_id=%d\n", __func__, dev_id);
 if (dev_id > 0)
  goto skip_netdev;
...
skip_netdev:
 if (!svn->driver_info) {
  svn = share_svn;
  if (!svn) {
   printk("<3>" "%s %s: " "svnet device doesn't be allocated\n" , dev_driver_string(&intf->dev) , dev_name(&intf->dev));
 ...
 if (((data_desc->endpoint[0].desc.bEndpointAddress) & 0x80)) {
  svn->devdata[dev_id].rx_pipe = ((3 << 30) | __create_pipe(usbdev, data_desc->endpoint[0].desc.bEndpointAddress) | 0x80);

  svn->devdata[dev_id].tx_pipe = ((3 << 30) | __create_pipe(usbdev, data_desc->endpoint[1].desc.bEndpointAddress));

 } else {
  svn->devdata[dev_id].rx_pipe = ((3 << 30) | __create_pipe(usbdev, data_desc->endpoint[1].desc.bEndpointAddress) | 0x80);

  svn->devdata[dev_id].tx_pipe = ((3 << 30) | __create_pipe(usbdev, data_desc->endpoint[0].desc.bEndpointAddress));
 ...
 if (dev_id == 0)
  printk("<6>" "%s %s: " "USB CDC SVNET device found\n" , dev_driver_string(&usbdev->dev) , dev_name(&usbdev->dev));
...
static void usbsvn_disconnect(struct usb_interface *intf)
{
 ...
 printk("<3>" "%s %s: " "%s\n" , dev_driver_string(&usbdev->dev) , dev_name(&usbdev->dev) , __func__);
 svn->dev_count--;
 ...
 if (svn->dev_count == 0) {
  ...
  if (!svn->driver_info) {
# 1147 "../../drivers/net/usb/cdc-svnet.c"
   _wake_unlock(svn, SVNET_WLOCK_RUNTIME);
  }
 }
}
...
int usbsvn_suspend(struct usb_interface *intf, pm_message_t message)
{
 ...
 printk("<7>" "%s %s: " "svn L2 p.s=%d\n" , dev_driver_string(dev) , dev_name(dev) , dev->power.status);

 svn->suspended = 1;
 usbsvn_stop(svn);

 netif_wake_queue(svn->netdev);
 _wake_lock_timeout(svn, SVNET_WLOCK_RUNTIME);
 return 0;
}
...
int usbsvn_resume(struct usb_interface *intf)
{
 ...
 if (svn->resume_debug != 1) {
  printk("<3>" "%s %s: " "not expected !!\n" , dev_driver_string(dev) , dev_name(dev));
  dump_stack();
 }

 printk("<7>" "%s %s: " "svn L0 p.s=%d\n" , dev_driver_string(dev) , dev_name(dev) , dev->power.status);
 _wake_lock(svn, SVNET_WLOCK_RUNTIME);
 svn->suspended = 0;
...
static int _netdev_init(void)
{
 ...
 netdev = alloc_netdev_mq(sizeof(*svn) + sizeof(svn->urbs[0]) * rxq_size * 4, ifname, usbsvn_setup, 1);
 ...
 ({ void *__p = (svn->urbs); size_t __n = sizeof(svn->urbs[0]) * rxq_size * 4; if ((__n) != 0) { if (__builtin_constant_p((0x00)) && (0x00) == 0) __memzero((__p),(__n)); else memset((__p),(0x00),(__n)); } (__p); });
 ...
 svn->tx_workqueue = __create_workqueue_key((("svnet_txq")), (1), (0), (0), ((void *)0), ((void *)0));
 do { do { __init_work(((&svn->tx_work)), 0); ((&svn->tx_work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&svn->tx_work))->entry); do { (((&svn->tx_work)))->func = (((usbsvn_tx_worker))); } while (0); } while (0); } while (0);
 do { do { __init_work(((&svn->post_resume_work)), 0); ((&svn->post_resume_work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&svn->post_resume_work))->entry); do { (((&svn->post_resume_work)))->func = (((usbsvn_post_resume_work))); } while (0); } while (0); } while (0);
 do { do { do { __init_work(((&(&svn->pm_runtime_work)->work)), 0); ((&(&svn->pm_runtime_work)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&svn->pm_runtime_work)->work))->entry); do { (((&(&svn->pm_runtime_work)->work)))->func = ((((usbsvn_runtime_start)))); } while (0); } while (0); } while (0); init_timer_key((&(&svn->pm_runtime_work)->timer), ((void *)0), ((void *)0)); } while (0);
 do { do { do { __init_work(((&(&svn->try_reconnect_work)->work)), 0); ((&(&svn->try_reconnect_work)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&svn->try_reconnect_work)->work))->entry); do { (((&(&svn->try_reconnect_work)->work)))->func = ((((usbsvn_try_reconnect_work)))); } while (0); } while (0); } while (0); init_timer_key((&(&svn->try_reconnect_work)->timer), ((void *)0), ((void *)0)); } while (0);
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) usbsvn_init(void)
{
 int err;
 ...
 err = _netdev_init();
 if (err < 0) {
  printk("<3>" "%s: netdev register fail\n", __func__);
  goto out;
 }

 err = usb_register(&usbsvn_driver);
 if (err < 0) {
  printk("<3>" "%s: usb register fail\n", __func__);
  goto out;
 }
out:
 return err;
...
static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) usbsvn_exit(void)
{
...
static initcall_t __initcall_usbsvn_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = usbsvn_init;;
static exitcall_t __exitcall_usbsvn_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = usbsvn_exit;;
