...
# 25 "../../drivers/mmc/host/mshci.c" 2

# 1 "../../include/linux/mmc/host.h" 1
...
# 15 "../../include/linux/mmc/host.h" 2

# 1 "../../include/linux/mmc/core.h" 1
...
struct mmc_command {
 ...
 u32 resp[4];
 unsigned int flags;
# 78 "../../include/linux/mmc/core.h"
 unsigned int retries;
 unsigned int error;
# 97 "../../include/linux/mmc/core.h"
...
static inline __attribute__((always_inline)) void mmc_claim_host(struct mmc_host *host)
{
 __mmc_claim_host(host, ((void *)0));
}

extern u32 mmc_vddrange_to_ocrmask(int vdd_min, int vdd_max);
# 17 "../../include/linux/mmc/host.h" 2
# 1 "../../include/linux/mmc/pm.h" 1
...
# 235 "../../include/linux/mmc/host.h"
 unsigned long private[0] __attribute__((__aligned__((1 << 5))));
};
...
# 251 "../../include/linux/mmc/host.h"
static inline __attribute__((always_inline)) void *mmc_priv(struct mmc_host *host)
{
 return (void *)host->private;
}
# 264 "../../include/linux/mmc/host.h"
static inline __attribute__((always_inline)) void mmc_set_bus_resume_policy(struct mmc_host *host, int manual)
{
 if (manual)
...
static inline __attribute__((always_inline)) void mmc_signal_sdio_irq(struct mmc_host *host)
{
...
static inline __attribute__((always_inline)) void mmc_set_disable_delay(struct mmc_host *host,
      unsigned int disable_delay)
{
 host->disable_delay = disable_delay;
}
# 27 "../../drivers/mmc/host/mshci.c" 2

# 1 "../../drivers/mmc/host/mshci.h" 1
# 326 "../../drivers/mmc/host/mshci.h"
struct mshci_ops;
...
static inline __attribute__((always_inline)) void mshci_writel(struct mshci_host *host, u32 val, int reg)
{
 ({ do { __asm__ __volatile__ ("dsb" : : : "memory"); outer_sync(); } while (0); ((void)((void)0, *(volatile unsigned int *)((host->ioaddr + reg)) = (( u32) (( __le32)(__u32)(val))))); });
}

static inline __attribute__((always_inline)) u32 mshci_readl(struct mshci_host *host, int reg)
{
 return ({ u32 __v = ({ u32 __v = (( __u32)(__le32)(( __le32) ((void)0, *(volatile unsigned int *)((host->ioaddr + reg))))); __v; }); __asm__ __volatile__ ("dmb" : : : "memory"); __v; });
}
...
static inline __attribute__((always_inline)) void *mshci_priv(struct mshci_host *host)
{
 return (void *)host->private;
}
...
# 32 "../../drivers/mmc/host/mshci.c" 2
# 41 "../../drivers/mmc/host/mshci.c"
...
{
 printk("<7>" "mshci" ": ============== REGISTER DUMP ==============\n");

 printk("<7>" "mshci" ": MSHCI_CTRL:      0x%08x \n",
  mshci_readl(host, 0x00));
 printk("<7>" "mshci" ": MSHCI_PWREN:     0x%08x\n",
  mshci_readl(host, 0x04));
 printk("<7>" "mshci" ": MSHCI_CLKDIV:    0x%08x\n",
  mshci_readl(host, 0x08));
 printk("<7>" "mshci" ": MSHCI_CLKSRC:    0x%08x\n",
  mshci_readl(host, 0x0C));
 printk("<7>" "mshci" ": MSHCI_CLKENA:    0x%08x\n",
  mshci_readl(host, 0x10));
 printk("<7>" "mshci" ": MSHCI_TMOUT:     0x%08x\n",
  mshci_readl(host, 0x14));
 printk("<7>" "mshci" ": MSHCI_CTYPE:     0x%08x\n",
  mshci_readl(host, 0x18));
 printk("<7>" "mshci" ": MSHCI_BLKSIZ:    0x%08x\n",
  mshci_readl(host, 0x1C));
 printk("<7>" "mshci" ": MSHCI_BYTCNT:    0x%08x\n",
  mshci_readl(host, 0x20));
 printk("<7>" "mshci" ": MSHCI_INTMSK:    0x%08x\n",
  mshci_readl(host, 0x24));
 printk("<7>" "mshci" ": MSHCI_CMDARG:    0x%08x\n",
  mshci_readl(host, 0x28));
 printk("<7>" "mshci" ": MSHCI_CMD:       0x%08x\n",
  mshci_readl(host, 0x2C));
 printk("<7>" "mshci" ": MSHCI_MINTSTS:   0x%08x\n",
  mshci_readl(host, 0x40));
 printk("<7>" "mshci" ": MSHCI_RINTSTS:   0x%08x\n",
  mshci_readl(host, 0x44));
 printk("<7>" "mshci" ": MSHCI_STATUS:    0x%08x\n",
  mshci_readl(host, 0x48));
 printk("<7>" "mshci" ": MSHCI_FIFOTH:    0x%08x\n",
  mshci_readl(host, 0x4C));
 printk("<7>" "mshci" ": MSHCI_CDETECT:   0x%08x\n",
  mshci_readl(host, 0x50));
 printk("<7>" "mshci" ": MSHCI_WRTPRT:    0x%08x\n",
  mshci_readl(host, 0x54));
 printk("<7>" "mshci" ": MSHCI_GPIO:      0x%08x\n",
  mshci_readl(host, 0x58));
 printk("<7>" "mshci" ": MSHCI_TCBCNT:    0x%08x\n",
  mshci_readl(host, 0x5C));
 printk("<7>" "mshci" ": MSHCI_TBBCNT:    0x%08x\n",
  mshci_readl(host, 0x60));
 printk("<7>" "mshci" ": MSHCI_DEBNCE:    0x%08x\n",
  mshci_readl(host, 0x64));
 printk("<7>" "mshci" ": MSHCI_USRID:     0x%08x\n",
  mshci_readl(host, 0x68));
 printk("<7>" "mshci" ": MSHCI_VERID:     0x%08x\n",
  mshci_readl(host, 0x6C));
 printk("<7>" "mshci" ": MSHCI_HCON:      0x%08x\n",
  mshci_readl(host, 0x70));
 printk("<7>" "mshci" ": MSHCI_UHS_REG:   0x%08x\n",
  mshci_readl(host, 0x74));
 printk("<7>" "mshci" ": MSHCI_BMOD:      0x%08x\n",
  mshci_readl(host, 0x80));
 printk("<7>" "mshci" ": MSHCI_PLDMND:   0x%08x\n",
  mshci_readl(host, 0x84));
 printk("<7>" "mshci" ": MSHCI_DBADDR:    0x%08x\n",
  mshci_readl(host, 0x88));
 printk("<7>" "mshci" ": MSHCI_IDSTS:     0x%08x\n",
  mshci_readl(host, 0x8C));
 printk("<7>" "mshci" ": MSHCI_IDINTEN:   0x%08x\n",
  mshci_readl(host, 0x90));
 printk("<7>" "mshci" ": MSHCI_DSCADDR:   0x%08x\n",
  mshci_readl(host, 0x94));
 printk("<7>" "mshci" ": MSHCI_BUFADDR:   0x%08x\n",
  mshci_readl(host, 0x98));
 printk("<7>" "mshci" ": MSHCI_WAKEUPCON: 0x%08x\n",
  mshci_readl(host, 0xA0));
 printk("<7>" "mshci" ": MSHCI_CLOCKCON:  0x%08x\n",
  mshci_readl(host, 0xA4));
 printk("<7>" "mshci" ": MSHCI_FIFODAT:   0x%08x\n",
  mshci_readl(host, 0x100));

  printk("<7>" "mshci" ": ===========================================\n");
}
# 145 "../../drivers/mmc/host/mshci.c"
static void mshci_clear_set_irqs(struct mshci_host *host, u32 clear, u32 set)
{
 u32 ier;

 ier = mshci_readl(host, 0x24);
 ier &= ~clear;
 ier |= set;
 mshci_writel(host, ier, 0x24);
}
...
static void mshci_reset_ciu(struct mshci_host *host)
{
 ...
 ier = mshci_readl(host, 0x00);
 ier |= (0x1<<0);

 mshci_writel(host, ier, 0x00);
 while (mshci_readl(host, 0x00) & (0x1<<0)) {
  if (timeout == 0) {
   printk("<3>" "%s: Reset CTRL never completed.\n",
    (dev_name(&(host->mmc)->class_dev)));
   mshci_dumpregs(host);
   return;
  }
  timeout--;
  ( (__builtin_constant_p(1) && (1)<=2) ? (__builtin_constant_p((1)*1000) ? (((1)*1000) > (2 * 1000) ? __bad_udelay() : __const_udelay(((1)*1000) * ((2199023U*200)>>11))) : __udelay((1)*1000)) : ({unsigned long __ms=(1); while (__ms--) (__builtin_constant_p(1000) ? ((1000) > (2 * 1000) ? __bad_udelay() : __const_udelay((1000) * ((2199023U*200)>>11))) : __udelay(1000));}));
 }
}
...
static void mshci_reset_fifo(struct mshci_host *host)
{
 ...
 mshci_writel(host, ier, 0x00);
 while (mshci_readl(host, 0x00) & (0x1<<1)) {
  if (timeout == 0) {
   printk("<3>" "%s: Reset FIFO never completed.\n",
    (dev_name(&(host->mmc)->class_dev)));
   mshci_dumpregs(host);
   return;
  }
...
static void mshci_reset_dma(struct mshci_host *host)
{
 ...
 mshci_writel(host, ier, 0x00);
 while (mshci_readl(host, 0x00) & (0x1<<2)) {
  if (timeout == 0) {
   printk("<3>" "%s: Reset DMA never completed.\n",
    (dev_name(&(host->mmc)->class_dev)));
   mshci_dumpregs(host);
   return;
  }
...
static void mshci_reset_all(struct mshci_host *host)
{
 ...
 do {
  if (!(mshci_readl(host, 0x48) & (1<<9))) {
   (__builtin_constant_p(100) ? ((100) > (2 * 1000) ? __bad_udelay() : __const_udelay((100) * ((2199023U*200)>>11))) : __udelay(100));
   if (!(mshci_readl(host, 0x48) & (1<<9))) {
    (__builtin_constant_p(100) ? ((100) > (2 * 1000) ? __bad_udelay() : __const_udelay((100) * ((2199023U*200)>>11))) : __udelay(100));
    if (!(mshci_readl(host, 0x48) & (1<<9))) {

     for (gpio = (S5PV310_GPIO_K0_START + (3)); gpio <= (S5PV310_GPIO_K0_START + (6)); gpio++) {
      s3c_gpio_cfgpin(gpio, (((0xfffffff0) | (0))));
     }
     if (!__gpio_get_value((S5PV310_GPIO_K0_START + (3))) ||
      !__gpio_get_value((S5PV310_GPIO_K0_START + (4))) ||
      !__gpio_get_value((S5PV310_GPIO_K0_START + (5))) ||
      !__gpio_get_value((S5PV310_GPIO_K0_START + (6)))) {
      err = 1;
  ...
  if (count == 0) {
   printk("<3>" "%s: Controller never released "
  ...
  count--;
  (__builtin_constant_p(10) ? ((10) > (2 * 1000) ? __bad_udelay() : __const_udelay((10) * ((2199023U*200)>>11))) : __udelay(10));
 } while (1);


 for (gpio = (S5PV310_GPIO_K0_START + (3)); gpio <= (S5PV310_GPIO_K0_START + (6)); gpio++) {
  s3c_gpio_cfgpin(gpio, (((0xfffffff0) | (3))));
 }

 if(err && host->ops->init_card) {
  printk("<3>" "%s: eMMC's data lines get low.\n"
   "Reset eMMC.\n",(dev_name(&(host->mmc)->class_dev)));
 ...
 while ((mshci_readl(host, 0x48) & (1<<31)) && count != 0 ) {
  count--;
  ( (__builtin_constant_p(1) && (1)<=2) ? (__builtin_constant_p((1)*1000) ? (((1)*1000) > (2 * 1000) ? __bad_udelay() : __const_udelay(((1)*1000) * ((2199023U*200)>>11))) : __udelay((1)*1000)) : ({unsigned long __ms=(1); while (__ms--) (__builtin_constant_p(1000) ? ((1000) > (2 * 1000) ? __bad_udelay() : __const_udelay((1000) * ((2199023U*200)>>11))) : __udelay(1000));}));
 }

 while ((mshci_readl(host, 0x48) & (1<<10)) && count2 != 0 ) {
  count2--;
  ( (__builtin_constant_p(1) && (1)<=2) ? (__builtin_constant_p((1)*1000) ? (((1)*1000) > (2 * 1000) ? __bad_udelay() : __const_udelay(((1)*1000) * ((2199023U*200)>>11))) : __udelay((1)*1000)) : ({unsigned long __ms=(1); while (__ms--) (__builtin_constant_p(1000) ? ((1000) > (2 * 1000) ? __bad_udelay() : __const_udelay((1000) * ((2199023U*200)>>11))) : __udelay(1000));}));
 }
 ...
 if ( count == 0 || count2 == 0 ) {
  int fifo_cnt,tmp;
  if (count == 0)
   printk("<3>" "%s: dma request isgnal state\n"
    "can not get inactivate state.\n",
    (dev_name(&(host->mmc)->class_dev)));
  if (count2 == 0)
   printk("<3>" "%s: data_state_mc_busy\n"
    "is never released.\n",
    (dev_name(&(host->mmc)->class_dev)));


  fifo_cnt = (mshci_readl(host,0x48)&(0x1FFF<<17))>>17;
  while(fifo_cnt) {
   tmp = mshci_readl(host, 0x100);
   fifo_cnt--;
  }
 } else {
...
static void mshci_init(struct mshci_host *host)
{
 ...
 mshci_writel(host, 0xFFFFFFFF, 0x44);

 mshci_clear_set_irqs(host, 0xFFFFFFFF,
  (0x1<<0) | (0x1<<1) |
  (0x1<<2) | (0x1<<3) | (0x1<<4) | (0x1<<5) |
  (0x1<<6) | (0x1<<7) | (0x1<<8) | (0x1<<9) |
  (0x1<<10) | (0x1<<11) | (0x1<<12) | (0x1<<13) |
  (0x1<<15));
}
...
static void mshci_read_block_pio(struct mshci_host *host)
{
 ...
 ({ if (0) printk("<7>" "mshci" " [%s()]: " "PIO reading\n", __func__); 0; });
 ...
 while (fifo_cnt) {
  ...
  len = ({ typeof(host->sg_miter.length) _min1 = (host->sg_miter.length); typeof(fifo_cnt) _min2 = (fifo_cnt); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
  ...
  while (len) {
   if (chunk == 0) {
    scratch = mshci_readl(host, 0x100);
    chunk = 4;
   }
...
static void mshci_write_block_pio(struct mshci_host *host)
{
 ...
 ({ if (0) printk("<7>" "mshci" " [%s()]: " "PIO writing\n", __func__); 0; });
...
# 456 "../../drivers/mmc/host/mshci.c"
  ...
  while (len) {
   ...
   if ((chunk == 4) || ((len == 0) && (fifo_cnt == 0))) {
    mshci_writel(host, scratch, 0x100);
    chunk = 0;
    scratch = 0;
   }
  }
...
{
 do { if (__builtin_expect(!!(!host->data), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 if (host->blocks == 0)
  return;

 if (host->data->flags & (1 << 9))
  mshci_read_block_pio(host);
 else
  mshci_write_block_pio(host);

 ({ if (0) printk("<7>" "mshci" " [%s()]: " "PIO transfer complete.\n", __func__); 0; });
}
 ...
 struct mmc_data *data)
{
 ...
 host->sg_count = dma_map_sg(((host->mmc)->parent),
 ...
 host->idma_addr = dma_map_single(((host->mmc)->parent),
    host->idma_desc,
    128 * size_idmac,
    DMA_TO_DEVICE);
 if (dma_mapping_error(((host->mmc)->parent), host->idma_addr))
  goto unmap_entries;
 do { if (__builtin_expect(!!(host->idma_addr & 0x3), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 desc_phy = (u8 *)host->idma_addr;

 for (i = 0, sg = (data->sg); i < (host->sg_count); i++, sg = sg_next(sg)) {
  addr = ((sg)->dma_address);
  len = ((sg)->length);
  ...
  ({ int __ret_warn_on = !!((desc_vir - host->idma_desc) > 128 * size_idmac); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 569); __builtin_expect(!!(__ret_warn_on), 0); });
 }




 ((struct mshci_idmac *)(desc_vir-size_idmac))->des0 |= (1<<2);
 ...
 struct mmc_data *data)
{
 ...
 dma_unmap_single(((host->mmc)->parent), host->idma_addr,
...
{
 u32 dma_irqs = ((0x1<<14)|(0x1<<5)|(0x1<<4));
 u32 pio_irqs = (0x1<<4) | (0x1<<5);
...
static void mshci_prepare_data(struct mshci_host *host, struct mmc_data *data)
{
 ...
 ({ int __ret_warn_on = !!(host->data); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 634); __builtin_expect(!!(__ret_warn_on), 0); });

 if (data == ((void *)0))
  return;

 do { if (__builtin_expect(!!(data->blksz * data->blocks > (host->mmc->max_req_size * host->mmc->max_hw_segs)), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 do { if (__builtin_expect(!!(data->blksz > host->mmc->max_blk_size), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 do { if (__builtin_expect(!!(data->blocks > 400000), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 host->data = data;
 host->data_early = 0;

 count = mshci_calc_timeout(host, data);
 mshci_writel(host, count, 0x14);
 ...
 if (host->flags & (1<<2)) {
  ...
  struct scatterlist *sg;
   ...
   for (i = 0, sg = (data->sg); i < (data->sg_len); i++, sg = sg_next(sg)) {
    if (sg->length & 0x3) {
     ({ if (0) printk("<7>" "mshci" " [%s()]: " "Reverting to PIO because of " "transfer size (%d)\n", __func__, sg->length); 0; });


     host->flags &= ~(1<<2);
     break;
   } else if (sg->offset & 0x3) {
    ({ if (0) printk("<7>" "mshci" " [%s()]: " "Reverting to PIO because of " "bad alignment\n", __func__); 0; });
 ...
 if (host->flags & (1<<2)) {
  ...
  if (ret) {
   ...
   ({ int __ret_warn_on = !!(1); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 688); __builtin_expect(!!(__ret_warn_on), 0); });
   host->flags &= ~(1<<2);
  } else {
 ...
 if (host->flags & (1<<2)) {
  ...
  mshci_writel(host, (mshci_readl(host, 0x00) |
     (0x1<<25)|(0x1<<5)),0x00);
  mshci_writel(host, (mshci_readl(host, 0x80) |
     ((0x1<<7)|(0x1<<1))),
     0x80);
  mshci_writel(host, (0x214), 0x90);
 }
 ...
 if (!(host->flags & (1<<2))) {
  ...
  printk("<3>" "it starts transfer on PIO\n");
 }

 host->data_transfered = 0;
 mshci_set_transfer_irqs(host);

 mshci_writel(host, data->blksz, 0x1C);
 mshci_writel(host, (data->blocks * data->blksz), 0x20);
}
 ...
 struct mmc_data *data)
{
 ...
 ({ int __ret_warn_on = !!(!host->data); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 737); __builtin_expect(!!(__ret_warn_on), 0); });
...
static void mshci_clock_onoff(struct mshci_host *host, bool val)
{
 ...
 if (val) {
  mshci_writel(host, (0x1<<0), 0x10);
  mshci_writel(host, 0, 0x2C);
  mshci_writel(host, ((0x1<<31) | (0x1<<21) | (0x1<<13)), 0x2C);
  do {
   if (!(mshci_readl(host, 0x2C) & (0x1<<31)))
    break;
   loop_count--;
  } while (loop_count);
 } else {
  mshci_writel(host, (0x0<<0), 0x10);
  mshci_writel(host, 0, 0x2C);
  mshci_writel(host, ((0x1<<31) | (0x1<<21) | (0x1<<13)), 0x2C);
  do {
   if (!(mshci_readl(host, 0x2C) & (0x1<<31)))
 ...
 if (loop_count == 0) {
  printk("<3>" "%s: Clock %s has been failed.\n "
    , (dev_name(&(host->mmc)->class_dev)),val ? "ON":"OFF");
 }
}
...
static void mshci_send_command(struct mshci_host *host, struct mmc_command *cmd)
{
 ...
 ({ int __ret_warn_on = !!(host->cmd); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 818); __builtin_expect(!!(__ret_warn_on), 0); });

 if (host->mmc->caps & (1 << 13)) {
 ...
 mshci_writel(host, (mshci_readl(host, 0x00) & ~(0x1<<4)),
     0x00);

 mod_timer(&host->timer, jiffies + 10 * 200);

 host->cmd = cmd;

 mshci_prepare_data(host, cmd->data);

 mshci_writel(host, cmd->arg, 0x28);

 flags = mshci_set_transfer_mode(host, cmd->data);

 if ((cmd->flags & (1 << 1)) && (cmd->flags & (1 << 3))) {
  printk("<3>" "%s: Unsupported response type!\n",
   (dev_name(&(host->mmc)->class_dev)));
  cmd->error = -22;
  tasklet_schedule(&host->finish_tasklet);
  return;
 ...
 if (cmd->flags & (1 << 2))
  flags |= (0x1<<8);
 flags |= (cmd->opcode | (0x1<<31) | (0x1<<13));

 ret = mshci_readl(host, 0x2C);
 if (ret & (0x1<<31))
  printk("<3>" "CMD busy. current cmd %d. last cmd reg 0x%x\n",
   cmd->opcode, ret);

 mshci_writel(host, flags, 0x2C);
...
static void mshci_finish_command(struct mshci_host *host)
{
 ...
 do { if (__builtin_expect(!!(host->cmd == ((void *)0)), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 if (host->cmd->flags & (1 << 0)) {
  if (host->cmd->flags & (1 << 1)) {



   for (i = 0;i < 4;i++) {
    host->cmd->resp[0] = mshci_readl(host, 0x3C);
    host->cmd->resp[1] = mshci_readl(host, 0x38);
    host->cmd->resp[2] = mshci_readl(host, 0x34);
    host->cmd->resp[3] = mshci_readl(host, 0x30);
   }
  } else {
   host->cmd->resp[0] = mshci_readl(host, 0x30);
  }
 }
    ...
    unsigned int clock, u32 bus_width)
{
 ...
 if (loop_count == 0) {
  printk("<3>" "%s: Changing clock has been failed.\n "
    , (dev_name(&(host->mmc)->class_dev)));
 }
 mshci_writel(host, mshci_readl(host, 0x2C)&(~(0x1<<21)),
     0x2C);

 mshci_clock_onoff(host, (0x1<<0));

out:
 host->clock = clock;
}
# 1022 "../../drivers/mmc/host/mshci.c"
static void mshci_set_power(struct mshci_host *host, unsigned short power)
{
 ...
 if (pwr == 0) {
  mshci_writel(host, 0, 0x04);
 } else {
  mshci_writel(host, 0x1, 0x04);
 }
}
...
static void mshci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
 ...
 ({ int __ret_warn_on = !!(host->mrq != ((void *)0)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 1058); __builtin_expect(!!(__ret_warn_on), 0); });
 ...
 if (mrq->cmd->opcode == 12) {
  ...
  for(;;) {
   do { do { ({ unsigned long __dummy; typeof(flags) __dummy2; (void)(&__dummy == &__dummy2); 1; }); flags = _raw_spin_lock_irqsave(spinlock_check(&host->lock)); } while (0); } while (0);
   if (mshci_readl(host, 0x48) & (1<<9)) {
    if (timeout == 0) {
     printk("<3>" "%s: Controller never"
      " released  data0.\n",
      (dev_name(&(host->mmc)->class_dev)));
     mshci_dumpregs(host);
    ...
    if ( host->error_state == 1) {
     ...
     expires = ktime_add_ns
      (ktime_get(), add_time);
     do { (get_current())->state = (2); __asm__ __volatile__ ("dmb" : : : "memory"); } while (0);
 ...
 if (host->quirks & (1<<1) ||
  ...
  present = !(mshci_readl(host, 0x50) & (0x1<<0));

 if (!present || host->flags & (1<<3)) {
...
# 1172 "../../drivers/mmc/host/mshci.c"
 ...
 if (ios->bus_width == 3) {
  mshci_writel(host, (0x1<<16), 0x18);
  mshci_writel(host, (0x0<<0), 0x74);
  mshci_writel(host, (0x00010001), 0x9C);
 } else if (ios->bus_width == 2) {
  mshci_writel(host, (0x1<<0), 0x18);
  mshci_writel(host, (0x0<<0), 0x74);
  mshci_writel(host, (0x00010001), 0x9C);
 } else if (ios->bus_width == 5) {
  mshci_writel(host, (0x1<<16), 0x18);
  mshci_writel(host, (0x1<<16), 0x74);
  mshci_writel(host, (0x00020002), 0x9C);
 } else if (ios->bus_width == 4) {
  mshci_writel(host, (0x1<<0), 0x18);
  mshci_writel(host, (0x1<<16), 0x74);
  mshci_writel(host, (0x00020002), 0x9C);
 } else {
  mshci_writel(host, 0, 0x74);
  mshci_writel(host, (0x0<<0), 0x18);
  mshci_writel(host, (0x00010001), 0x9C);
 }
out:
 ;
...
static int mshci_get_ro(struct mmc_host *mmc)
{
 ...
 if (host->quirks & (1<<0))
  wrtprt = host->ops->get_ro(mmc) ? 0:(0x1<<0);
 else if (host->flags & (1<<3))
  wrtprt = 0;
 else
  wrtprt = mshci_readl(host, 0x54);
...
static void mshci_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
 ...
 if (enable)
  mshci_unmask_irqs(host, (0x1<<16));
 else
  mshci_mask_irqs(host, (0x1<<16));
out:
 ;
...
static void mshci_tasklet_card(unsigned long param)
{
 ...
 if ((host->quirks & (1<<1)) ||
   (host->quirks & (1<<2)) ||
   (mshci_readl(host, 0x50) & (0x1<<0))) {
  if (host->mrq) {
   printk("<3>" "%s: Card removed during transfer!\n",
    (dev_name(&(host->mmc)->class_dev)));
   printk("<3>" "%s: Resetting controller.\n",
    (dev_name(&(host->mmc)->class_dev)));
...
static void mshci_timeout_timer(unsigned long data)
{
 ...
 if (host->mrq) {
  printk("<3>" "%s: Timeout waiting for hardware "
   "interrupt.\n", (dev_name(&(host->mmc)->class_dev)));
  mshci_dumpregs(host);
...
# 1435 "../../drivers/mmc/host/mshci.c"
static void mshci_cmd_irq(struct mshci_host *host, u32 intmask)
{
 do { if (__builtin_expect(!!(intmask == 0), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 if (!host->cmd) {
  printk("<3>" "%s: Got command interrupt 0x%08x even "
   "though no command operation was in progress.\n",
   (dev_name(&(host->mmc)->class_dev)), (unsigned)intmask);
  mshci_dumpregs(host);
  return;
 }
 ...
 if (intmask & (0x1<<8)) {
  host->cmd->error = -110;
  printk("<3>" "%s: cmd %d response timeout error\n",
    (dev_name(&(host->mmc)->class_dev)),host->cmd->opcode);
 } else if (intmask & ((0x1<<6) | (0x1<<1))) {
  host->cmd->error = -84;
  printk("<3>" "%s: cmd %d repsonse %s error\n",
    (dev_name(&(host->mmc)->class_dev)),host->cmd->opcode,
    (intmask & (0x1<<6)) ? "crc":"RE");
 }
 if (host->cmd->error) {
...
static void mshci_data_irq(struct mshci_host *host, u32 intmask, u8 intr_src)
{
 ...
 if (!host->data) {
  ...
  if (host->cmd && (host->cmd->flags & (1 << 3))) {
   if (intmask & (0x1<<3)) {
    mshci_finish_command(host);
    return;
   }
  ...
  printk("<3>" "%s: Got data interrupt 0x%08x from %s "
 ...
 if (intr_src == (0x1)) {
  if (intmask & (0x1<<10)) {
   printk("<3>" "%s: Host timeout error\n",
       (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -110;
  } else if (intmask & (0x1<<9)) {
   printk("<3>" "%s: Data read timeout error\n",
       (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -110;
  } else if (intmask & (0x1<<13)) {
   printk("<3>" "%s: FIFO Start bit error\n",
      (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  } else if (intmask & (0x1<<15)) {
   printk("<3>" "%s: FIFO Endbit/Write no CRC error\n",
       (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  } else if (intmask & (0x1<<7)) {
   printk("<3>" "%s: Data CRC error\n",
       (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  } else if (intmask & (0x1<<11)) {
   printk("<3>" "%s: FIFO underrun/overrun error\n",
       (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  }
 } else {
  if (intmask & (0x1<<2)) {
   printk("<3>" "%s: Fatal Bus error on DMA\n",
     (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  } else if (intmask & (0x1<<5)) {
   printk("<3>" "%s: Card error on DMA\n",
     (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  } else if (intmask & (0x1<<4)) {
   printk("<3>" "%s: Description error on DMA\n",
     (dev_name(&(host->mmc)->class_dev)));
   host->data->error = -5;
  }
 }
...
static irqreturn_t mshci_irq(int irq, void *dev_id)
{
 ...
 intmask = mshci_readl(host, 0x40);

 if (!intmask || intmask == 0xffffffff) {

  intmask = mshci_readl(host, 0x8C);
  if (intmask) {
   mshci_writel(host, intmask,0x8C);
   mshci_data_irq(host, intmask, (0x0));
   result = IRQ_HANDLED;
   goto out;
   }
  result = IRQ_NONE;
 ...
 ({ if (0) printk("<7>" "mshci" " [%s()]: " "*** %s got interrupt: 0x%08x\n", __func__, (dev_name(&(host->mmc)->class_dev)), intmask); 0; });


 mshci_writel(host, intmask, 0x44);

 if (intmask & ((0x1<<0))) {
  if(!(host->mmc->caps & (1 << 8)))
   tasklet_schedule(&host->card_tasklet);
 }
 intmask &= ~(0x1<<0);

 if (intmask & ((0x1<<8)|(0x1<<6)|(0x1<<2)|(0x1<<1))) {
  if ( !(intmask & (0x1<<2)) && (intmask & (0x1<<8))) {
   ...
   while ( --timeout &&
    !(mshci_readl(host, 0x40)
      & (0x1<<2)));
   if (!timeout)
    printk("<3>""*** %s time out for					CDONE intr\n",

     (dev_name(&(host->mmc)->class_dev)));
   else
    mshci_writel(host, (0x1<<2),
     0x44);
   mshci_cmd_irq(host, intmask & ((0x1<<8)|(0x1<<6)|(0x1<<2)|(0x1<<1)));
  } else {
   mshci_cmd_irq(host, intmask & ((0x1<<8)|(0x1<<6)|(0x1<<2)|(0x1<<1)));
  }
 }

 if (intmask & (((0x1<<15)|(0x1<<13)|(0x1<<12)|(0x1<<11) | (0x1<<15) |(0x1<<7))|((0x1<<10)|(0x1<<9))|(0x1<<5)|(0x1<<4)|(0x1<<3))) {
  if ( !(intmask & (0x1<<3)) && (intmask & (0x1<<9))) {
   ...
   if (!timeout)
    ...
    mshci_writel(host, (0x1<<3),
     0x44);
   mshci_data_irq(host, intmask & (((0x1<<15)|(0x1<<13)|(0x1<<12)|(0x1<<11) | (0x1<<15) |(0x1<<7))|((0x1<<10)|(0x1<<9))|(0x1<<5)|(0x1<<4)|(0x1<<3)),(0x1));
  } else {
   mshci_data_irq(host, intmask & (((0x1<<15)|(0x1<<13)|(0x1<<12)|(0x1<<11) | (0x1<<15) |(0x1<<7))|((0x1<<10)|(0x1<<9))|(0x1<<5)|(0x1<<4)|(0x1<<3)),(0x1));
  }
 }

 intmask &= ~(((0x1<<8)|(0x1<<6)|(0x1<<2)|(0x1<<1)) | (((0x1<<15)|(0x1<<13)|(0x1<<12)|(0x1<<11) | (0x1<<15) |(0x1<<7))|((0x1<<10)|(0x1<<9))|(0x1<<5)|(0x1<<4)|(0x1<<3)));
 ...
 if (intmask) {
  printk("<3>" "%s: Unexpected interrupt 0x%08x.\n",
   (dev_name(&(host->mmc)->class_dev)), intmask);
  mshci_dumpregs(host);
 }
...
extern typeof(mshci_suspend_host) mshci_suspend_host; static const char __kstrtab_mshci_suspend_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_suspend_host"; static const struct kernel_symbol __ksymtab_mshci_suspend_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_suspend_host, __kstrtab_mshci_suspend_host };
...
int mshci_resume_host(struct mshci_host *host)
{
 ...
 ret = request_irq(host->irq, mshci_irq, 0x00000080,
     (dev_name(&(host->mmc)->class_dev)), host);
 if (ret)
  return ret;
 ...
 mshci_writel(host, 0xfffff, 0x64);


 mshci_writel(host, 0x0, 0x18);


 if (host->flags & (1<<1)) {
  mshci_writel(host, (0x1<<0), 0x80);
  count = 100;
  while( (mshci_readl(host, 0x80) & (0x1<<0) )
   && --count ) ;
...
extern typeof(mshci_resume_host) mshci_resume_host; static const char __kstrtab_mshci_resume_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_resume_host"; static const struct kernel_symbol __ksymtab_mshci_resume_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_resume_host, __kstrtab_mshci_resume_host };
# 1755 "../../drivers/mmc/host/mshci.c"
 ...
 size_t priv_size)
{
 ...
 ({ int __ret_warn_on = !!(dev == ((void *)0)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 1761); __builtin_expect(!!(__ret_warn_on), 0); });
...
static void mshci_fifo_init(struct mshci_host *host)
{
 ...
 fifo_val = mshci_readl(host, 0x4C);
 fifo_depth = ((fifo_val & (0xFFF<<16))>>16)+1;
 fifo_threshold = fifo_depth/2;
 host->fifo_threshold = fifo_threshold;
 host->fifo_depth = fifo_threshold*2;

 printk("<6>" "%s: FIFO WMARK FOR RX 0x%x WX 0x%x. ###########\n",
  (dev_name(&(host->mmc)->class_dev)),fifo_depth,((fifo_val & (0xFFF<<0))>>16)+1 );

 fifo_val &= ~((0xFFF<<16) | (0xFFF<<0) | (0x7<<28));


 fifo_val |= (0x8 | (0x8<<16));
 fifo_val |= (2<<28);

 mshci_writel(host, fifo_val, 0x4C);
}

extern typeof(mshci_alloc_host) mshci_alloc_host; static const char __kstrtab_mshci_alloc_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_alloc_host"; static const struct kernel_symbol __ksymtab_mshci_alloc_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_alloc_host, __kstrtab_mshci_alloc_host };
...
int mshci_add_host(struct mshci_host *host)
{
 ...
 ({ int __ret_warn_on = !!(host == ((void *)0)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../drivers/mmc/host/mshci.c", 1802); __builtin_expect(!!(__ret_warn_on), 0); });
 if (host == ((void *)0))
  return -22;
 ...
 host->version = mshci_readl(host, 0x6C);
 ...
 if (host->flags & (1<<1)) {
  ...
  if (!host->idma_desc) {
   ...
   printk("<4>" "%s: Unable to allocate IDMA "
 ...
 if (!(host->flags & ((1<<1)))) {
  host->dma_mask = (((64) == 64) ? ~0ULL : ((1ULL<<(64))-1));
  ((host->mmc)->parent)->dma_mask = &host->dma_mask;
 }

 printk("<3>" "%s: Version ID 0x%x.\n",
  (dev_name(&(host->mmc)->class_dev)), host->version);
 ...
 mmc->ocr_avail |= 0x00020000|0x00040000;


 if (mmc->ocr_avail == 0) {
  printk("<3>" "%s: Hardware doesn't report any "
   "support voltages.\n", (dev_name(&(mmc)->class_dev)));
  return -19;
 }

 do { spinlock_check(&host->lock); do { *(&(&host->lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
...
# 1906 "../../drivers/mmc/host/mshci.c"
 ...
 setup_timer_key((&host->timer), ((void *)0), ((void *)0), (mshci_timeout_timer), ((unsigned long)host));
 if (host->mmc->caps & (1 << 13))
  setup_timer_key((&host->clock_timer), ((void *)0), ((void *)0), (mshci_clock_gate_timer), ((unsigned long)host));
 ...
 printk("<6>" "%s: MSHCI controller on %s [%s] using %s\n",
  (dev_name(&(mmc)->class_dev)), host->hw_name, dev_name(((mmc)->parent)),
  (host->flags & (1<<1)) ? "IDMA" : "PIO");
...
extern typeof(mshci_add_host) mshci_add_host; static const char __kstrtab_mshci_add_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_add_host"; static const struct kernel_symbol __ksymtab_mshci_add_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_add_host, __kstrtab_mshci_add_host };
...
void mshci_remove_host(struct mshci_host *host, int dead)
{
 ...
 if (dead) {
  ...
  if (host->mrq) {
   printk("<3>" "%s: Controller removed during "
    " transfer!\n", (dev_name(&(host->mmc)->class_dev)));
 ...
 host->idma_desc = ((void *)0);
 host->align_buffer = ((void *)0);
}

extern typeof(mshci_remove_host) mshci_remove_host; static const char __kstrtab_mshci_remove_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_remove_host"; static const struct kernel_symbol __ksymtab_mshci_remove_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_remove_host, __kstrtab_mshci_remove_host };
...
extern typeof(mshci_free_host) mshci_free_host; static const char __kstrtab_mshci_free_host[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "mshci_free_host"; static const struct kernel_symbol __ksymtab_mshci_free_host __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&mshci_free_host, __kstrtab_mshci_free_host };
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) mshci_drv_init(void)
{
 printk("<6>" "mshci"
  ": Mobile Storage Host Controller Interface driver\n");
 printk("<6>" "mshci" ": Copyright(c) Pierre Ossman\n");

 return 0;
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) mshci_drv_exit(void)
{
}

static initcall_t __initcall_mshci_drv_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = mshci_drv_init;;
static exitcall_t __exitcall_mshci_drv_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = mshci_drv_exit;;

static inline __attribute__((always_inline)) unsigned int *__check_debug_quirks(void) { return(&(debug_quirks)); }; static int __param_perm_check_debug_quirks __attribute__((unused)) = (sizeof(struct { int:-!!((0444) < 0 || (0444) > 0777 || ((0444) & 2)); })) + (sizeof(struct { int:-!!(sizeof("""mshci" ".") > (64 - sizeof(unsigned long))); })); static const char __param_str_debug_quirks[] = "mshci" "." "debug_quirks"; static struct kernel_param const __param_debug_quirks __attribute__((__used__)) __attribute__ ((unused,__section__ ("__param"),aligned(sizeof(void *)))) = { __param_str_debug_quirks, 0444, __builtin_types_compatible_p(typeof(*(&debug_quirks)), typeof(bool)) ? 2 : 0, param_set_uint, param_get_uint, { &debug_quirks } }; ;
