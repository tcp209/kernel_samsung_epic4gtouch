# 1 "../../fs/fat/namei_vfat.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../fs/fat/namei_vfat.c"
# 18 "../../fs/fat/namei_vfat.c"
...
# 25 "../../fs/fat/namei_vfat.c" 2
# 34 "../../fs/fat/namei_vfat.c"
...
static inline __attribute__((always_inline)) wchar_t vfat_bad_char(wchar_t w)
{
 return (w < 0x0020)
     || (w == '*') || (w == '?') || (w == '<') || (w == '>')
     || (w == '|') || (w == '"') || (w == ':') || (w == '/')
...
static inline __attribute__((always_inline)) wchar_t vfat_replace_char(wchar_t w)
{
 return (w == '[') || (w == ']') || (w == ';') || (w == ',')
     || (w == '+') || (w == '=');
}
...
static inline __attribute__((always_inline)) int vfat_is_used_badchars(const wchar_t *s, int len)
{
 int i;
...
static inline __attribute__((always_inline)) int to_shortname_char(struct nls_table *nls,
        ...
        wchar_t *src, struct shortname_info *info)
{
 ...
 if (len <= 0) {
  ...
  if ((((_ctype[(int)(unsigned char)(buf[0])])&(0x01|0x02)) != 0)) {
   if (buf[0] == prev)
    info->lower = 0;
   else
    info->upper = 0;
     ...
     unsigned char *name_res, unsigned char *lcase)
{
 ...
 is_shortname = 1;
 do { (&base_info)->lower = 1; (&base_info)->upper = 1; (&base_info)->valid = 1; } while (0);
 do { (&ext_info)->lower = 1; (&ext_info)->upper = 1; (&ext_info)->valid = 1; } while (0);
 ...
 ({ void *__p = (name_res); size_t __n = 11; if ((__n) != 0) { if (__builtin_constant_p((' ')) && (' ') == 0) __memzero((__p),(__n)); else memset((__p),(' '),(__n)); } (__p); });
 ...
 *lcase = 0;
 if (is_shortname && base_info.valid && ext_info.valid) {
  ...
  if (opts->shortname & 0x0100) {
   return (base_info.upper && ext_info.upper);
  } else if (opts->shortname & 0x0200) {
 ...
 *nr_slots = 0;
 ...
 uname = kmem_cache_alloc(names_cachep, (((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u))));
 if (!uname)
  return -12;
...
shortname:
 ...
 (*nr_slots)++;
 memcpy(de->name, msdos_name, 11);
 de->attr = is_dir ? 16 : 32;
 de->lcase = lcase;
 fat_time_unix2fat(sbi, ts, &time, &date, &time_cs);
 de->time = de->ctime = time;
 de->date = de->cdate = de->adate = date;
 de->ctime_cs = time_cs;
 de->start = (( __le16)(__u16)(cluster));
 de->starthi = (( __le16)(__u16)(cluster >> 16));
 de->size = 0;
out_free:
 kmem_cache_free(names_cachep, (void *)(uname));
 return err;
}
     ...
     struct fat_slot_info *sinfo)
{
 ...
 slots = kmalloc(sizeof(*slots) * 21, ((( gfp_t)0x10u) | (( gfp_t)0x40u)));
 if (slots == ((void *)0))
  return -12;
...
{
 return ((dentry) == (dentry)->d_parent) && (dentry->d_flags & 0x0004);
}
      ...
      struct nameidata *nd)
{
 ...
 if (alias && !vfat_d_anon_disconn(alias)) {
  ...
  do { if (__builtin_expect(!!(d_unhashed(alias)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
  if (!(((inode->i_mode) & 00170000) == 0040000))
   d_move(alias, dentry);
  iput(inode);
  unlock_super(sb);
         ...
         struct nameidata *nd)
{
 ...
 ts = ((struct timespec) { get_seconds(), 0 });
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
 ...
 clear_nlink(inode);
 inode->i_mtime = inode->i_atime = ((struct timespec) { get_seconds(), 0 });
 fat_detach(inode);
out:
 unlock_super(sb);
         ...
         struct inode *new_dir, struct dentry *new_dentry)
{
 ...
 old_sinfo.bh = sinfo.bh = dotdot_bh = ((void *)0);
 ...
 is_dir = (((old_inode->i_mode) & 00170000) == 0040000);
 ...
 if ((((new_dir)->i_sb->s_flags & (16|128)) || ((new_dir)->i_flags & (1|64)))) {
 ...
 if (update_dotdot) {
  ...
  dotdot_de->start = (( __le16)(__u16)(start));
  dotdot_de->starthi = (( __le16)(__u16)(start >> 16));
 ...
 if ((((old_dir)->i_sb->s_flags & (16|128)) || ((old_dir)->i_flags & (1|64))))
...
error_inode:
 ...
 if (corrupt < 0) {
  __fat_fs_error(new_dir->i_sb, 1, "%s: Filesystem corrupted (i_pos %lld)" , __func__, sinfo.i_pos);
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) init_vfat_fs(void)
{
 return register_filesystem(&vfat_fs_type);
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) exit_vfat_fs(void)
{
...
static initcall_t __initcall_init_vfat_fs6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = init_vfat_fs;
static exitcall_t __exitcall_exit_vfat_fs __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = exit_vfat_fs;
