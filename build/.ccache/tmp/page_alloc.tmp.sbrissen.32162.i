...
# 1 "../../arch/arm/include/asm/dma.h" 1
...
static inline __attribute__((always_inline)) void oom_killer_disable(void)
{
 oom_killer_disabled = true;
}

static inline __attribute__((always_inline)) void oom_killer_enable(void)
{
...
# 114 "../../include/linux/stop_machine.h"
int stop_machine(int (*fn)(void *), void *data, const struct cpumask *cpus);
# 125 "../../include/linux/stop_machine.h"
...
# 108 "../../include/linux/page_cgroup.h"
struct page_cgroup;

static inline __attribute__((always_inline)) void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) pgdat_page_cgroup_init(struct pglist_data *pgdat)
{
}
...
static inline __attribute__((always_inline)) void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) page_cgroup_init_flatmem(void)
{
}
# 140 "../../include/linux/page_cgroup.h"
static inline __attribute__((always_inline))
...
# 1 "../../include/linux/compaction.h" 1
# 56 "../../include/linux/compaction.h"
static inline __attribute__((always_inline)) unsigned long try_to_compact_pages(struct zonelist *zonelist,
   int order, gfp_t gfp_mask, nodemask_t *nodemask)
{
 return 1;
}

static inline __attribute__((always_inline)) void defer_compaction(struct zone *zone)
{
}

static inline __attribute__((always_inline)) bool compaction_deferred(struct zone *zone)
{
 return 1;
}
# 79 "../../include/linux/compaction.h"
static inline __attribute__((always_inline)) int compaction_register_node(struct node *node)
{
 return 0;
}

static inline __attribute__((always_inline)) void compaction_unregister_node(struct node *node)
{
}
# 53 "../../mm/page_alloc.c" 2
# 1 "../../include/trace/events/kmem.h" 1
# 344 "../../include/trace/events/kmem.h"
# 1 "../../include/trace/define_trace.h" 1
# 344 "../../include/trace/events/kmem.h" 2
...
extern typeof(node_states) node_states; static const char __kstrtab_node_states[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "node_states"; static const struct kernel_symbol __ksymtab_node_states __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&node_states, __kstrtab_node_states };

unsigned long totalram_pages ;
unsigned long totalreserve_pages ;
int percpu_pagelist_fraction;
gfp_t gfp_allowed_mask = ((( gfp_t)((1 << 22) - 1)) & ~((( gfp_t)0x10u)|(( gfp_t)0x40u)|(( gfp_t)0x80u)));
# 106 "../../mm/page_alloc.c"
void set_gfp_allowed_mask(gfp_t mask)
{
 ({ int __ret_warn_on = !!(!mutex_is_locked(&pm_mutex)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../mm/page_alloc.c", 108); __builtin_expect(!!(__ret_warn_on), 0); });
...
gfp_t clear_gfp_allowed_mask(gfp_t mask)
{
 ...
 ({ int __ret_warn_on = !!(!mutex_is_locked(&pm_mutex)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../mm/page_alloc.c", 116); __builtin_expect(!!(__ret_warn_on), 0); });
 gfp_allowed_mask &= ~mask;
 return ret;
}
...
# 139 "../../mm/page_alloc.c"
int sysctl_lowmem_reserve_ratio[3 -1] = {
...
static char * const zone_names[3] = {
...
static unsigned long __attribute__ ((__section__(".meminit.data"))) nr_kernel_pages;
static unsigned long __attribute__ ((__section__(".meminit.data"))) nr_all_pages;
...
static void bad_page(struct page *page)
{
 ...
 if (nr_shown++ == 0)
  resume = jiffies + 60 * 200;

 printk("<1>" "BUG: Bad page state in process %s  pfn:%05lx\n",
  (get_current())->comm, ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); }));
 dump_page(page);
  ...
  int migratetype)
{
 ...
 page_idx = ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); }) & ((1 << 11) - 1);
...
{
 if (__builtin_expect(!!(page_mapcount(page) | (page->mapping != ((void *)0)) | ((*(volatile int *)&(&page->_count)->counter) != 0) | (page->flags & (1 << PG_lru | 1 << PG_locked | 1 << PG_private | 1 << PG_private_2 | 1 << PG_buddy | 1 << PG_writeback | 1 << PG_reserved | 1 << PG_slab | 1 << PG_swapcache | 1 << PG_active | 1 << PG_unevictable | (1 << PG_mlocked) | 0))), 0)) {
...
# 609 "../../mm/page_alloc.c"
  ...
  do {
   ...
   __free_one_page(page, zone, 0, ((page)->private));
   trace_mm_page_pcpu_drain(page, 0, ((page)->private));
...
static bool free_pages_prepare(struct page *page, unsigned int order)
{
 ...
 if (!is_highmem(page_zone(page))) {
  debug_check_no_locks_freed(page_address(page),((1UL) << 12)<<order);
...
static void __free_pages_ok(struct page *page, unsigned int order)
{
 ...
 do { ({ unsigned long __dummy; typeof(flags) __dummy2; (void)(&__dummy == &__dummy2); 1; }); ({ __asm__ __volatile__( "mrs	%0, cpsr		@ local_irq_save\n" "cpsid	i" : "=r" (flags) : : "memory", "cc"); }); do { } while (0); } while (0);
 if (__builtin_expect(!!(wasMlocked), 0))
...
void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) __free_pages_bootmem(struct page *page, unsigned int order)
{
 if (order == 0) {
...
static inline __attribute__((always_inline)) int check_new_page(struct page *page)
{
 if (__builtin_expect(!!(page_mapcount(page) | (page->mapping != ((void *)0)) | ((*(volatile int *)&(&page->_count)->counter) != 0) | (page->flags & ((1 << 22) - 1))), 0)) {
...
static int prep_new_page(struct page *page, int order, gfp_t gfp_flags)
{
 ...
 for (i = 0; i < (1 << order); i++) {
  struct page *p = page + i;
  if (__builtin_expect(!!(check_new_page(p)), 0))
   return 1;
 }
     ...
     int migratetype)
{
 ...
 int pages_moved = 0;
# 850 "../../mm/page_alloc.c"
 do { if (__builtin_expect(!!(page_zone(start_page) != page_zone(end_page)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
    ...
    int migratetype)
{
 ...
 start_pfn = ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); });
 start_pfn = start_pfn & ~((1UL << (11 -1))-1);
 start_page = ({ unsigned long __pfn = (start_pfn); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; });
...
{
 int nr_pageblocks = 1 << (start_order - (11 -1));

 while (nr_pageblocks--) {
  set_pageblock_migratetype(pageblock_page, migratetype);
  pageblock_page += (1UL << (11 -1));
 }
}
...
retry_reserve:
 ...
 if (__builtin_expect(!!(!page), 0) && migratetype != 3) {
   ...
   int migratetype, int cold)
{
 ...
 for (i = 0; i < count; ++i) {
  ...
  if (__builtin_expect(!!(page == ((void *)0)), 0))
   break;
# 1036 "../../mm/page_alloc.c"
  if (__builtin_expect(!!(cold == 0), 1))
...
# 1080 "../../mm/page_alloc.c"
static void drain_pages(unsigned int cpu)
{
 unsigned long flags;
 struct zone *zone;

 for (zone = (first_online_pgdat())->node_zones; zone; zone = next_zone(zone)) if (!populated_zone(zone)) ; else {
  struct per_cpu_pageset *pset;
  struct per_cpu_pages *pcp;

  do { ({ unsigned long __dummy; typeof(flags) __dummy2; (void)(&__dummy == &__dummy2); 1; }); ({ __asm__ __volatile__( "mrs	%0, cpsr		@ local_irq_save\n" "cpsid	i" : "=r" (flags) : : "memory", "cc"); }); do { } while (0); } while (0);
  pset = ({ do { const void *__vpp_verify = (typeof(((zone->pageset))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((zone->pageset))) *)((zone->pageset)))); (typeof((typeof(*((zone->pageset))) *)((zone->pageset)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); });
...
{
 drain_pages((current_thread_info()->cpu));
}
...
# 1180 "../../mm/page_alloc.c"
 if (migratetype >= 3) {
  if (__builtin_expect(!!(migratetype == 4), 0)) {
 ...
 pcp = &({ do { const void *__vpp_verify = (typeof((zone->pageset)))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(zone->pageset)) *)(zone->pageset))); (typeof((typeof(*(zone->pageset)) *)(zone->pageset))) (__ptr + (((__per_cpu_offset[(current_thread_info()->cpu)])))); }); })->pcp;
 if (cold)
...
void split_page(struct page *page, unsigned int order)
{
 ...
 do { } while (0);
 do { } while (0);
# 1227 "../../mm/page_alloc.c"
 for (i = 1; i < (1 << order); i++)
  set_page_refcounted(page + i);
}
# 1241 "../../mm/page_alloc.c"
int split_free_page(struct page *page)
{
 unsigned int order;
 ...
 do { if (__builtin_expect(!!(!PageBuddy(page)), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 zone = page_zone(page);
 order = page_order(page);


 watermark = (zone->watermark[WMARK_LOW]) + (1 << order);
 ...
 if (order >= (11 -1) - 1) {
  ...
  for (; page < endpage; page += (1UL << (11 -1)))
 ...
 if (__builtin_expect(!!(order == 0), 1)) {
  ...
  if (list_empty(list)) {
   pcp->count += rmqueue_bulk(zone, 0,
     pcp->batch, list,
     migratetype, cold);
   if (__builtin_expect(!!(list_empty(list)), 0))
    goto failed;
  }
  ...
  if (cold)
   ...
   page = ({ const typeof( ((struct page *)0)->lru ) *__mptr = (list->next); (struct page *)( (char *)__mptr - __builtin_offsetof(struct page,lru) );});

  list_del(&page->lru);
  pcp->count--;
 } else {
  if (__builtin_expect(!!(gfp_flags & (( gfp_t)0x800u)), 0)) {
# 1325 "../../mm/page_alloc.c"
   ({ static bool __warned; int __ret_warn_once = !!(order > 1); if (__builtin_expect(!!(__ret_warn_once), 0)) if (({ int __ret_warn_on = !!(!__warned); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../mm/page_alloc.c", 1325); __builtin_expect(!!(__ret_warn_on), 0); })) __warned = true; __builtin_expect(!!(__ret_warn_once), 0); });
  }
  do { do { ({ unsigned long __dummy; typeof(flags) __dummy2; (void)(&__dummy == &__dummy2); 1; }); flags = _raw_spin_lock_irqsave(spinlock_check(&zone->lock)); } while (0); } while (0);
...
zonelist_scan:
 ...
 for (z = first_zones_zonelist(zonelist, high_zoneidx, nodemask, &zone); zone; z = next_zones_zonelist(++z, high_zoneidx, nodemask, &zone)) {
  ...
  ((void)(sizeof(struct { int:-!!(0x04 < NR_WMARK); })));
  if (!(alloc_flags & 0x04)) {
   unsigned long mark;
   int ret;

   mark = zone->watermark[alloc_flags & (0x04 -1)];
    ...
    unsigned long pages_reclaimed)
{
 ...
 if (order <= 3)
  return 1;
# 1721 "../../mm/page_alloc.c"
 if (gfp_mask & (( gfp_t)0x400u) && pages_reclaimed < (1 << order))
  return 1;
 ...
 int migratetype, unsigned long *did_some_progress)
{
 ...
 ({ __might_sleep("../../mm/page_alloc.c", 1850, 0); _cond_resched(); });
 ...
 *did_some_progress = try_to_free_pages(zonelist, order, gfp_mask, nodemask);
 ...
 ({ __might_sleep("../../mm/page_alloc.c", 1865, 0); _cond_resched(); });

 if (__builtin_expect(!!(!(*did_some_progress)), 0))
  return ((void *)0);
 ...
 int migratetype)
{
 ...
 do {
  page = get_page_from_freelist(gfp_mask, nodemask, order,
   zonelist, high_zoneidx, 0x04,
   preferred_zone, migratetype);

  if (!page && gfp_mask & (( gfp_t)0x800u))
   congestion_wait(BLK_RW_ASYNC, 200/50);
 } while (!page && (gfp_mask & (( gfp_t)0x800u)));
...
{
 struct task_struct *p = (get_current());
 int alloc_flags = WMARK_MIN | 0x40;
 const gfp_t wait = gfp_mask & (( gfp_t)0x10u);


 ((void)(sizeof(struct { int:-!!((( gfp_t)0x20u) != 0x20); })));
 ...
 alloc_flags |= (gfp_mask & (( gfp_t)0x20u));
 ...
 if (!wait) {
  ...
  alloc_flags &= ~0x40;
 } else if (__builtin_expect(!!(rt_task(p)), 0) && !(((current_thread_info()->preempt_count) & ((((1UL << (10))-1) << ((0 + 8) + 8)) | (((1UL << (8))-1) << (0 + 8)) | (((1UL << (1))-1) << (((0 + 8) + 8) + 10))))))
  alloc_flags |= 0x10;

 if (__builtin_expect(!!(!(gfp_mask & (( gfp_t)0x10000u))), 1)) {
  if (!(((current_thread_info()->preempt_count) & ((((1UL << (10))-1) << ((0 + 8) + 8)) | (((1UL << (8))-1) << (0 + 8)) | (((1UL << (1))-1) << (((0 + 8) + 8) + 10))))) &&
      ((p->flags & 0x00000800) ||
       __builtin_expect(!!(test_ti_thread_flag(current_thread_info(), 18)), 0)))
   alloc_flags |= 0x04;
 }
 ...
 int migratetype)
{
 ...
 if (order >= 11) {
  ({ static bool __warned; int __ret_warn_once = !!(!(gfp_mask & (( gfp_t)0x200u))); if (__builtin_expect(!!(__ret_warn_once), 0)) if (({ int __ret_warn_on = !!(!__warned); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../mm/page_alloc.c", 1982); __builtin_expect(!!(__ret_warn_on), 0); })) __warned = true; __builtin_expect(!!(__ret_warn_once), 0); });
  return ((void *)0);
 }
# 1994 "../../mm/page_alloc.c"
...
rebalance:
 ...
 if (p->flags & 0x00000800)
  goto nopage;


 if (test_ti_thread_flag(current_thread_info(), 18) && !(gfp_mask & (( gfp_t)0x800u)))
  goto nopage;
 ...
 if (!did_some_progress) {
  if ((gfp_mask & (( gfp_t)0x80u)) && !(gfp_mask & (( gfp_t)0x1000u))) {
...
nopage:
 if (!(gfp_mask & (( gfp_t)0x200u)) && __printk_ratelimit(__func__)) {
   ...
   struct zonelist *zonelist, nodemask_t *nodemask)
{
 ...
 do { if (gfp_mask & (( gfp_t)0x10u)) do { do { } while (0); } while (0); } while (0);
 ...
 if (__builtin_expect(!!(!zonelist->_zonerefs->zone), 0))
  return ((void *)0);
 ...
 page = get_page_from_freelist(gfp_mask|(( gfp_t)0x20000u), nodemask, order,
   zonelist, high_zoneidx, WMARK_LOW|0x40,
   preferred_zone, migratetype);
...
{
 return __get_free_pages(gfp_mask | (( gfp_t)0x8000u), 0);
}
...
void free_pages(unsigned long addr, unsigned int order)
{
 if (addr != 0) {
  do { } while (0);
  __free_pages(({ unsigned long __pfn = ((((unsigned long)((void *)addr)) - (0xC0000000UL) + (0x40000000UL)) >> 12); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; }), order);
 }
}

extern typeof(free_pages) free_pages; static const char __kstrtab_free_pages[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "free_pages"; static const struct kernel_symbol __ksymtab_free_pages __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&free_pages, __kstrtab_free_pages };
# 2230 "../../mm/page_alloc.c"
void *alloc_pages_exact(size_t size, gfp_t gfp_mask)
{
 unsigned int order = get_order(size);
 unsigned long addr;
 ...
 if (addr) {
  ...
  split_page(({ unsigned long __pfn = ((((unsigned long)((void *)addr)) - (0xC0000000UL) + (0x40000000UL)) >> 12); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; }), order);
...
extern typeof(alloc_pages_exact) alloc_pages_exact; static const char __kstrtab_alloc_pages_exact[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "alloc_pages_exact"; static const struct kernel_symbol __ksymtab_alloc_pages_exact __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&alloc_pages_exact, __kstrtab_alloc_pages_exact };
# 2258 "../../mm/page_alloc.c"
void free_pages_exact(void *virt, size_t size)
{
...
static unsigned int nr_free_zone_pages(int offset)
{
 ...
 struct zonelist *zonelist = node_zonelist(numa_node_id(), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
...
{
 return nr_free_zone_pages(gfp_zone(((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u) | (( gfp_t)0x02u) | (( gfp_t)0x08u))));
}
...
extern typeof(si_meminfo) si_meminfo; static const char __kstrtab_si_meminfo[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "si_meminfo"; static const struct kernel_symbol __ksymtab_si_meminfo __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&si_meminfo, __kstrtab_si_meminfo };
# 2352 "../../mm/page_alloc.c"
void show_free_areas(void)
{
 ...
 for (zone = (first_online_pgdat())->node_zones; zone; zone = next_zone(zone)) if (!populated_zone(zone)) ; else {
  ...
  for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_online_mask)), ((cpu)) < nr_cpu_ids;) {
   ...
   pageset = ({ do { const void *__vpp_verify = (typeof(((zone->pageset))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((zone->pageset))) *)((zone->pageset)))); (typeof((typeof(*((zone->pageset))) *)((zone->pageset)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); });
 ...
 for (zone = (first_online_pgdat())->node_zones; zone; zone = next_zone(zone)) if (!populated_zone(zone)) ; else {
  ...
  printk("%s"
   ...
   "\n",
   zone->name,
   ((zone_nr_free_pages(zone)) << (12 -10)),
   (((zone->watermark[WMARK_MIN])) << (12 -10)),
   (((zone->watermark[WMARK_LOW])) << (12 -10)),
   (((zone->watermark[WMARK_HIGH])) << (12 -10)),
   ...
   ((zone_page_state(zone, NR_ISOLATED_ANON)) << (12 -10)),
   ((zone_page_state(zone, NR_ISOLATED_FILE)) << (12 -10)),
   ((zone->present_pages) << (12 -10)),
   ((zone_page_state(zone, NR_MLOCK)) << (12 -10)),
   ((zone_page_state(zone, NR_FILE_DIRTY)) << (12 -10)),
   ((zone_page_state(zone, NR_WRITEBACK)) << (12 -10)),
   ((zone_page_state(zone, NR_FILE_MAPPED)) << (12 -10)),
   ((zone_page_state(zone, NR_SHMEM)) << (12 -10)),
   ((zone_page_state(zone, NR_SLAB_RECLAIMABLE)) << (12 -10)),
   ((zone_page_state(zone, NR_SLAB_UNRECLAIMABLE)) << (12 -10)),
   zone_page_state(zone, NR_KERNEL_STACK) *
    8192 / 1024,
   ((zone_page_state(zone, NR_PAGETABLE)) << (12 -10)),
   ((zone_page_state(zone, NR_UNSTABLE_NFS)) << (12 -10)),
   ((zone_page_state(zone, NR_BOUNCE)) << (12 -10)),
   ((zone_page_state(zone, NR_WRITEBACK_TEMP)) << (12 -10)),
   zone->pages_scanned,
    ...
    int nr_zones, enum zone_type zone_type)
{
 ...
 do { if (__builtin_expect(!!(zone_type >= 3), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 zone_type++;
...
static void build_zonelists(pg_data_t *pgdat)
{
 ...
 j = build_zonelists_node(pgdat, zonelist, 0, 3 - 1);
# 2934 "../../mm/page_alloc.c"
...
struct mutex zonelists_mutex = { .count = { (1) } , .wait_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } } , .wait_list = { &(zonelists_mutex.wait_list), &(zonelists_mutex.wait_list) } };


static __attribute__ ((__section__(".ref.text"))) __attribute__((noinline)) int __build_all_zonelists(void *data)
{
 int nid;
 int cpu;
...
# 3021 "../../mm/page_alloc.c"
 for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_possible_mask)), ((cpu)) < nr_cpu_ids;) {
  setup_pageset(&(*({ do { const void *__vpp_verify = (typeof((&(boot_pageset))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(&(boot_pageset))) *)(&(boot_pageset)))); (typeof((typeof(*(&(boot_pageset))) *)(&(boot_pageset)))) (__ptr + (((__per_cpu_offset[cpu])))); }); })), 0);
# 3036 "../../mm/page_alloc.c"
 }
...
{
 unsigned long end_pfn = start_pfn + (1UL << (11 -1));
 unsigned long pfn;

 for (pfn = start_pfn; pfn < end_pfn; pfn++)
  if (PageReserved(({ unsigned long __pfn = (pfn); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; })))
   return 1;
 return 0;
}
# 3172 "../../mm/page_alloc.c"
static void setup_zone_migrate_reserve(struct zone *zone)
{
 ...
 reserve = (((((zone->watermark[WMARK_MIN])) + (((1UL << (11 -1))) - 1)) / ((1UL << (11 -1)))) * ((1UL << (11 -1)))) >>
       (11 -1);
# 3192 "../../mm/page_alloc.c"
 reserve = ({ typeof(2) _min1 = (2); typeof(reserve) _min2 = (reserve); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });

 for (pfn = start_pfn; pfn < end_pfn; pfn += (1UL << (11 -1))) {
  if (!pfn_valid(pfn))
   continue;
  page = ({ unsigned long __pfn = (pfn); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; });
...
void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) memmap_init_zone(unsigned long size, int nid, unsigned long zone,
...
static void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) zone_init_free_lists(struct zone *zone)
{
 int order, t;
...
static __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) void setup_zone_pageset(struct zone *zone)
{
 int cpu;

 zone->pageset = (typeof(struct per_cpu_pageset) *)__alloc_percpu(sizeof(struct per_cpu_pageset), __alignof__(struct per_cpu_pageset));

 for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_possible_mask)), ((cpu)) < nr_cpu_ids;) {
  struct per_cpu_pageset *pcp = ({ do { const void *__vpp_verify = (typeof(((zone->pageset))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((zone->pageset))) *)((zone->pageset)))); (typeof((typeof(*((zone->pageset))) *)((zone->pageset)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); });
...
void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) setup_per_cpu_pageset(void)
{
 struct zone *zone;
...
int zone_wait_table_init(struct zone *zone, unsigned long zone_size_pages)
{
 ...
 if (!slab_is_available()) {
  zone->wait_table = (wait_queue_head_t *)
   __alloc_bootmem_node(pgdat, alloc_size, (1 << 5), (((unsigned long)(0xffffffff)) - (0xC0000000UL) + (0x40000000UL)));
 } else {
# 3455 "../../mm/page_alloc.c"
 ...
 for(i = 0; i < zone->wait_table_hash_nr_entries; ++i)
  do { static struct lock_class_key __key; __init_waitqueue_head((zone->wait_table + i), &__key); } while (0);
...
{
 stop_machine(__zone_pcp_update, zone, ((void *)0));
}

static __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) void zone_pcp_init(struct zone *zone)
{
 ...
 if (zone->present_pages)
  printk("<7>" "  %s zone: %lu pages, LIFO batch:%u\n",
   zone->name, zone->present_pages,
      zone_batchsize(zone));
}

__attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) int init_currently_empty_zone(struct zone *zone,
     unsigned long zone_start_pfn,
     unsigned long size,
     enum memmap_context context)
{
...
# 3935 "../../mm/page_alloc.c"
static inline __attribute__((always_inline)) unsigned long __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) zone_spanned_pages_in_node(int nid,
...
static inline __attribute__((always_inline)) unsigned long __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) zone_absent_pages_in_node(int nid,
      unsigned long zone_type,
...
static void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) calculate_node_totalpages(struct pglist_data *pgdat,
...
# 4004 "../../mm/page_alloc.c"
static void inline __attribute__((always_inline)) setup_usemap(struct pglist_data *pgdat,
    struct zone *zone, unsigned long zonesize) {}
# 4040 "../../mm/page_alloc.c"
static inline __attribute__((always_inline)) int pageblock_default_order(unsigned int order)
{
 return 11 -1;
}
# 4054 "../../mm/page_alloc.c"
static void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) free_area_init_core(struct pglist_data *pgdat,
  unsigned long *zones_size, unsigned long *zholes_size)
{
 enum zone_type j;
 ...
 pgdat->nr_zones = 0;
 do { static struct lock_class_key __key; __init_waitqueue_head((&pgdat->kswapd_wait), &__key); } while (0);
 ...
 for (j = 0; j < 3; j++) {
  ...
  do { spinlock_check(&zone->lock); do { *(&(&zone->lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
  do { spinlock_check(&zone->lru_lock); do { *(&(&zone->lru_lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
  ...
  ret = init_currently_empty_zone(zone, zone_start_pfn,
      size, MEMMAP_EARLY);
  do { if (__builtin_expect(!!(ret), 0)) do { *(int *)0 = 0; } while (1); } while(0);
...
static void __attribute__ ((__section__(".ref.text"))) __attribute__((noinline)) alloc_node_mem_map(struct pglist_data *pgdat)
{
 ...
 if (!pgdat->node_spanned_pages)
  return;
# 4185 "../../mm/page_alloc.c"
}

void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) free_area_init_node(int nid, unsigned long *zones_size,
...
# 4736 "../../mm/page_alloc.c"
void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) set_dma_reserve(unsigned long new_dma_reserve)
{
 dma_reserve = new_dma_reserve;
}


struct pglist_data __attribute__ ((__section__(".ref.data"))) contig_page_data = {

 .bdata = &bootmem_node_data[0]

 };
extern typeof(contig_page_data) contig_page_data; static const char __kstrtab_contig_page_data[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "contig_page_data"; static const struct kernel_symbol __ksymtab_contig_page_data __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&contig_page_data, __kstrtab_contig_page_data };


void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) free_area_init(unsigned long *zones_size)
{
 free_area_init_node(0, zones_size,
   (((unsigned long)((0xC0000000UL))) - (0xC0000000UL) + (0x40000000UL)) >> 12, ((void *)0));
}
...
# 4779 "../../mm/page_alloc.c"
  refresh_cpu_vm_stats(cpu);
 }
 return 0x0001;
}

void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) page_alloc_init(void)
{
 { static struct notifier_block page_alloc_cpu_notify_nb __attribute__ ((__section__(".cpuinit.data"))) = { .notifier_call = page_alloc_cpu_notify, .priority = 0 }; register_cpu_notifier(&page_alloc_cpu_notify_nb); };
}
...
static void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) setup_per_zone_inactive_ratio(void)
{
 struct zone *zone;
...
# 4989 "../../mm/page_alloc.c"
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) init_per_zone_wmark_min(void)
{
...
static initcall_t __initcall_init_per_zone_wmark_min6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = init_per_zone_wmark_min;
 ...
 for (zone = (first_online_pgdat())->node_zones; zone; zone = next_zone(zone)) if (!populated_zone(zone)) ; else {
  for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_possible_mask)), ((cpu)) < nr_cpu_ids;) {
   ...
   setup_pagelist_highmark(
    ({ do { const void *__vpp_verify = (typeof(((zone->pageset))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((zone->pageset))) *)((zone->pageset)))); (typeof((typeof(*((zone->pageset))) *)((zone->pageset)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); }), high);
  }
 }
 return 0;
}
...
int hashdist = 0;
# 5118 "../../mm/page_alloc.c"
void *__attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) alloc_large_system_hash(const char *tablename,
         unsigned long bucketsize,
         unsigned long numentries,
         int scale,
         int flags,
         ...
         unsigned long limit)
{
 ...
 if (!numentries) {
  ...
  if (__builtin_expect(!!(flags & 0x00000002), 0)) {

   ({ int __ret_warn_on = !!(!(flags & 0x00000001)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../mm/page_alloc.c", 5148); __builtin_expect(!!(__ret_warn_on), 0); });
   if (!(numentries >> *_hash_shift)) {
    numentries = 1UL << *_hash_shift;
    do { if (__builtin_expect(!!(!numentries), 0)) do { *(int *)0 = 0; } while (1); } while(0);
   }
  } else if (__builtin_expect(!!((numentries * bucketsize) < ((1UL) << 12)), 0))
 ...
 do {
  ...
  if (flags & 0x00000001)
   table = __alloc_bootmem_nopanic(size, (1 << 5), (((unsigned long)(0xffffffff)) - (0xC0000000UL) + (0x40000000UL)));
  else if (hashdist)
   ...
   if (get_order(size) < 11) {
    table = alloc_pages_exact(size, ((( gfp_t)0x20u)));
     ...
     int start_bitidx, int end_bitidx)
{
 ...
 zone = page_zone(page);
 pfn = ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); });
...
int set_migratetype_isolate(struct page *page)
{
 ...
 pfn = ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); });
 arg.start_pfn = pfn;
 arg.nr_pages = (1UL << (11 -1));
 arg.pages_found = 0;
# 5327 "../../mm/page_alloc.c"
 notifier_ret = memory_isolate_notify((1<<0), &arg);
 notifier_ret = notifier_to_errno(notifier_ret);
 if (notifier_ret || !arg.pages_found)
  goto out;

 for (iter = pfn; iter < (pfn + (1UL << (11 -1))); iter++) {
  if (!(1))
   continue;

  curr_page = ({ unsigned long __pfn = (iter); struct mem_section *__sec = __pfn_to_section(__pfn); __section_mem_map_addr(__sec) + __pfn; });
