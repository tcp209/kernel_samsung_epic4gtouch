...
# 1 "../../arch/arm/include/asm/mman.h" 1
# 1 "../../include/asm-generic/mman.h" 1



# 1 "../../include/asm-generic/mman-common.h" 1
# 5 "../../include/asm-generic/mman.h" 2
# 2 "../../arch/arm/include/asm/mman.h" 2
# 5 "../../include/linux/mman.h" 2
...
static inline __attribute__((always_inline)) int arch_validate_prot(unsigned long prot)
{
...
# 28 "../../fs/aio.c" 2
# 1 "../../include/linux/mmu_context.h" 1
...
static inline __attribute__((always_inline)) u32 hash_32(u32 val, unsigned int bits)
{
...
static inline __attribute__((always_inline)) unsigned long hash_ptr(void *ptr, unsigned int bits)
{
...
# 40 "../../fs/aio.c" 2
# 51 "../../fs/aio.c"
static spinlock_t aio_nr_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } };
...
static struct work_struct fput_work = { .data = { (2) }, .entry = { &(fput_work).entry, &(fput_work).entry }, .func = (aio_fput_routine), };

static spinlock_t fput_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } };
static struct list_head fput_head = { &(fput_head), &(fput_head) };
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) aio_setup(void)
{
 kiocb_cachep = kmem_cache_create("kiocb", sizeof(struct kiocb), __alignof__(struct kiocb), (0x00002000UL|0x00040000UL), ((void *)0));
 kioctx_cachep = kmem_cache_create("kioctx", sizeof(struct kioctx), __alignof__(struct kioctx), (0x00002000UL|0x00040000UL), ((void *)0));

 aio_wq = __create_workqueue_key((("aio")), (0), (0), (0), ((void *)0), ((void *)0));
 abe_pool = mempool_create_kmalloc_pool(1, sizeof(struct aio_batch_entry));
 do { if (__builtin_expect(!!(!abe_pool), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 ({ if (0) printk("<7>" "aio_setup: sizeof(struct page) = %d\n", (int)sizeof(struct page)); 0; });

 return 0;
}
static initcall_t __initcall_aio_setup6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = aio_setup;
...
static int aio_setup_ring(struct kioctx *ctx)
{
 ...
 nr_pages = (size + ((1UL) << 12)-1) >> 12;

 if (nr_pages < 0)
  return -22;

 nr_events = (((1UL) << 12) * nr_pages - sizeof(struct aio_ring)) / sizeof(struct io_event);

 info->nr = 0;
 info->ring_pages = info->internal_pages;
 if (nr_pages > 8) {
  info->ring_pages = kcalloc(nr_pages, sizeof(struct page *), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
  if (!info->ring_pages)
   return -12;
 }

 info->mmap_size = nr_pages * ((1UL) << 12);
 do { ; } while (0);
 down_write(&ctx->mm->mmap_sem);
 ...
 do { ; } while (0);
 info->nr_pages = get_user_pages((get_current()), ctx->mm,
     info->mmap_base, nr_pages,
     1, 0, info->ring_pages, ((void *)0));
 up_write(&ctx->mm->mmap_sem);

 if (__builtin_expect(!!(info->nr_pages != nr_pages), 0)) {
  aio_free_ring(ctx);
  return -11;
 }
 ...
 ring->magic = 0xa10a10a1;
 ring->compat_features = 1;
 ring->incompat_features = 0;
...
{
 struct kioctx *ctx = ({ const typeof( ((struct kioctx *)0)->rcu_head ) *__mptr = (head); (struct kioctx *)( (char *)__mptr - __builtin_offsetof(struct kioctx,rcu_head) );});
 unsigned nr_events = ctx->max_reqs;
 ...
 if (nr_events) {
  ...
  do { if (__builtin_expect(!!(aio_nr - nr_events > aio_nr), 0)) do { *(int *)0 = 0; } while (1); } while(0);
  aio_nr -= nr_events;
...
{
 do { if (__builtin_expect(!!(ctx->reqs_active), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 mmdrop(ctx->mm);
 ctx->mm = ((void *)0);
 ({ if (0) printk("<7>" "__put_ioctx: freeing %p\n", ctx); 0; });
...
static struct kioctx *ioctx_alloc(unsigned nr_events)
{
 ...
 if ((nr_events > (0x10000000U / sizeof(struct io_event))) ||
  ...
  ({ if (0) printk("<7>" "ENOMEM: nr_events too high\n"); 0; });
  return ERR_PTR(-22);
 }
 ...
 ctx = kmem_cache_zalloc(kioctx_cachep, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!ctx)
  return ERR_PTR(-12);

 ctx->max_reqs = nr_events;
 mm = ctx->mm = (get_current())->mm;
 atomic_add(1, &mm->mm_count);

 (((&ctx->users)->counter) = (1));
 do { spinlock_check(&ctx->ctx_lock); do { *(&(&ctx->ctx_lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
 do { spinlock_check(&ctx->ring_info.ring_lock); do { *(&(&ctx->ring_info.ring_lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
 do { static struct lock_class_key __key; __init_waitqueue_head((&ctx->wait), &__key); } while (0);

 INIT_LIST_HEAD(&ctx->active_reqs);
 INIT_LIST_HEAD(&ctx->run_list);
 do { do { do { __init_work(((&(&ctx->wq)->work)), 0); ((&(&ctx->wq)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&ctx->wq)->work))->entry); do { (((&(&ctx->wq)->work)))->func = ((((aio_kick_handler)))); } while (0); } while (0); } while (0); init_timer_key((&(&ctx->wq)->timer), ((void *)0), ((void *)0)); } while (0);
...
static void wait_for_all_aios(struct kioctx *ctx)
{
 ...
 wait_queue_t wait = { .private = tsk, .func = default_wake_function, .task_list = { ((void *)0), ((void *)0) } };
 ...
 do { (tsk)->state = (2); __asm__ __volatile__ ("dmb" : : : "memory"); } while (0);
...
extern typeof(wait_on_sync_kiocb) wait_on_sync_kiocb; static const char __kstrtab_wait_on_sync_kiocb[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "wait_on_sync_kiocb"; static const struct kernel_symbol __ksymtab_wait_on_sync_kiocb __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&wait_on_sync_kiocb, __kstrtab_wait_on_sync_kiocb };
# 408 "../../fs/aio.c"
void exit_aio(struct mm_struct *mm)
{
 ...
 while (!hlist_empty(&mm->ioctx_list)) {
  ctx = ({ const typeof( ((struct kioctx *)0)->list ) *__mptr = (mm->ioctx_list.first); (struct kioctx *)( (char *)__mptr - __builtin_offsetof(struct kioctx,list) );});
  ...
  if (1 != (*(volatile int *)&(&ctx->users)->counter))
   printk("<7>"
    "exit_aio:ioctx still alive: %d %d %d\n",
    (*(volatile int *)&(&ctx->users)->counter), ctx->dead,
    ctx->reqs_active);
  do { do { if (__builtin_expect(!!((*(volatile int *)&(&(ctx)->users)->counter) <= 0), 0)) do { *(int *)0 = 0; } while (1); } while(0); if (__builtin_expect(!!((atomic_sub_return(1, &(ctx)->users) == 0)), 0)) __put_ioctx(ctx); } while (0);
 }
}
# 443 "../../fs/aio.c"
static struct kiocb *__aio_get_req(struct kioctx *ctx)
{
 ...
 req = kmem_cache_alloc(kiocb_cachep, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (__builtin_expect(!!(!req), 0))
  return ((void *)0);
 ...
 if (ctx->reqs_active < (((ring)->head + (&ctx->ring_info)->nr - 1 - (ring)->tail) % (&ctx->ring_info)->nr)) {
...
static inline __attribute__((always_inline)) struct kiocb *aio_get_req(struct kioctx *ctx)
{
 struct kiocb *req;
 ...
 if (__builtin_expect(!!(((void *)0) == req), 0)) {
...
static inline __attribute__((always_inline)) void really_put_req(struct kioctx *ctx, struct kiocb *req)
{
 ...
 if (__builtin_expect(!!(!ctx->reqs_active && ctx->dead), 0))
  __wake_up(&ctx->wait, (1 | 2), 1, ((void *)0));
}

static void aio_fput_routine(struct work_struct *data)
{
 spin_lock_irq(&fput_lock);
 while (__builtin_expect(!!(!list_empty(&fput_head)), 1)) {
...
static int __aio_put_req(struct kioctx *ctx, struct kiocb *req)
{
 ...
 req->ki_users--;
 do { if (__builtin_expect(!!(req->ki_users < 0), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 if (__builtin_expect(!!(req->ki_users), 1))
  return 0;
 list_del(&req->ki_list);
 req->ki_cancel = ((void *)0);
 req->ki_retry = ((void *)0);
 ...
 if (__builtin_expect(!!(!atomic_long_add_unless(&(req->ki_filp)->f_count, -1, 1)), 0)) {
  do { do { if (__builtin_expect(!!((*(volatile int *)&(&(ctx)->users)->counter) <= 0), 0)) do { *(int *)0 = 0; } while (1); } while(0); atomic_add(1, &(ctx)->users); } while (0);
  spin_lock(&fput_lock);
...
extern typeof(aio_put_req) aio_put_req; static const char __kstrtab_aio_put_req[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "aio_put_req"; static const struct kernel_symbol __ksymtab_aio_put_req __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&aio_put_req, __kstrtab_aio_put_req };

static struct kioctx *lookup_ioctx(unsigned long ctx_id)
{
 struct mm_struct *mm = (get_current())->mm;
 struct kioctx *ctx, *ret = ((void *)0);
 struct hlist_node *n;

 rcu_read_lock();

 for (n = ({ typeof((&mm->ioctx_list)->first) _________p1 = (*(volatile typeof((&mm->ioctx_list)->first) *)&((&mm->ioctx_list)->first)); do { } while(0); (_________p1); }); n && ({ prefetch(n->next); 1; }) && ({ ctx = ({ const typeof( ((typeof(*ctx) *)0)->list ) *__mptr = (n); (typeof(*ctx) *)( (char *)__mptr - __builtin_offsetof(typeof(*ctx),list) );}); 1; }); n = ({ typeof(n->next) _________p1 = (*(volatile typeof(n->next) *)&(n->next)); do { } while(0); (_________p1); })) {
...
# 624 "../../fs/aio.c"
static inline __attribute__((always_inline)) int __queue_kicked_iocb(struct kiocb *iocb)
{
...
# 690 "../../fs/aio.c"
 ...
 if (ret != -530 && ret != -529) {
  ...
  if (__builtin_expect(!!(ret == -512 || ret == -513 || ret == -514 || ret == -516), 0))
...
static int __aio_run_iocbs(struct kioctx *ctx)
{
 ...
 while (!list_empty(&run_list)) {
  iocb = ({ const typeof( ((struct kiocb *)0)->ki_run_list ) *__mptr = (run_list.next); (struct kiocb *)( (char *)__mptr - __builtin_offsetof(struct kiocb,ki_run_list) );});
...
# 809 "../../fs/aio.c"
static inline __attribute__((always_inline)) void aio_run_iocbs(struct kioctx *ctx)
{
 int requeue;
...
static inline __attribute__((always_inline)) void aio_run_all_iocbs(struct kioctx *ctx)
{
 spin_lock_irq(&ctx->ctx_lock);
...
{
 struct kioctx *ctx = ({ const typeof( ((struct kioctx *)0)->wq.work ) *__mptr = (work); (struct kioctx *)( (char *)__mptr - __builtin_offsetof(struct kioctx,wq.work) );});
...
static void try_queue_kicked_iocb(struct kiocb *iocb)
{
 ...
 do { do { ({ unsigned long __dummy; typeof(flags) __dummy2; (void)(&__dummy == &__dummy2); 1; }); flags = _raw_spin_lock_irqsave(spinlock_check(&ctx->ctx_lock)); } while (0); } while (0);


 if (!_test_and_set_bit_le(1,&(iocb)->ki_flags))
...
extern typeof(kick_iocb) kick_iocb; static const char __kstrtab_kick_iocb[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "kick_iocb"; static const struct kernel_symbol __ksymtab_kick_iocb __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&kick_iocb, __kstrtab_kick_iocb };
...
# 929 "../../fs/aio.c"
 if (((iocb)->ki_key == (~0U))) {
  do { if (__builtin_expect(!!(iocb->ki_users != 1), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 tail = info->tail;
 event = ({ unsigned pos = (tail) + ((((1UL) << 12) / sizeof(struct io_event)) - ((((1UL) << 12) - sizeof(struct aio_ring)) / sizeof(struct io_event))); struct io_event *__event; __event = kmap_atomic( (info)->ring_pages[pos / (((1UL) << 12) / sizeof(struct io_event))], KM_IRQ0); __event += pos % (((1UL) << 12) / sizeof(struct io_event)); __event; });
 if (++tail >= info->nr)
  tail = 0;
 ...
 do { struct io_event *__event = (event); (void)__event; kunmap_atomic((void *)((unsigned long)__event & (~(((1UL) << 12)-1))), KM_IRQ0); } while(0);
 kunmap_atomic(ring, KM_IRQ1);

 ({ if (0) printk("<7>" "added to ring %p at [%lu]\n", iocb, tail); 0; });
...
extern typeof(aio_complete) aio_complete; static const char __kstrtab_aio_complete[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "aio_complete"; static const struct kernel_symbol __ksymtab_aio_complete __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&aio_complete, __kstrtab_aio_complete };
...
static int aio_read_evt(struct kioctx *ioctx, struct io_event *ent)
{
 ...
 if (head != ring->tail) {
  struct io_event *evp = ({ unsigned pos = (head) + ((((1UL) << 12) / sizeof(struct io_event)) - ((((1UL) << 12) - sizeof(struct aio_ring)) / sizeof(struct io_event))); struct io_event *__event; __event = kmap_atomic( (info)->ring_pages[pos / (((1UL) << 12) / sizeof(struct io_event))], KM_USER1); __event += pos % (((1UL) << 12) / sizeof(struct io_event)); __event; });
  *ent = *evp;
  head = (head + 1) % info->nr;
  __asm__ __volatile__ ("dmb" : : : "memory");
  ring->head = head;
  ret = 1;
  do { struct io_event *__event = (evp); (void)__event; kunmap_atomic((void *)((unsigned long)__event & (~(((1UL) << 12)-1))), KM_USER1); } while(0);
 }
...
static inline __attribute__((always_inline)) void init_timeout(struct aio_timeout *to)
{
 setup_timer_on_stack_key((&to->timer), ((void *)0), ((void *)0), (timeout_func), ((unsigned long) to));
 to->timed_out = 0;
 to->p = (get_current());
}

static inline __attribute__((always_inline)) void set_timeout(long start_jiffies, struct aio_timeout *to,
          const struct timespec *ts)
{
 to->timer.expires = start_jiffies + timespec_to_jiffies(ts);
 if ((({ unsigned long __dummy; typeof(to->timer.expires) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(jiffies) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(jiffies) - (long)(to->timer.expires) < 0)))
  add_timer(&to->timer);
 else
  to->timed_out = 1;
}

static inline __attribute__((always_inline)) void clear_timeout(struct aio_timeout *to)
{
   ...
   struct timespec *timeout)
{
 ...
 ({ void *__p = (&ent); size_t __n = sizeof(ent); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
retry:
 ret = 0;
 while (__builtin_expect(!!(i < nr), 1)) {
  ...
  ret = -14;
  if (__builtin_expect(!!(copy_to_user(event, &ent, sizeof(ent))), 0)) {
   do { ; } while (0);
   break;
  }
  ret = 0;
 ...
 if (!retry && __builtin_expect(!!(!list_empty(&ctx->run_list)), 0)) {
  retry = 1;
 ...
 if (timeout) {
  struct timespec ts;
  ret = -14;
  if (__builtin_expect(!!(copy_from_user(&ts, timeout, sizeof(ts))), 0))
   goto out;
 ...
 while (__builtin_expect(!!(i < nr), 1)) {
  ...
  do {
   do { (tsk)->state = (1); __asm__ __volatile__ ("dmb" : : : "memory"); } while (0);
   ret = aio_read_evt(ctx, &ent);
   if (ret)
    break;
   if (min_nr <= i)
    break;
   if (__builtin_expect(!!(ctx->dead), 0)) {
    ret = -22;
    break;
   }
   if (to.timed_out)
  ...
  do { (tsk)->state = (0); __asm__ __volatile__ ("dmb" : : : "memory"); } while (0);
  remove_wait_queue(&ctx->wait, &wait);
...
static void io_destroy(struct kioctx *ioctx)
{
 ...
 do { ; } while (0);
 if (__builtin_expect(!!(!was_dead), 1))
  do { do { if (__builtin_expect(!!((*(volatile int *)&(&(ioctx)->users)->counter) <= 0), 0)) do { *(int *)0 = 0; } while (1); } while(0); if (__builtin_expect(!!((atomic_sub_return(1, &(ioctx)->users) == 0)), 0)) __put_ioctx(ioctx); } while (0);
 ...
 __wake_up(&ioctx->wait, (1 | 2), 1, ((void *)0));
 do { do { if (__builtin_expect(!!((*(volatile int *)&(&(ioctx)->users)->counter) <= 0), 0)) do { *(int *)0 = 0; } while (1); } while(0); if (__builtin_expect(!!((atomic_sub_return(1, &(ioctx)->users) == 0)), 0)) __put_ioctx(ioctx); } while (0);
}
# 1253 "../../fs/aio.c"
 long sys_io_setup(unsigned nr_events, aio_context_t * ctxp)
{
 struct kioctx *ioctx = ((void *)0);
 unsigned long ctx;
 long ret;

 ret = ({ register const typeof(*(ctxp)) *__p asm("r0") = (ctxp); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } ctx = (typeof(*(ctxp))) __r2; __e; });
 if (__builtin_expect(!!(ret), 0))
  goto out;

 ret = -22;
 if (__builtin_expect(!!(ctx || nr_events == 0), 0)) {
  ({ if (0) printk("<7>" "EINVAL: io_setup: ctx %lu nr_events %u\n", ctx, nr_events); 0; });
 ...
 long sys_io_destroy(aio_context_t ctx)
{
 ...
 if (__builtin_expect(!!(((void *)0) != ioctx), 1)) {
  io_destroy(ioctx);
  return 0;
 }
 ({ if (0) printk("<7>" "EINVAL: io_destroy: invalid context id\n"); 0; });
 return -22;
}
...
static void aio_advance_iovec(struct kiocb *iocb, ssize_t ret)
{
 ...
 do { if (__builtin_expect(!!(ret <= 0), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 while (iocb->ki_cur_seg < iocb->ki_nr_segs && ret > 0) {
  ssize_t this = ({ typeof((ssize_t)iov->iov_len) _min1 = ((ssize_t)iov->iov_len); typeof(ret) _min2 = (ret); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
  iov->iov_base += this;
  iov->iov_len -= this;
  iocb->ki_left -= this;
  ret -= this;
 ...
 do { if (__builtin_expect(!!(ret > 0 && iocb->ki_left == 0), 0)) do { *(int *)0 = 0; } while (1); } while(0);
}
...
static ssize_t aio_rw_vect_retry(struct kiocb *iocb)
{
 ...
 do {
  ...
  if (ret > 0)
   ...
   (opcode == IOCB_CMD_PWRITEV ||
    (!(((inode->i_mode) & 00170000) == 0010000) && !(((inode->i_mode) & 00170000) == 0140000))));
...
static ssize_t aio_setup_iocb(struct kiocb *kiocb, bool compat)
{
 ...
 case IOCB_CMD_PREAD:
  ret = -9;
  if (__builtin_expect(!!(!(file->f_mode & (( fmode_t)0x1))), 0))
   break;
  ret = -14;
  if (__builtin_expect(!!(!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (kiocb->ki_buf), "Ir" (kiocb->ki_left), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0)), 0))

   break;
  ret = security_file_permission(file, 4);
  if (__builtin_expect(!!(ret), 0))
   break;
 ...
 case IOCB_CMD_PWRITE:
  ret = -9;
  if (__builtin_expect(!!(!(file->f_mode & (( fmode_t)0x2))), 0))
   break;
  ret = -14;
  if (__builtin_expect(!!(!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (kiocb->ki_buf), "Ir" (kiocb->ki_left), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0)), 0))

   break;
  ret = security_file_permission(file, 2);
  if (__builtin_expect(!!(ret), 0))
   break;
  ret = aio_setup_single_vector(kiocb);
  if (ret)
 ...
 case IOCB_CMD_PREADV:
  ...
  if (__builtin_expect(!!(ret), 0))
   break;
  ret = aio_setup_vectored_rw(0, kiocb, compat);
  if (ret)
   break;
  ret = -22;
  if (file->f_op->aio_read)
 ...
 case IOCB_CMD_PWRITEV:
  ...
  if (__builtin_expect(!!(ret), 0))
   break;
  ret = aio_setup_vectored_rw(1, kiocb, compat);
  if (ret)
   break;
  ret = -22;
  if (file->f_op->aio_write)
     ...
     struct hlist_head *batch_hash)
{
 ...
 bucket = hash_ptr(mapping, 3);
 for (pos = (&batch_hash[bucket])->first; pos && ({ prefetch(pos->next); 1;}) && ({ abe = ({ const typeof( ((typeof(*abe) *)0)->list ) *__mptr = (pos); (typeof(*abe) *)( (char *)__mptr - __builtin_offsetof(typeof(*abe),list) );}); 1;}); pos = pos->next) {
  if (abe->mapping == mapping)
   return;
 }

 abe = mempool_alloc(abe_pool, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 do { if (__builtin_expect(!!(!igrab(mapping->host)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 abe->mapping = mapping;
...
static void aio_batch_free(struct hlist_head *batch_hash)
{
 ...
 for (i = 0; i < (1 << 3); i++) {
  for (pos = (&batch_hash[i])->first; pos && ({ n = pos->next; 1; }) && ({ abe = ({ const typeof( ((typeof(*abe) *)0)->list ) *__mptr = (pos); (typeof(*abe) *)( (char *)__mptr - __builtin_offsetof(typeof(*abe),list) );}); 1;}); pos = n) {
    ...
    bool compat)
{
 ...
 if (__builtin_expect(!!(iocb->aio_reserved1 || iocb->aio_reserved2), 0)) {
  ({ if (0) printk("<7>" "EINVAL: io_submit: reserve field set\n"); 0; });
  return -22;
 }


 if (__builtin_expect(!!((iocb->aio_buf != (unsigned long)iocb->aio_buf) || (iocb->aio_nbytes != (size_t)iocb->aio_nbytes) || ((ssize_t)iocb->aio_nbytes < 0)), 0)) {




  ({ if (0) printk("<7>" "EINVAL: io_submit: overflow check\n"); 0; });
  return -22;
 }
 ...
 ret = ({ register const typeof(*(&user_iocb->aio_key)) __r2 asm("r2") = (req->ki_key); register const typeof(*(&user_iocb->aio_key)) *__p asm("r0") = (&user_iocb->aio_key); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; });
 if (__builtin_expect(!!(ret), 0)) {
  do { ; } while (0);
  goto out_put_req;
 }
    ...
    struct iocb * *iocbpp, bool compat)
{
 ...
 long ret = 0;
 int i;
 struct hlist_head batch_hash[(1 << 3)] = { { 0, }, };

 if (__builtin_expect(!!(nr < 0), 0))
  return -22;

 if (__builtin_expect(!!(nr > ((long)(~0UL>>1))/sizeof(*iocbpp)), 0))
  nr = ((long)(~0UL>>1))/sizeof(*iocbpp);

 if (__builtin_expect(!!(!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (iocbpp), "Ir" ((nr*sizeof(*iocbpp))), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0)), 0))
  return -14;

 ctx = lookup_ioctx(ctx_id);
 if (__builtin_expect(!!(!ctx), 0)) {
  ({ if (0) printk("<7>" "EINVAL: io_submit: invalid context id\n"); 0; });
  return -22;
 }
 ...
 for (i=0; i<nr; i++) {
  struct iocb *user_iocb;
  struct iocb tmp;

  if (__builtin_expect(!!(({ long __gu_err = 0; do { unsigned long __gu_addr = (unsigned long)((iocbpp + i)); unsigned long __gu_val; (void)0; switch (sizeof(*((iocbpp + i)))) { case 1: __asm__ __volatile__( "1:	ldrbt	%1,[%2]\n" "2:\n" "	.pushsection .fixup,\"ax\"\n" "	.align	2\n" "3:	mov	%0, %3\n" "	mov	%1, #0\n" "	b	2b\n" "	.popsection\n" "	.pushsection __ex_table,\"a\"\n" "	.align	3\n" "	.long	1b, 3b\n" "	.popsection" : "+r" (__gu_err), "=&r" (__gu_val) : "r" (__gu_addr), "i" (-14) : "cc"); break; case 2: ({ unsigned long __b1, __b2; __asm__ __volatile__( "1:	ldrbt	%1,[%2]\n" "2:\n" "	.pushsection .fixup,\"ax\"\n" "	.align	2\n" "3:	mov	%0, %3\n" "	mov	%1, #0\n" "	b	2b\n" "	.popsection\n" "	.pushsection __ex_table,\"a\"\n" "	.align	3\n" "	.long	1b, 3b\n" "	.popsection" : "+r" (__gu_err), "=&r" (__b1) : "r" (__gu_addr), "i" (-14) : "cc"); __asm__ __volatile__( "1:	ldrbt	%1,[%2]\n" "2:\n" "	.pushsection .fixup,\"ax\"\n" "	.align	2\n" "3:	mov	%0, %3\n" "	mov	%1, #0\n" "	b	2b\n" "	.popsection\n" "	.pushsection __ex_table,\"a\"\n" "	.align	3\n" "	.long	1b, 3b\n" "	.popsection" : "+r" (__gu_err), "=&r" (__b2) : "r" (__gu_addr + 1), "i" (-14) : "cc"); (__gu_val) = __b1 | (__b2 << 8); }); break; case 4: __asm__ __volatile__( "1:	ldrt	%1,[%2]\n" "2:\n" "	.pushsection .fixup,\"ax\"\n" "	.align	2\n" "3:	mov	%0, %3\n" "	mov	%1, #0\n" "	b	2b\n" "	.popsection\n" "	.pushsection __ex_table,\"a\"\n" "	.align	3\n" "	.long	1b, 3b\n" "	.popsection" : "+r" (__gu_err), "=&r" (__gu_val) : "r" (__gu_addr), "i" (-14) : "cc"); break; default: (__gu_val) = __get_user_bad(); } ((user_iocb)) = (__typeof__(*((iocbpp + i))))__gu_val; } while (0); __gu_err; })), 0)) {
   ret = -14;
   break;
  }

  if (__builtin_expect(!!(copy_from_user(&tmp, user_iocb, sizeof(tmp))), 0)) {
   ret = -14;
   break;
  }
...
static struct kiocb *lookup_kiocb(struct kioctx *ctx, struct iocb *iocb,
      u32 key)
{
 ...
 for (pos = (&ctx->active_reqs)->next; prefetch(pos->next), pos != (&ctx->active_reqs); pos = pos->next) {
...

{
 ...
 ret = ({ register const typeof(*(&iocb->aio_key)) *__p asm("r0") = (&iocb->aio_key); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } key = (typeof(*(&iocb->aio_key))) __r2; __e; });
 if (__builtin_expect(!!(ret), 0))
  return -14;
 ...
 if (((void *)0) != cancel) {
  struct io_event tmp;
  ({ if (0) printk("<7>" "calling cancel\n"); 0; });
  ({ void *__p = (&tmp); size_t __n = sizeof(tmp); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
...

{
 ...
 if (__builtin_expect(!!(ioctx), 1)) {
  if (__builtin_expect(!!(min_nr <= nr && min_nr >= 0 && nr >= 0), 1))
