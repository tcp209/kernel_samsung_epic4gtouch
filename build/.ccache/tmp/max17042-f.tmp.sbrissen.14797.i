# 1 "../../drivers/power/max17042-fuelgauge.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../drivers/power/max17042-fuelgauge.c"
# 16 "../../drivers/power/max17042-fuelgauge.c"
...
# 17 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 19 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 21 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 22 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 23 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 24 "../../drivers/power/max17042-fuelgauge.c" 2
# 1 "../../include/linux/max17042-fuelgauge.h" 1
# 48 "../../include/linux/max17042-fuelgauge.h"
...
# 25 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 26 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 27 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 28 "../../drivers/power/max17042-fuelgauge.c" 2
...
# 29 "../../drivers/power/max17042-fuelgauge.c" 2
...
static int max17042_get_property(struct power_supply *psy,
       ...
       union power_supply_propval *val)
{
 struct max17042_chip *chip = ({ const typeof( ((struct max17042_chip *)0)->battery ) *__mptr = (psy); (struct max17042_chip *)( (char *)__mptr - __builtin_offsetof(struct max17042_chip,battery) );});
...
static int max17042_write_reg(struct i2c_client *client, int reg, u8 * buf)
{
 ...
 if (ret < 0)
  printk("<3>" "%s %s: " "%s: err %d\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, ret);
...
static void max17042_init_regs(struct i2c_client *client)
{
 ...
 printk("<6>" "%s %s: " "%s\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__);





 if (max17042_read_reg(client, 0x29, data) < 0)
  return;


 data[0] &= 0x8f;

 max17042_write_reg(client, 0x29, data);
...
static int max17042_read_vfocv(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0xFB, data) < 0)
  return -1;
...
# 217 "../../drivers/power/max17042-fuelgauge.c"
...
static void max17042_reset_soc(struct i2c_client *client)
{
 ...
 printk("<6>" "%s %s: " "%s : Before quick-start - " "VfOCV(%d), VfSOC(%d)\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, max17042_read_vfocv(client), max17042_read_vfsoc(client));




 if (max17042_read_reg(client, 0x2B, data) < 0)
  return;


 data[1] |= (0x1 << 2);
 max17042_write_reg(client, 0x2B, data);

 msleep(500);

 printk("<6>" "%s %s: " "%s : After quick-start - " "VfOCV(%d), VfSOC(%d)\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, max17042_read_vfocv(client), max17042_read_vfsoc(client));
...
static void max17042_get_vcell(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0x09, data) < 0)
  return;

 chip->vcell = ((data[0] >> 3) + (data[1] << 5)) * 625;

 if (max17042_read_reg(client, 0x19, data) < 0)
  return;
...
static int max17042_recalc_soc(struct i2c_client *client, int boot_cnt)
{
 ...
 if (boot_cnt < 4)
  max17042_read_reg(client, 0x09, data);
 else
  max17042_read_reg(client, 0x19, data);
 temp_vfocv = (data[1] << 8);
 temp_vfocv |= data[0];
 ...
 if (psy != ((void *)0)) {
  ...
  if (value.intval == 0)
   temp_vfocv = temp_vfocv + 0x0380;
  else
   temp_vfocv = temp_vfocv - 0x0380;

  printk("<6>" "%s %s: " "cable = %d, " , dev_driver_string(&client->dev) , dev_name(&client->dev) , value.intval);
 } else
  temp_vfocv = temp_vfocv + 0x0380;

 data[1] = temp_vfocv >> 8;
 data[0] = 0x00FF & temp_vfocv;
 printk("<6>" "%s %s: " "forced write to vfocv %d mV\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , (temp_vfocv >> 4) * 125 / 100);

 max17042_write_reg(client, 0xFB, data);

 msleep(200);

 max17042_read_reg(client, 0xFF, data);
 soc = ({ typeof((int)data[1]) _min1 = ((int)data[1]); typeof(100) _min2 = (100); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });

 max17042_read_reg(client, 0x09, data);
 printk("<6>" "%s %s: " "new vcell = %d, vfocv = %d, soc = %d\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , ((data[0] >> 3) + (data[1] << 5)) * 625 / 1000, max17042_read_vfocv(client), soc);
...
static void max17042_get_soc(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0xFF, data) < 0)
  return;
 printk("<6>" "%s %s: " "%s : soc(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 soc = (data[1] * 100) + (data[0] * 100 / 256);

 chip->raw_soc = ({ typeof(soc / 100) _min1 = (soc / 100); typeof(100) _min2 = (100); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });


 if (chip->pdata->need_soc_recal()) {
  printk("<6>" "%s %s: " "%s : recalculate soc\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__);
  ...
  printk("<6>" "%s %s: " "vcell = %d, vfocv = %d, soc = %d\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , chip->vcell, max17042_read_vfocv(client), soc);


  if (soc < 5) {
   printk("<6>" "%s %s: " "recalc force soc = %d, diff = %d!\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , soc, diff);
  ...
  if (diff > 10) {
   printk("<6>" "%s %s: " "recalc 180s soc = %d, diff = %d!\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , soc, diff);
  ...
  printk("<6>" "%s %s: " "%s : use raw (%d), soc (%d)\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, chip->raw_soc, soc);
 ...
 soc = ({ typeof(soc) _min1 = (soc); typeof(100) _min2 = (100); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
...
static void max17042_get_temperature(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0x08, data) < 0)
  return;
 ...
 ({ if (0) printk("<7>" "%s %s: " "%s: MAX17042 Temperature = %d\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, chip->temperature); 0; });
...
static void max17042_get_version(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0x21, data) < 0)
  return;

 printk("<6>" "%s %s: " "MAX17042 Fuel-Gauge Ver %d%d\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , data[0], data[1]);
...
static void max17042_work(struct work_struct *work)
{
 ...
 chip = ({ const typeof( ((struct max17042_chip *)0)->work.work ) *__mptr = (work); (struct max17042_chip *)( (char *)__mptr - __builtin_offsetof(struct max17042_chip,work.work) );});
 ...
 if (chip->raw_soc < chip->fuel_alert_soc) {
  if(!(chip->is_fuel_alerted) &&
   ...
   printk("<6>" "%s %s: " "fuel alert activated by polling check (raw:%d)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , chip->raw_soc);


  }
  else
   printk("<6>" "%s %s: " "fuel alert already activated (raw:%d)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , chip->raw_soc);
  ...
  if(chip->is_fuel_alerted) {
   ...
   printk("<6>" "%s %s: " "fuel alert deactivated by polling check (raw:%d)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , chip->raw_soc);


  }
 }
# 542 "../../drivers/power/max17042-fuelgauge.c"
 schedule_delayed_work(&chip->work, 2000);
}
...
# 563 "../../drivers/power/max17042-fuelgauge.c"
static struct device_attribute sec_fg_attrs[] = {
 { .attr = { .name = "fg_reset_soc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_fg_show_property, .store = sec_fg_store, },
 { .attr = { .name = "fg_read_soc", .mode = 0664, .owner = ((struct module *)0) }, .show = sec_fg_show_property, .store = sec_fg_store, },
};
...
static int fuelgauge_create_attrs(struct device *dev)
{
 ...
 for (i = 0; i < (sizeof(sec_fg_attrs) / sizeof((sec_fg_attrs)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sec_fg_attrs), typeof(&sec_fg_attrs[0]))); }))); i++) {
...
static bool max17042_check_status(struct i2c_client *client)
{
 ...
 if (max17042_read_reg(client, 0x00, data) < 0)
  return ret;

 printk("<6>" "%s %s: " "%s : status_reg(%02x%02x)\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , __func__, data[1], data[0]);
 ...
 if (!ret) {
  data[1] = 0;
  max17042_write_reg(client, 0x00, data);
  msleep(200);
 }
...
static irqreturn_t max17042_irq_thread(int irq, void *irq_data)
{
 ...
 if (max17042_alert_status ) {
  if (max17042_read_reg(chip->client, 0x1D, data)
   < 0)
   return IRQ_HANDLED;
  ...
  if (chip->pdata->low_batt_cb && !(chip->is_fuel_alerted)) {
   ...
   printk("<3>" "%s %s: " "failed to call low_batt_cb()\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev));


  max17042_write_reg(chip->client, 0x1D, data);

  printk("<6>" "%s %s: " "%s : low batt alerted!! config_reg(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 } else if (!max17042_alert_status ) {
  if (chip->is_fuel_alerted)
   wake_unlock(&chip->fuel_alert_wake_lock);
  chip->is_fuel_alerted = false;
  ...
  printk("<6>" "%s %s: " "%s : low batt released!! config_reg(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 }

 max17042_read_reg(chip->client, 0x09, data);
 printk("<6>" "%s %s: " "%s : MAX17042_REG_VCELL(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 max17042_read_reg(chip->client, 0x08, data);
 printk("<6>" "%s %s: " "%s : MAX17042_REG_TEMPERATURE(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 max17042_read_reg(chip->client, 0x1D, data);
 printk("<6>" "%s %s: " "%s : MAX17042_REG_CONFIG(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 max17042_read_reg(chip->client, 0xFB, data);
 printk("<6>" "%s %s: " "%s : MAX17042_REG_VFOCV(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 max17042_read_reg(chip->client, 0xFF, data);
 printk("<6>" "%s %s: " "%s : MAX17042_REG_SOC_VF(%02x%02x)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0]);


 printk("<3>" "%s %s: " "%s : PMIC IRQ (%d), FUEL GAUGE IRQ (%d)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, __gpio_get_value((S5PV310_GPIO_X0_START + (7))), __gpio_get_value(chip->pdata->alert_gpio));
# 788 "../../drivers/power/max17042-fuelgauge.c"
 return IRQ_HANDLED;
}
...
static int max17042_irq_init(struct max17042_chip *chip)
{
 ...
 if (chip->pdata->alert_irq) {
  ret = request_threaded_irq(chip->pdata->alert_irq, ((void *)0),
    max17042_irq_thread, 0x00000008 | 0x00002000,
       "max17042 fuel alert", chip);
  if (ret) {
   printk("<3>" "%s %s: " "failed to reqeust IRQ\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev));
   return ret;
  }

  ret = enable_irq_wake(chip->pdata->alert_irq);
  if (ret < 0)
   printk("<3>" "%s %s: " "failed to enable wakeup src %d\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , ret);
 ...
 printk("<6>" "%s %s: " "%s : config_reg(%02x%02x) irq(%d)\n" , dev_driver_string(&chip->client->dev) , dev_name(&chip->client->dev) , __func__, data[1], data[0], chip->pdata->alert_irq);


 return 0;
}

static int __attribute__ ((__section__(".devinit.text"))) __attribute__((__cold__)) max17042_probe(struct i2c_client *client,
   const struct i2c_device_id *id)
{
 struct i2c_adapter *adapter = ({ const typeof( ((struct i2c_adapter *)0)->dev ) *__mptr = (client->dev.parent); (struct i2c_adapter *)( (char *)__mptr - __builtin_offsetof(struct i2c_adapter,dev) );});
 struct max17042_chip *chip;
 int i;
 struct max17042_reg_data *data;
 int ret;

 if (!i2c_check_functionality(adapter, (0x00020000 | 0x00040000)))
  return -5;

 chip = kzalloc(sizeof(*chip), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!chip)
  return -12;
 ...
 chip->battery.num_properties = (sizeof(max17042_battery_props) / sizeof((max17042_battery_props)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(max17042_battery_props), typeof(&max17042_battery_props[0]))); })));
 ...
 if (ret) {
  printk("<3>" "%s %s: " "failed: power supply register\n" , dev_driver_string(&client->dev) , dev_name(&client->dev));
  kfree(chip);
  return ret;
 }
 ...
 chip->pdata->alert_irq = __gpio_to_irq(chip->pdata->alert_gpio);
 ...
 printk("<6>" "%s %s: " "fuel alert soc (%d)\n" , dev_driver_string(&client->dev) , dev_name(&client->dev) , chip->fuel_alert_soc);
 ...
 do { do { do { __init_work(((&(&chip->work)->work)), 0); ((&(&chip->work)->work))->data = (atomic_long_t) { (0) }; INIT_LIST_HEAD(&((&(&chip->work)->work))->entry); do { (((&(&chip->work)->work)))->func = ((((max17042_work)))); } while (0); } while (0); } while (0); init_timer_deferrable_key((&(&chip->work)->timer), ((void *)0), ((void *)0)); } while (0);
...
static int __attribute__ ((__section__(".devexit.text"))) __attribute__((__used__)) __attribute__((__cold__)) max17042_remove(struct i2c_client *client)
{
 struct max17042_chip *chip = i2c_get_clientdata(client);
...
# 950 "../../drivers/power/max17042-fuelgauge.c"
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) max17042_init(void)
{
 return i2c_add_driver(&max17042_i2c_driver);
}

static initcall_t __initcall_max17042_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = max17042_init;;

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) max17042_exit(void)
{
 i2c_del_driver(&max17042_i2c_driver);
}

static exitcall_t __exitcall_max17042_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = max17042_exit;;
