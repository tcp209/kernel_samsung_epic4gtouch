...
# 1 "<command-line>" 2
# 1 "../../drivers/usb/storage/scsiglue.c"
# 46 "../../drivers/usb/storage/scsiglue.c"
...
# 47 "../../drivers/usb/storage/scsiglue.c" 2
...
# 50 "../../drivers/usb/storage/scsiglue.c" 2
...
# 51 "../../drivers/usb/storage/scsiglue.c" 2
# 1 "../../include/scsi/scsi_devinfo.h" 1
# 52 "../../drivers/usb/storage/scsiglue.c" 2
...
# 53 "../../drivers/usb/storage/scsiglue.c" 2
...
# 54 "../../drivers/usb/storage/scsiglue.c" 2
...
# 56 "../../drivers/usb/storage/scsiglue.c" 2
...
# 57 "../../drivers/usb/storage/scsiglue.c" 2
# 1 "../../drivers/usb/storage/debug.h" 1
# 58 "../../drivers/usb/storage/scsiglue.c" 2
...
# 59 "../../drivers/usb/storage/scsiglue.c" 2
...
# 60 "../../drivers/usb/storage/scsiglue.c" 2
# 73 "../../drivers/usb/storage/scsiglue.c"
...
static int slave_alloc (struct scsi_device *sdev)
{
 ...
 sdev->inquiry_len = 36;
# 105 "../../drivers/usb/storage/scsiglue.c"
 blk_queue_update_dma_alignment(sdev->request_queue, (512 - 1));
# 116 "../../drivers/usb/storage/scsiglue.c"
 if (us->subclass == 0x04)
...
static int slave_configure(struct scsi_device *sdev)
{
 ...
 if (us->fflags & (US_FL_MAX_SECTORS_64 | US_FL_MAX_SECTORS_MIN)) {
  ...
  if (us->fflags & US_FL_MAX_SECTORS_MIN)
   max_sectors = ((1UL) << 12) >> 9;
  if (queue_max_hw_sectors(sdev->request_queue) > max_sectors)
   blk_queue_max_hw_sectors(sdev->request_queue,
           max_sectors);
 } else if (sdev->type == 0x01) {
 ...
 if (!us->pusb_dev->bus->controller->dma_mask)
  blk_queue_bounce_limit(sdev->request_queue, ((u64)blk_max_low_pfn << 12));
 ...
 if (sdev->type == 0x00) {
  ...
  switch ((( __u16)(__le16)(us->pusb_dev->descriptor.idVendor))) {
  case 0x0421:
  case 0x04b0:
  case 0x0a17:
  case 0x22b8:
  ...
  if (us->subclass != 0x06 && us->subclass != 0xf1)
   sdev->use_10_for_ms = 1;



  sdev->use_192_bytes_for_3f = 1;
# 193 "../../drivers/usb/storage/scsiglue.c"
 ...
 if ((us->protocol == 0x01 || us->protocol == 0x00) &&
   sdev->scsi_level == 0)
  us->max_lun = 0;
...
static int command_abort(struct scsi_cmnd *srb)
{
 ...
 _set_bit_le(5,&us->dflags);
 if (!test_bit(4, &us->dflags)) {
  _set_bit_le(2,&us->dflags);
...
# 413 "../../drivers/usb/storage/scsiglue.c"
  ...
  char **start, off_t offset, int length, int inout)
{
 ...
 do { if (pos < buffer+length) pos += sprintf(pos,"   Host scsi%d: usb-storage\n", host->host_no); } while (0);
 ...
 if (us->pusb_dev->manufacturer)
  ...
  string = "Unknown";
 do { if (pos < buffer+length) pos += sprintf(pos,"       Vendor: %s\n", string); } while (0);
 if (us->pusb_dev->product)
  ...
  string = "Unknown";
 do { if (pos < buffer+length) pos += sprintf(pos,"      Product: %s\n", string); } while (0);
 if (us->pusb_dev->serial)
  string = us->pusb_dev->serial;
 else
  string = "None";
 do { if (pos < buffer+length) pos += sprintf(pos,"Serial Number: %s\n", string); } while (0);


 do { if (pos < buffer+length) pos += sprintf(pos,"     Protocol: %s\n", us->protocol_name); } while (0);
 do { if (pos < buffer+length) pos += sprintf(pos,"    Transport: %s\n", us->transport_name); } while (0);
...
if (us->fflags & 0x00000001) pos += sprintf(pos, " " "SINGLE_LUN"); if (us->fflags & 0x00000002) pos += sprintf(pos, " " "NEED_OVERRIDE"); if (us->fflags & 0x00000004) pos += sprintf(pos, " " "SCM_MULT_TARG"); if (us->fflags & 0x00000008) pos += sprintf(pos, " " "FIX_INQUIRY"); if (us->fflags & 0x00000010) pos += sprintf(pos, " " "FIX_CAPACITY"); if (us->fflags & 0x00000020) pos += sprintf(pos, " " "IGNORE_RESIDUE"); if (us->fflags & 0x00000040) pos += sprintf(pos, " " "BULK32"); if (us->fflags & 0x00000080) pos += sprintf(pos, " " "NOT_LOCKABLE"); if (us->fflags & 0x00000100) pos += sprintf(pos, " " "GO_SLOW"); if (us->fflags & 0x00000200) pos += sprintf(pos, " " "NO_WP_DETECT"); if (us->fflags & 0x00000400) pos += sprintf(pos, " " "MAX_SECTORS_64"); if (us->fflags & 0x00000800) pos += sprintf(pos, " " "IGNORE_DEVICE"); if (us->fflags & 0x00001000) pos += sprintf(pos, " " "CAPACITY_HEURISTICS"); if (us->fflags & 0x00002000) pos += sprintf(pos, " " "MAX_SECTORS_MIN"); if (us->fflags & 0x00004000) pos += sprintf(pos, " " "BULK_IGNORE_TAG"); if (us->fflags & 0x00008000) pos += sprintf(pos, " " "SANE_SENSE"); if (us->fflags & 0x00010000) pos += sprintf(pos, " " "CAPACITY_OK"); if (us->fflags & 0x00020000) pos += sprintf(pos, " " "BAD_SENSE");
...
static struct device_attribute dev_attr_max_sectors = { .attr = {.name = "max_sectors", .mode = (00400|00040|00004) | 00200 }, .show = show_max_sectors, .store = store_max_sectors, };
...
extern typeof(usb_stor_sense_invalidCDB) usb_stor_sense_invalidCDB; static const char __kstrtab_usb_stor_sense_invalidCDB[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "usb_stor_sense_invalidCDB"; static const struct kernel_symbol __ksymtab_usb_stor_sense_invalidCDB __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&usb_stor_sense_invalidCDB, __kstrtab_usb_stor_sense_invalidCDB };
