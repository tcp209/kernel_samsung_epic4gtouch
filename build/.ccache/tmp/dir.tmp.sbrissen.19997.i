...
static inline __attribute__((always_inline)) unsigned char nls_tolower(struct nls_table *t, unsigned char c)
{
...
static inline __attribute__((always_inline)) unsigned char nls_toupper(struct nls_table *t, unsigned char c)
{
...
static inline __attribute__((always_inline)) int nls_strnicmp(struct nls_table *t, const unsigned char *s1,
...
nls_nullsize(const struct nls_table *codepage)
{
 ...
 charlen = codepage->uni2char(0, tmp, 6);
...
# 10 "../../fs/fat/fat.h" 2
# 1 "../../include/linux/msdos_fs.h" 1




# 1 "../../include/linux/magic.h" 1
# 6 "../../include/linux/msdos_fs.h" 2
...
static inline __attribute__((always_inline)) int fat_valid_media(u8 media)
{
...
struct msdos_sb_info {
 ...
 struct hlist_head inode_hashtable[(1UL << 8)];
};
...
static inline __attribute__((always_inline)) struct msdos_sb_info *MSDOS_SB(struct super_block *sb)
{
 return sb->s_fs_info;
}

static inline __attribute__((always_inline)) struct msdos_inode_info *MSDOS_I(struct inode *inode)
{
 return ({ const typeof( ((struct msdos_inode_info *)0)->vfs_inode ) *__mptr = (inode); (struct msdos_inode_info *)( (char *)__mptr - __builtin_offsetof(struct msdos_inode_info,vfs_inode) );});
}
# 141 "../../fs/fat/fat.h"
static inline __attribute__((always_inline)) int fat_mode_can_hold_ro(struct inode *inode)
{
 ...
 if (!(mask & (00200|00020|00002)))
  return 0;
 return 1;
}


static inline __attribute__((always_inline)) mode_t fat_make_mode(struct msdos_sb_info *sbi,
       u8 attrs, mode_t mode)
{
 if (attrs & 1 && !((attrs & 16) && !sbi->options.rodir))
  mode &= ~(00200|00020|00002);

 if (attrs & 16)
  return (mode & ~sbi->options.fs_dmask) | 0040000;
 else
  return (mode & ~sbi->options.fs_fmask) | 0100000;
}


static inline __attribute__((always_inline)) u8 fat_make_attrs(struct inode *inode)
{
 u8 attrs = MSDOS_I(inode)->i_attrs;
 if ((((inode->i_mode) & 00170000) == 0040000))
  attrs |= 16;
 if (fat_mode_can_hold_ro(inode) && !(inode->i_mode & (00200|00020|00002)))
  attrs |= 1;
 return attrs;
}

static inline __attribute__((always_inline)) void fat_save_attrs(struct inode *inode, u8 attrs)
{
 if (fat_mode_can_hold_ro(inode))
  MSDOS_I(inode)->i_attrs = attrs & (8 | 32 | 4 | 2);
 else
  MSDOS_I(inode)->i_attrs = attrs & ((8 | 32 | 4 | 2) | 1);
}

static inline __attribute__((always_inline)) unsigned char fat_checksum(const __u8 *name)
{
...
static inline __attribute__((always_inline)) sector_t fat_clus_to_blknr(struct msdos_sb_info *sbi, int clus)
{
 return ((sector_t)clus - 2) * sbi->sec_per_clus
  + sbi->data_start;
}

static inline __attribute__((always_inline)) void fat16_towchar(wchar_t *dst, const __u8 *src, size_t len)
{
...
static inline __attribute__((always_inline)) void fatwchar_to16(__u8 *dst, const wchar_t *src, size_t len)
{
# 229 "../../fs/fat/fat.h"
 memcpy(dst, src, len * 2);
...
static inline __attribute__((always_inline)) void fatent_init(struct fat_entry *fatent)
{
 fatent->nr_bhs = 0;
 fatent->entry = 0;
...
static inline __attribute__((always_inline)) void fatent_set_entry(struct fat_entry *fatent, int entry)
{
 fatent->entry = entry;
 fatent->u.ent32_p = ((void *)0);
}

static inline __attribute__((always_inline)) void fatent_brelse(struct fat_entry *fatent)
{
 int i;
 fatent->u.ent32_p = ((void *)0);
...
__fat_fs_error(struct super_block *s, int report, const char *fmt, ...)
 __attribute__ ((format (printf, 3, 4))) __attribute__((__cold__));
...
# 24 "../../fs/fat/dir.c" 2
# 38 "../../fs/fat/dir.c"
static inline __attribute__((always_inline)) loff_t fat_make_i_pos(struct super_block *sb,
        struct buffer_head *bh,
        struct msdos_dir_entry *de)
{
...
static inline __attribute__((always_inline)) void fat_dir_readahead(struct inode *dir, sector_t iblock,
         sector_t phys)
{
 ...
 if ((sbi->fat_bits != 32) && (dir->i_ino == 1))
  return;
 ...
 *bh = sb_bread(sb, phys);
 if (*bh == ((void *)0)) {
  printk("<3>" "FAT: Directory bread(block %llu) failed\n",
         (llu)phys);
...
static inline __attribute__((always_inline)) int fat_get_entry(struct inode *dir, loff_t *pos,
    struct buffer_head **bh,
    struct msdos_dir_entry **de)
{
         ...
         int uni_xlate, struct nls_table *nls)
{
 ...
 while (*ip && ((len - 6) > 0)) {
  ec = *ip++;
  if ((charlen = nls->uni2char(ec, op, 6)) > 0) {
   op += charlen;
   len -= charlen;
  } else {
   if (uni_xlate == 1) {
 ...
 if (__builtin_expect(!!(*ip), 0)) {
  printk("<4>" "FAT: filename was truncated while "
         "converting.");
 }
...
static inline __attribute__((always_inline)) int fat_uni_to_x8(struct msdos_sb_info *sbi, const wchar_t *uni,
    unsigned char *buf, int size)
{
 if (sbi->options.utf8)
  return utf16s_to_utf8s(uni, ((21 - 1) * 13 + 1),
...
static inline __attribute__((always_inline)) int fat_name_match(struct msdos_sb_info *sbi,
     ...
     wchar_t **unicode, unsigned char *nr_slots)
{
 ...
 if (!*unicode) {
  *unicode = kmem_cache_alloc(names_cachep, (((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u))));
  if (!*unicode) {
   brelse(*bh);
   return -12;
  }
 ...
 *nr_slots = slots;
 ...
 if ((*de)->attr == (1 | 2 | 4 | 8))
  goto parse_long;
 if ((!*((*de)->name) || *((*de)->name) == 0xe5) || ((*de)->attr & 8))
  return PARSE_INVALID;
      ...
      int name_len, struct fat_slot_info *sinfo)
{
 ...
 unsigned char work[11];
 unsigned char bufname[((11 + 1) * 6 + 1)];
...
parse_record:
  ...
  if (de->name[0] == 0xe5)
   continue;
  if (de->attr != (1 | 2 | 4 | 8) && (de->attr & 8))
   continue;
  if (de->attr != (1 | 2 | 4 | 8) && (!*(de->name) || *(de->name) == 0xe5))
   continue;
  if (de->attr == (1 | 2 | 4 | 8)) {
  ...
  if (nr_slots) {
   void *longname = unicode + ((21 - 1) * 13 + 1);
   int size = 4096 - (((21 - 1) * 13 + 1) * sizeof(wchar_t));
...
end_of_dir:
 if (unicode)
  kmem_cache_free(names_cachep, (void *)(unicode));

 return err;
}

extern typeof(fat_search_long) fat_search_long; static const char __kstrtab_fat_search_long[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_search_long"; static const struct kernel_symbol __ksymtab_fat_search_long __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_search_long, __kstrtab_fat_search_long };
    ...
    filldir_t filldir, int short_only, int both)
{
 ...
 wchar_t bufuname[14];
 wchar_t *unicode = ((void *)0);
 unsigned char c, work[11];
 unsigned char bufname[((11 + 1) * 6 + 1)], *ptname = bufname;
 unsigned short opt_shortname = sbi->options.shortname;
 ...
 if (inode->i_ino == 1) {
  while (cpos < 2) {
   if (filldir(dirent, "..", cpos+1, cpos, 1, 4) < 0)
    goto out;
   cpos++;
   filp->f_pos++;
  }
...
parse_record:
 ...
 if (isvfat && !short_only) {
  ...
  if ((de->attr & 8) || (!*(de->name) || *(de->name) == 0xe5))
   goto record_end;
 }

 if (isvfat && de->attr == (1 | 2 | 4 | 8)) {
  int status = fat_parse_long(inode, &cpos, &bh, &de,
         &unicode, &nr_slots);
  if (status < 0) {
   filp->f_pos = cpos;
 ...
 for (i2 = 8; i2 < 11;) {
  if (!(c = work[i2]))
   break;
  chl = fat_shortname2uni(nls_disk, &work[i2], 11 - i2,
     &bufuname[j++], opt_shortname,
     de->lcase & 16);
  if (chl <= 1) {
   i2++;
...
start_filldir:
 ...
 if (!memcmp(de->name, ".          ", 11))
  inum = inode->i_ino;
 else if (!memcmp(de->name, "..         ", 11)) {
...
# 733 "../../fs/fat/dir.c"
static int fat_ioctl_filldir(void *__buf, const char *name, int name_len, loff_t offset, u64 ino, unsigned int d_type) { struct fat_ioctl_filldir_callback *buf = __buf; struct __fat_dirent *d1 = buf->dirent; struct __fat_dirent *d2 = d1 + 1; if (buf->result) return -22; buf->result++; if (name != ((void *)0)) { if (name_len >= sizeof(d1->d_name)) name_len = sizeof(d1->d_name) - 1; if (({ register const typeof(*(d2->d_name)) __r2 asm("r2") = (0); register const typeof(*(d2->d_name)) *__p asm("r0") = (d2->d_name); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d2->d_reclen)) __r2 asm("r2") = (0); register const typeof(*(&d2->d_reclen)) *__p asm("r0") = (&d2->d_reclen); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || copy_to_user(d1->d_name, name, name_len) || ({ register const typeof(*(d1->d_name + name_len)) __r2 asm("r2") = (0); register const typeof(*(d1->d_name + name_len)) *__p asm("r0") = (d1->d_name + name_len); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d1->d_reclen)) __r2 asm("r2") = (name_len); register const typeof(*(&d1->d_reclen)) *__p asm("r0") = (&d1->d_reclen); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; })) goto efault; } else { const char *longname = buf->longname; int long_len = buf->long_len; const char *shortname = buf->shortname; int short_len = buf->short_len; if (long_len >= sizeof(d1->d_name)) long_len = sizeof(d1->d_name) - 1; if (short_len >= sizeof(d1->d_name)) short_len = sizeof(d1->d_name) - 1; if (copy_to_user(d2->d_name, longname, long_len) || ({ register const typeof(*(d2->d_name + long_len)) __r2 asm("r2") = (0); register const typeof(*(d2->d_name + long_len)) *__p asm("r0") = (d2->d_name + long_len); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d2->d_reclen)) __r2 asm("r2") = (long_len); register const typeof(*(&d2->d_reclen)) *__p asm("r0") = (&d2->d_reclen); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d2->d_ino)) __r2 asm("r2") = (ino); register const typeof(*(&d2->d_ino)) *__p asm("r0") = (&d2->d_ino); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d2->d_off)) __r2 asm("r2") = (offset); register const typeof(*(&d2->d_off)) *__p asm("r0") = (&d2->d_off); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || copy_to_user(d1->d_name, shortname, short_len) || ({ register const typeof(*(d1->d_name + short_len)) __r2 asm("r2") = (0); register const typeof(*(d1->d_name + short_len)) *__p asm("r0") = (d1->d_name + short_len); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }) || ({ register const typeof(*(&d1->d_reclen)) __r2 asm("r2") = (short_len); register const typeof(*(&d1->d_reclen)) *__p asm("r0") = (&d1->d_reclen); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; })) goto efault; } return 0; efault: buf->result = -14; return -14; }
...
{
 struct inode *inode = filp->f_path.dentry->d_inode;
 struct __fat_dirent *d1 = (struct __fat_dirent *)arg;
 int short_only, both;

 switch (cmd) {
 case (((2U) << (((0 +8)+8)+14)) | ((('r')) << (0 +8)) | (((2)) << 0) | (((((sizeof(struct __fat_dirent[2]) == sizeof(struct __fat_dirent[2][1]) && sizeof(struct __fat_dirent[2]) < (1 << 14)) ? sizeof(struct __fat_dirent[2]) : __invalid_size_argument_for_IOC))) << ((0 +8)+8))):
  short_only = 1;
  both = 0;
  break;
 case (((2U) << (((0 +8)+8)+14)) | ((('r')) << (0 +8)) | (((1)) << 0) | (((((sizeof(struct __fat_dirent[2]) == sizeof(struct __fat_dirent[2][1]) && sizeof(struct __fat_dirent[2]) < (1 << 14)) ? sizeof(struct __fat_dirent[2]) : __invalid_size_argument_for_IOC))) << ((0 +8)+8))):
  short_only = 0;
  both = 1;
  break;
 case (((2U) << (((0 +8)+8)+14)) | ((('r')) << (0 +8)) | (((0x12)) << 0) | (((((sizeof(__u32) == sizeof(__u32[1]) && sizeof(__u32) < (1 << 14)) ? sizeof(__u32) : __invalid_size_argument_for_IOC))) << ((0 +8)+8))):
  return fat_ioctl_volume_id(inode);
 default:
  return fat_generic_ioctl(filp, cmd, arg);
 }

 if (!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (d1), "Ir" (sizeof(struct __fat_dirent[2])), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0))
  return -14;
 ...
 if (({ register const typeof(*(&d1->d_reclen)) __r2 asm("r2") = (0); register const typeof(*(&d1->d_reclen)) *__p asm("r0") = (&d1->d_reclen); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }))
  return -14;
...
extern typeof(fat_get_dotdot_entry) fat_get_dotdot_entry; static const char __kstrtab_fat_get_dotdot_entry[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_get_dotdot_entry"; static const struct kernel_symbol __ksymtab_fat_get_dotdot_entry __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_get_dotdot_entry, __kstrtab_fat_get_dotdot_entry };
...
extern typeof(fat_dir_empty) fat_dir_empty; static const char __kstrtab_fat_dir_empty[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_dir_empty"; static const struct kernel_symbol __ksymtab_fat_dir_empty __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_dir_empty, __kstrtab_fat_dir_empty };
      ...
      struct fat_slot_info *sinfo)
{
 ...
 while (fat_get_short_entry(dir, &sinfo->slot_off, &sinfo->bh,
       &sinfo->de) >= 0) {
  if (!strncmp(sinfo->de->name, name, 11)) {
...
extern typeof(fat_scan) fat_scan; static const char __kstrtab_fat_scan[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_scan"; static const struct kernel_symbol __ksymtab_fat_scan __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_scan, __kstrtab_fat_scan };
...
int fat_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
{
 ...
 dir->i_mtime = dir->i_atime = ((struct timespec) { get_seconds(), 0 });
 if ((((dir)->i_sb->s_flags & (16|128)) || ((dir)->i_flags & (1|64))))
...
extern typeof(fat_remove_entries) fat_remove_entries; static const char __kstrtab_fat_remove_entries[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_remove_entries"; static const struct kernel_symbol __ksymtab_fat_remove_entries __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_remove_entries, __kstrtab_fat_remove_entries };
         ...
         struct buffer_head **bhs, int nr_bhs)
{
 ...
 while (blknr < last_blknr) {
  ...
  ({ void *__p = (bhs[n]->b_data); size_t __n = sb->s_blocksize; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
...
int fat_alloc_new_dir(struct inode *dir, struct timespec *ts)
{
 struct super_block *sb = dir->i_sb;
 struct msdos_sb_info *sbi = MSDOS_SB(sb);
 struct buffer_head *bhs[(((1UL) << 12) / 512)];
 struct msdos_dir_entry *de;
 sector_t blknr;
 __le16 date, time;
 u8 time_cs;
 ...
 de[0].start = (( __le16)(__u16)(cluster));
 de[0].starthi = (( __le16)(__u16)(cluster >> 16));
 de[1].start = (( __le16)(__u16)(MSDOS_I(dir)->i_logstart));
 de[1].starthi = (( __le16)(__u16)(MSDOS_I(dir)->i_logstart >> 16));
 de[0].size = de[1].size = 0;
 ({ void *__p = (de + 2); size_t __n = sb->s_blocksize - 2 * sizeof(*de); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 set_buffer_uptodate(bhs[0]);
 mark_buffer_dirty_inode(bhs[0], dir);

 err = fat_zeroed_cluster(dir, blknr, 1, bhs, (((1UL) << 12) / 512));
 if (err)
  goto error_free;
...
extern typeof(fat_alloc_new_dir) fat_alloc_new_dir; static const char __kstrtab_fat_alloc_new_dir[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_alloc_new_dir"; static const struct kernel_symbol __ksymtab_fat_alloc_new_dir __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_alloc_new_dir, __kstrtab_fat_alloc_new_dir };
 ...
 *nr_cluster = (size + (sbi->cluster_size - 1)) >> sbi->cluster_bits;
 do { if (__builtin_expect(!!(*nr_cluster > 2), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 do {
  ...
  while (blknr < last_blknr) {
   ...
   copy = ({ typeof(size) _min1 = (size); typeof(sb->s_blocksize) _min2 = (sb->s_blocksize); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
 ...
 ({ void *__p = (bhs[n]->b_data + copy); size_t __n = sb->s_blocksize - copy; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 ...
 *i_pos = fat_make_i_pos(sb, *bh, *de);
 ...
 err = fat_zeroed_cluster(dir, start_blknr, ++n, bhs, (((1UL) << 12) / 512));
 if (err)
  goto error_free;
      ...
      struct fat_slot_info *sinfo)
{
 ...
 if (dir->i_ino == 1) {
  ...
  printk("<3>" "FAT: Corrupted directory (i_pos %lld)\n",
         MSDOS_I(dir)->i_pos);
  err = -5;
  goto error;
 }
...
found:
 ...
 if (free_slots) {
  ...
  for (i = 0; i < long_bhs; i++) {
   int copy = ({ int __min1 = (sb->s_blocksize - offset); int __min2 = (size); __min1 < __min2 ? __min1: __min2; });
  ...
  if (long_bhs && (((dir)->i_sb->s_flags & (16|128)) || ((dir)->i_flags & (1|64))))
 ...
 if (nr_slots) {
  ...
  if (dir->i_size & (sbi->cluster_size - 1)) {
   __fat_fs_error(sb, 1, "Odd directory size");
...
extern typeof(fat_add_entries) fat_add_entries; static const char __kstrtab_fat_add_entries[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "fat_add_entries"; static const struct kernel_symbol __ksymtab_fat_add_entries __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&fat_add_entries, __kstrtab_fat_add_entries };
