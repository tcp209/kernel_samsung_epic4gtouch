# 1 "../../net/phonet/pn_netlink.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../net/phonet/pn_netlink.c"
# 26 "../../net/phonet/pn_netlink.c"
...
# 31 "../../net/phonet/pn_netlink.c" 2
# 1 "../../include/net/phonet/pn_dev.h" 1
...
struct phonet_device {
 ...
 unsigned long addrs[(((64) + (8 * sizeof(long)) - 1) / (8 * sizeof(long)))];
};
...
void phonet_address_notify(int event, struct net_device *dev, u8 addr)
{
 ...
 skb = nlmsg_new(( ((sizeof(struct ifaddrmsg))+4 -1) & ~(4 -1) ) +
   nla_total_size(1), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (skb == ((void *)0))
  goto errout;
 err = fill_addr(skb, dev, addr, 0, 0, event);
 if (err < 0) {
  ({ int __ret_warn_on = !!(err == -90); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../net/phonet/pn_netlink.c", 49); __builtin_expect(!!(__ret_warn_on), 0); });
  kfree_skb(skb);
  goto errout;
 }
 rtnl_notify(skb, dev_net(dev), 0,
      RTNLGRP_PHONET_IFADDR, ((void *)0), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 return;
errout:
 rtnl_set_sk_err(dev_net(dev), RTNLGRP_PHONET_IFADDR, err);
}

static const struct nla_policy ifa_phonet_policy[(__IFA_MAX - 1)+1] = {
...
static int addr_doit(struct sk_buff *skb, struct nlmsghdr *nlh, void *attr)
{
 ...
 struct nlattr *tb[(__IFA_MAX - 1)+1];
 struct net_device *dev;
 ...
 do { if (__builtin_expect(!!(!rtnl_is_locked()), 0)) { printk("<3>" "RTNL: assertion failed at %s (%d)\n", "../../net/phonet/pn_netlink.c", 76); dump_stack(); } } while(0);

 err = nlmsg_parse(nlh, sizeof(*ifm), tb, (__IFA_MAX - 1), ifa_phonet_policy);
 if (err < 0)
  return err;
   ...
   u32 pid, u32 seq, int event)
{
 ...
 do { u8 __tmp = addr; do { if (__builtin_expect(!!(nla_put(skb, IFA_LOCAL, sizeof(u8), &__tmp) < 0), 0)) goto nla_put_failure; } while(0); } while(0);
...
static int getaddr_dumpit(struct sk_buff *skb, struct netlink_callback *cb)
{
 ...
 rcu_read_lock();
 for (pnd = ({ const typeof( ((typeof(*pnd) *)0)->list ) *__mptr = (({ typeof((&pndevs->list)->next) _________p1 = (*(volatile typeof((&pndevs->list)->next) *)&((&pndevs->list)->next)); do { } while(0); (_________p1); })); (typeof(*pnd) *)( (char *)__mptr - __builtin_offsetof(typeof(*pnd),list) );}); prefetch(pnd->list.next), &pnd->list != (&pndevs->list); pnd = ({ const typeof( ((typeof(*pnd) *)0)->list ) *__mptr = (({ typeof(pnd->list.next) _________p1 = (*(volatile typeof(pnd->list.next) *)&(pnd->list.next)); do { } while(0); (_________p1); })); (typeof(*pnd) *)( (char *)__mptr - __builtin_offsetof(typeof(*pnd),list) );})) {
  u8 addr;
  ...
  addr_idx = 0;
  for ((addr) = _find_first_bit_le((pnd->addrs),(64)); (addr) < (64); (addr) = _find_next_bit_le((pnd->addrs),(64),(addr) + 1)) {
   ...
   u32 pid, u32 seq, int event)
{
 ...
 rtm->rtm_flags = 0;
 do { u8 __tmp = dst; do { if (__builtin_expect(!!(nla_put(skb, RTA_DST, sizeof(u8), &__tmp) < 0), 0)) goto nla_put_failure; } while(0); } while(0);
 do { u32 __tmp = dev->ifindex; do { if (__builtin_expect(!!(nla_put(skb, RTA_OIF, sizeof(u32), &__tmp) < 0), 0)) goto nla_put_failure; } while(0); } while(0);
 return nlmsg_end(skb, nlh);
...
void rtm_phonet_notify(int event, struct net_device *dev, u8 dst)
{
 ...
 skb = nlmsg_new(( ((sizeof(struct ifaddrmsg))+4 -1) & ~(4 -1) ) +
   nla_total_size(1) + nla_total_size(4), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (skb == ((void *)0))
  goto errout;
 err = fill_route(skb, dev, dst, 0, 0, event);
 if (err < 0) {
  ({ int __ret_warn_on = !!(err == -90); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../net/phonet/pn_netlink.c", 206); __builtin_expect(!!(__ret_warn_on), 0); });
  kfree_skb(skb);
  goto errout;
 }
 rtnl_notify(skb, dev_net(dev), 0,
     RTNLGRP_PHONET_ROUTE, ((void *)0), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 return;
errout:
 rtnl_set_sk_err(dev_net(dev), RTNLGRP_PHONET_ROUTE, err);
}

static const struct nla_policy rtm_phonet_policy[(__RTA_MAX - 1)+1] = {
 [RTA_DST] = { .type = NLA_U8 },
...
static int route_doit(struct sk_buff *skb, struct nlmsghdr *nlh, void *attr)
{
 ...
 do { if (__builtin_expect(!!(!rtnl_is_locked()), 0)) { printk("<3>" "RTNL: assertion failed at %s (%d)\n", "../../net/phonet/pn_netlink.c", 234); dump_stack(); } } while(0);

 err = nlmsg_parse(nlh, sizeof(*rtm), tb, (__RTA_MAX - 1), rtm_phonet_policy);
 if (err < 0)
  return err;
...
static int route_dumpit(struct sk_buff *skb, struct netlink_callback *cb)
{
 ...
 for (addr = 0; addr < 64; addr++) {
  ...
  if (addr_idx++ < addr_start_idx)
   continue;
  if (fill_route(skb, dev, addr << 2, (*(struct netlink_skb_parms*)&((cb->skb)->cb)).pid,
...
int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) phonet_netlink_register(void)
{
 int err = __rtnl_register(35, RTM_NEWADDR, addr_doit, ((void *)0));
 if (err)
  return err;


 __rtnl_register(35, RTM_DELADDR, addr_doit, ((void *)0));
 __rtnl_register(35, RTM_GETADDR, ((void *)0), getaddr_dumpit);
 __rtnl_register(35, RTM_NEWROUTE, route_doit, ((void *)0));
 __rtnl_register(35, RTM_DELROUTE, route_doit, ((void *)0));
 __rtnl_register(35, RTM_GETROUTE, ((void *)0), route_dumpit);
 return 0;
}
