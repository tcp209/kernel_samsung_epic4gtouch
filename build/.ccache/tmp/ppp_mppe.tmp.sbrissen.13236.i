# 1 "../../drivers/net/ppp_mppe.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../drivers/net/ppp_mppe.c"
# 46 "../../drivers/net/ppp_mppe.c"
...
# 58 "../../drivers/net/ppp_mppe.c" 2

# 1 "../../drivers/net/ppp_mppe.h" 1
# 60 "../../drivers/net/ppp_mppe.c" 2
...
static inline __attribute__((always_inline)) void sha_pad_init(struct sha_pad *shapad)
{
 ({ void *__p = (shapad->sha_pad1); size_t __n = sizeof(shapad->sha_pad1); if ((__n) != 0) { if (__builtin_constant_p((0x00)) && (0x00) == 0) __memzero((__p),(__n)); else memset((__p),(0x00),(__n)); } (__p); });
 ({ void *__p = (shapad->sha_pad2); size_t __n = sizeof(shapad->sha_pad2); if ((__n) != 0) { if (__builtin_constant_p((0xF2)) && (0xF2) == 0) __memzero((__p),(__n)); else memset((__p),(0xF2),(__n)); } (__p); });
}
...
static void *mppe_alloc(unsigned char *options, int optlen)
{
 ...
 if (optlen != 6 + sizeof(state->master_key) ||
     options[0] != 18 || options[1] != 6)
  goto out;

 state = kzalloc(sizeof(*state), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (state == ((void *)0))
  goto out;


 state->arc4 = crypto_alloc_blkcipher("ecb(arc4)", 0, 0x00000080);
 if (IS_ERR(state->arc4)) {
  state->arc4 = ((void *)0);
  goto out_free;
 }

 state->sha1 = crypto_alloc_hash("sha1", 0, 0x00000080);
 ...
 state->sha1_digest = kmalloc(digestsize, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!state->sha1_digest)
  goto out_free;


 memcpy(state->master_key, &options[6],
   ...
   const char *debugstr)
{
 ...
 do { u_char *ptr = &options[2]; mppe_opts = 0; if (!(ptr[0] & 0x01)) mppe_opts |= 0x04; if (ptr[3] & 0x40) mppe_opts |= 0x02; if (ptr[3] & 0x20) mppe_opts |= 0x01; if (ptr[3] & 0x80) mppe_opts |= 0x08; if (ptr[3] & 0x10) mppe_opts |= 0x20; if (ptr[3] & 0x01) mppe_opts |= 0x10; if (ptr[0] & ~0x01) mppe_opts |= 0x40; if (ptr[1] || ptr[2]) mppe_opts |= 0x40; if (ptr[3] & ~(0x10|0x20|0x40|0x80|0x01)) mppe_opts |= 0x40; } while ( 0);
 if (mppe_opts & 0x02)
  state->keylen = 16;
 else if (mppe_opts & 0x01)
  state->keylen = 8;
 else {
       ...
       int isize, int osize)
{
 ...
 proto = ((((__u8 *)(ibuf))[2] << 8) + ((__u8 *)(ibuf))[3]);
 ...
 if (osize < isize + 2 + 2) {
  ...
  printk("<7>" "mppe_compress[%d]: osize too small! "
         "(have: %d need: %d)\n", state->unit,
         osize, osize + 2 + 2);
  return -1;
 }
 ...
 state->ccount = (state->ccount + 1) % 0x1000;
 if (state->debug >= 7)
  ...
  int osize)
{
 ...
 if (osize < isize - 2 - 1) {
  printk("<7>" "mppe_decompress[%d]: osize too small! "
         "(have: %d need: %d)\n", state->unit,
         osize, isize - 2 - 1);
  return -1;
 }
 ...
 if (!state->stateful && !flushed) {
  printk("<7>" "mppe_decompress[%d]: FLUSHED bit not set in "
         "stateless mode!\n", state->unit);
  state->sanity_errors += 100;
  sanity = 1;
 }
 if (state->stateful && ((ccount & 0xff) == 0xff) && !flushed) {
  printk("<7>" "mppe_decompress[%d]: FLUSHED bit not set on "
         "flag packet!\n", state->unit);
  state->sanity_errors += 100;
  sanity = 1;
 }
...
static void mppe_incomp(void *arg, unsigned char *ibuf, int icnt)
{
 ...
 if (state->debug &&
     (((((__u8 *)(ibuf))[2] << 8) + ((__u8 *)(ibuf))[3]) >= 0x0021 && ((((__u8 *)(ibuf))[2] << 8) + ((__u8 *)(ibuf))[3]) <= 0x00fa))
  printk("<7>"
         "mppe_incomp[%d]: incompressible (unencrypted) data! "
         "(proto %04x)\n", state->unit, ((((__u8 *)(ibuf))[2] << 8) + ((__u8 *)(ibuf))[3]));
...
# 712 "../../drivers/net/ppp_mppe.c"
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) ppp_mppe_init(void)
{
 int answer;
 if (!(crypto_has_blkcipher("ecb(arc4)", 0, 0x00000080) &&
       crypto_has_hash("sha1", 0, 0x00000080)))
  return -19;

 sha_pad = kmalloc(sizeof(struct sha_pad), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!sha_pad)
  return -12;
...
static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) ppp_mppe_cleanup(void)
{
 ppp_unregister_compressor(&ppp_mppe);
 kfree(sha_pad);
}

static initcall_t __initcall_ppp_mppe_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = ppp_mppe_init;;
static exitcall_t __exitcall_ppp_mppe_cleanup __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = ppp_mppe_cleanup;;
