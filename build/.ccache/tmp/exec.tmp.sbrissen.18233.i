...
# 15 "../../include/linux/tsacct_kern.h"
static inline __attribute__((always_inline)) void bacct_add_tsk(struct taskstats *stats, struct task_struct *tsk)
{}
...
static inline __attribute__((always_inline)) void xacct_add_tsk(struct taskstats *stats, struct task_struct *p)
{}
static inline __attribute__((always_inline)) void acct_update_integrals(struct task_struct *tsk)
{}
static inline __attribute__((always_inline)) void acct_clear_integrals(struct task_struct *tsk)
{}
# 51 "../../fs/exec.c" 2
...
# 114 "../../include/linux/cn_proc.h"
static inline __attribute__((always_inline)) void proc_fork_connector(struct task_struct *task)
{}

static inline __attribute__((always_inline)) void proc_exec_connector(struct task_struct *task)
{}

static inline __attribute__((always_inline)) void proc_id_connector(struct task_struct *task,
         int which_id)
{}

static inline __attribute__((always_inline)) void proc_sid_connector(struct task_struct *task)
{}

static inline __attribute__((always_inline)) void proc_exit_connector(struct task_struct *task)
{}
# 52 "../../fs/exec.c" 2
...
static inline __attribute__((always_inline)) void ptrace_init_task(struct task_struct *child, bool ptrace)
{
 ...
 child->ptrace = 0;
 if (__builtin_expect(!!(ptrace), 0) && ((get_current())->ptrace & 0x00000001)) {
...
{
 do { if (__builtin_expect(!!(!list_empty(&task->ptraced)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ptrace_unlink(task);
 do { if (__builtin_expect(!!(!list_empty(&task->ptrace_entry)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
}
# 268 "../../include/linux/ptrace.h"
extern void user_enable_single_step(struct task_struct *);
extern void user_disable_single_step(struct task_struct *);
# 293 "../../include/linux/ptrace.h"
static inline __attribute__((always_inline)) void user_enable_block_step(struct task_struct *task)
{
 do { *(int *)0 = 0; } while (1);
}
# 305 "../../include/linux/ptrace.h"
static inline __attribute__((always_inline)) void user_single_step_siginfo(struct task_struct *tsk,
    struct pt_regs *regs, siginfo_t *info)
{
 ({ void *__p = (info); size_t __n = sizeof(*info); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 info->si_signo = 5;
}
...
# 111 "../../include/linux/tracehook.h"
static inline __attribute__((always_inline)) __attribute__((warn_unused_result)) int tracehook_report_syscall_entry(
 struct pt_regs *regs)
{
 ptrace_report_syscall(regs);
 return 0;
}
# 135 "../../include/linux/tracehook.h"
...
# 21 "../../arch/arm/include/asm/mmu_context.h"
...
# 22 "../../arch/arm/include/asm/mmu_context.h" 2

void __check_kvm_seq(struct mm_struct *mm);
# 45 "../../arch/arm/include/asm/mmu_context.h"
extern unsigned int cpu_last_asid;

extern __attribute__((section(".data..percpu" ""))) __typeof__(struct mm_struct *) current_mm;


void __init_new_context(struct task_struct *tsk, struct mm_struct *mm);
void __new_context(struct mm_struct *mm);

static inline __attribute__((always_inline)) void check_context(struct mm_struct *mm)
{
# 63 "../../arch/arm/include/asm/mmu_context.h"
 if (__builtin_expect(!!((mm->context.id ^ cpu_last_asid) >> 8), 0))
  __new_context(mm);

 if (__builtin_expect(!!(mm->context.kvm_seq != init_mm.context.kvm_seq), 0))
  __check_kvm_seq(mm);
}
# 97 "../../arch/arm/include/asm/mmu_context.h"
static inline __attribute__((always_inline)) void
   ...
   struct task_struct *tsk)
{
 ...
 if (!cpumask_empty((&(next)->cpu_vm_mask)) &&
     !test_bit(cpumask_check(cpu), ((((&(next)->cpu_vm_mask)))->bits)))
  __flush_icache_all();

 if (!cpumask_test_and_set_cpu(cpu, (&(next)->cpu_vm_mask)) || prev != next) {

  struct mm_struct **crt_mm = &(*({ do { const void *__vpp_verify = (typeof((&(current_mm))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(&(current_mm))) *)(&(current_mm)))); (typeof((typeof(*(&(current_mm))) *)(&(current_mm)))) (__ptr + (((__per_cpu_offset[cpu])))); }); }));
  *crt_mm = next;

  check_context(next);
  cpu_v7_switch_mm(virt_to_phys(next->pgd),next);
  if (cacheid_is((1 << 0)))
...
# 61 "../../fs/exec.c" 2
# 1 "../../arch/arm/include/asm/tlb.h" 1
# 21 "../../arch/arm/include/asm/tlb.h"
...
# 22 "../../arch/arm/include/asm/tlb.h" 2
# 30 "../../arch/arm/include/asm/tlb.h"
# 1 "../../arch/arm/include/asm/pgalloc.h" 1
# 33 "../../arch/arm/include/asm/pgalloc.h"
extern pgd_t *get_pgd_slow(struct mm_struct *mm);
extern void free_pgd_slow(struct mm_struct *mm, pgd_t *pgd);
# 57 "../../arch/arm/include/asm/pgalloc.h"
static inline __attribute__((always_inline)) pte_t *
pte_alloc_one_kernel(struct mm_struct *mm, unsigned long addr)
{
 pte_t *pte;

 pte = (pte_t *)__get_free_pages(((((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)) | (( gfp_t)0) | (( gfp_t)0x400u) | (( gfp_t)0x8000u))), 0);
 if (pte) {
  cpu_v7_dcache_clean_area(pte, sizeof(pte_t) * 512);
  pte += 512;
 }
...
static inline __attribute__((always_inline)) pgtable_t
pte_alloc_one(struct mm_struct *mm, unsigned long addr)
{
 ...
 pte = alloc_pages_node(numa_node_id(), (((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)) | (( gfp_t)0) | (( gfp_t)0x400u) | (( gfp_t)0x8000u)), 0);

 if (pte) {
  if (!is_highmem(page_zone(pte))) {
   void *page = page_address(pte);
   cpu_v7_dcache_clean_area(page, sizeof(pte_t) * 512);
  }
...
static inline __attribute__((always_inline)) void pte_free_kernel(struct mm_struct *mm, pte_t *pte)
{
 if (pte) {
  pte -= 512;
...
static inline __attribute__((always_inline)) void pte_free(struct mm_struct *mm, pgtable_t pte)
{
 pgtable_page_dtor(pte);
 __free_pages((pte), 0);
}

static inline __attribute__((always_inline)) void __pmd_populate(pmd_t *pmdp, unsigned long pmdval)
{
 pmdp[0] = (pmdval);
...
pmd_populate_kernel(struct mm_struct *mm, pmd_t *pmdp, pte_t *ptep)
{
 ...
 pte_ptr -= 512 * sizeof(void *);
 __pmd_populate(pmdp, (((unsigned long)(pte_ptr)) - (0xC0000000UL) + (0x40000000UL)) | ((1 << 0) | (1 << 4) | ((0) << 5)));
}

static inline __attribute__((always_inline)) void
pmd_populate(struct mm_struct *mm, pmd_t *pmdp, pgtable_t ptep)
{
 __pmd_populate(pmdp, ({ struct page *__pg = (ptep); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); }) << 12 | ((1 << 0) | (1 << 4) | ((1) << 5)));
}
# 31 "../../arch/arm/include/asm/tlb.h" 2
...
extern __attribute__((section(".data..percpu" ""))) __typeof__(struct mmu_gather) mmu_gathers;

static inline __attribute__((always_inline)) struct mmu_gather *
tlb_gather_mmu(struct mm_struct *mm, unsigned int full_mm_flush)
{
 struct mmu_gather *tlb = &(*({ do { do { (current_thread_info()->preempt_count) += (1); } while (0); __asm__ __volatile__("": : :"memory"); } while (0); &(*({ do { const void *__vpp_verify = (typeof((&(mmu_gathers))))((void *)0); (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(&(mmu_gathers))) *)(&(mmu_gathers)))); (typeof((typeof(*(&(mmu_gathers))) *)(&(mmu_gathers)))) (__ptr + (((__per_cpu_offset[(current_thread_info()->cpu)])))); }); })); }));
...
tlb_finish_mmu(struct mmu_gather *tlb, unsigned long start, unsigned long end)
{
 ...
 do { (void)&(mmu_gathers); do { do { __asm__ __volatile__("": : :"memory"); do { (current_thread_info()->preempt_count) -= (1); } while (0); } while (0); __asm__ __volatile__("": : :"memory"); do { if (__builtin_expect(!!(test_ti_thread_flag(current_thread_info(), 1)), 0)) preempt_schedule(); } while (0); } while (0); } while (0);
}
...
tlb_remove_tlb_entry(struct mmu_gather *tlb, pte_t *ptep, unsigned long addr)
{
 if (!tlb->fullmm) {
  ...
  if (addr + ((1UL) << 12) > tlb->range_end)
   tlb->range_end = addr + ((1UL) << 12);
 }
}
...
tlb_start_vma(struct mmu_gather *tlb, struct vm_area_struct *vma)
{
 if (!tlb->fullmm) {
  ...
  tlb->range_start = ((0xC0000000UL) - (0x01000000UL));
  tlb->range_end = 0;
 }
}
...
static rwlock_t binfmt_lock = (rwlock_t) { .raw_lock = { 0 }, };
...
int __register_binfmt(struct linux_binfmt * fmt, int insert)
{
 ...
 _raw_write_unlock(&binfmt_lock);
 return 0;
}

extern typeof(__register_binfmt) __register_binfmt; static const char __kstrtab___register_binfmt[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "__register_binfmt"; static const struct kernel_symbol __ksymtab___register_binfmt __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&__register_binfmt, __kstrtab___register_binfmt };
...
extern typeof(unregister_binfmt) unregister_binfmt; static const char __kstrtab_unregister_binfmt[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "unregister_binfmt"; static const struct kernel_symbol __ksymtab_unregister_binfmt __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&unregister_binfmt, __kstrtab_unregister_binfmt };

static inline __attribute__((always_inline)) void put_binfmt(struct linux_binfmt * fmt)
{
 ...
 long sys_uselib(const char * library)
{
 ...
 file = do_filp_open(-100, tmp,
    0400000 | 00000000 | (( fmode_t)0x20), 0,
    4 | 1 | 32);
 ...
 error = -22;
 if (!(((file->f_path.dentry->d_inode->i_mode) & 00170000) == 0100000))
  goto exit;

 error = -13;
 if (file->f_path.mnt->mnt_flags & 0x04)
  goto exit;
 ...
 if(file->f_op) {
  ...
  for (fmt = ({ const typeof( ((typeof(*fmt) *)0)->lh ) *__mptr = ((&formats)->next); (typeof(*fmt) *)( (char *)__mptr - __builtin_offsetof(typeof(*fmt),lh) );}); prefetch(fmt->lh.next), &fmt->lh != (&formats); fmt = ({ const typeof( ((typeof(*fmt) *)0)->lh ) *__mptr = (fmt->lh.next); (typeof(*fmt) *)( (char *)__mptr - __builtin_offsetof(typeof(*fmt),lh) );})) {
   if (!fmt->load_shlib)
    continue;
   if (!try_module_get(fmt->module))
    continue;
   _raw_read_unlock(&binfmt_lock);
  ...
  int write)
{
 ...
 int ret;
# 174 "../../fs/exec.c"
 ret = get_user_pages((get_current()), bprm->mm, pos,
   1, write, 1, &page, ((void *)0));
 if (ret <= 0)
  return ((void *)0);
 ...
 if (write) {
  ...
  if (size <= 131072)
   return page;
# 197 "../../fs/exec.c"
  rlim = (get_current())->signal->rlim;
  if (size > (*(volatile typeof(rlim[3].rlim_cur) *)&(rlim[3].rlim_cur)) / 4) {
   put_page(page);
   return ((void *)0);
  }
 }
...
static void flush_arg_page(struct linux_binprm *bprm, unsigned long pos,
  struct page *page)
{
 flush_cache_page(bprm->vma, pos, ({ struct page *__pg = (page); int __sec = page_to_section(__pg); (unsigned long)(__pg - __section_mem_map_addr(__nr_to_section(__sec))); }));
}
...
static int __bprm_mm_init(struct linux_binprm *bprm)
{
 ...
 bprm->vma = vma = kmem_cache_zalloc(vm_area_cachep, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!vma)
  return -12;
 ...
 do { if (__builtin_expect(!!((0x00000100 | ((((get_current())->personality & READ_IMPLIES_EXEC) ? 0x00000004 : 0) | 0x00000001 | 0x00000002 | 0x00000010 | 0x00000020 | 0x00000040) | 0x00100000) & (0x00010000 | 0x00008000)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 vma->vm_end = ((0xC0000000UL) - (0x01000000UL));
 vma->vm_start = vma->vm_end - ((1UL) << 12);
 vma->vm_flags = (0x00000100 | ((((get_current())->personality & READ_IMPLIES_EXEC) ? 0x00000004 : 0) | 0x00000001 | 0x00000002 | 0x00000010 | 0x00000020 | 0x00000040) | 0x00100000) | (0x00010000 | 0x00008000);
...
int bprm_mm_init(struct linux_binprm *bprm)
{
 ...
 err = (__init_new_context((get_current()),mm),0);
 if (err)
  goto err;
...
static int count(char * * argv, int max)
{
 ...
 if (argv != ((void *)0)) {
  for (;;) {
   ...
   if (({ register const typeof(*(argv)) *__p asm("r0") = (argv); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } p = (typeof(*(argv))) __r2; __e; }))
    return -14;
   if (!p)
    break;
   argv++;
   if (i++ >= max)
    return -7;
   ({ __might_sleep("../../fs/exec.c", 380, 0); _cond_resched(); });
  }
 }
 return i;
}
...
static int copy_strings(int argc, char * * argv,
   struct linux_binprm *bprm)
{
 struct page *kmapped_page = ((void *)0);
 char *kaddr = ((void *)0);
 ...
 while (argc-- > 0) {
  ...
  if (({ register const typeof(*(argv+argc)) *__p asm("r0") = (argv+argc); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } str = (typeof(*(argv+argc))) __r2; __e; }) ||
    !(len = strnlen_user(str, (((1UL) << 12) * 32)))) {
   ret = -14;
   goto out;
  }
  ...
  while (len > 0) {
   ...
   if (!kmapped_page || kpos != (pos & (~(((1UL) << 12)-1)))) {
    struct page *page;
...
int copy_strings_kernel(int argc,char ** argv, struct linux_binprm *bprm)
{
 ...
 set_fs(0x00000000);
 r = copy_strings(argc, (char * *)argv, bprm);
 set_fs(oldfs);
 return r;
}
extern typeof(copy_strings_kernel) copy_strings_kernel; static const char __kstrtab_copy_strings_kernel[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "copy_strings_kernel"; static const struct kernel_symbol __ksymtab_copy_strings_kernel __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&copy_strings_kernel, __kstrtab_copy_strings_kernel };
# 499 "../../fs/exec.c"
static int shift_arg_pages(struct vm_area_struct *vma, unsigned long shift)
{
 ...
 do { if (__builtin_expect(!!(new_start > new_end), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 if (vma_adjust(vma, new_start, old_end, vma->vm_pgoff, ((void *)0)))
  return -12;
 ...
 vma_adjust(vma, new_start, new_end, vma->vm_pgoff, ((void *)0));
      ...
      int executable_stack)
{
 ...
 struct vm_area_struct *prev = ((void *)0);
 unsigned long vm_flags;
...
# 595 "../../fs/exec.c"
 stack_top = (stack_top);
 stack_top = ((((stack_top)) + ((typeof((stack_top)))((((1UL) << 12))) - 1)) & ~((typeof((stack_top)))((((1UL) << 12))) - 1));
 ...
 down_write(&mm->mmap_sem);
 vm_flags = (0x00000100 | ((((get_current())->personality & READ_IMPLIES_EXEC) ? 0x00000004 : 0) | 0x00000001 | 0x00000002 | 0x00000010 | 0x00000020 | 0x00000040) | 0x00100000);
 ...
 if (__builtin_expect(!!(executable_stack == 2), 0))
  vm_flags |= 0x00000004;
 else if (executable_stack == 1)
  vm_flags &= ~0x00000004;
 vm_flags |= mm->def_flags;
 vm_flags |= (0x00010000 | 0x00008000);

 ret = mprotect_fixup(vma, &prev, vma->vm_start, vma->vm_end,
   vm_flags);
 if (ret)
  goto out_unlock;
 do { if (__builtin_expect(!!(prev != vma), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 ...
 vma->vm_flags &= ~(0x00010000 | 0x00008000);
...
extern typeof(setup_arg_pages) setup_arg_pages; static const char __kstrtab_setup_arg_pages[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "setup_arg_pages"; static const struct kernel_symbol __ksymtab_setup_arg_pages __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&setup_arg_pages, __kstrtab_setup_arg_pages };
...
extern typeof(open_exec) open_exec; static const char __kstrtab_open_exec[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "open_exec"; static const struct kernel_symbol __ksymtab_open_exec __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&open_exec, __kstrtab_open_exec };
  ...
  char *addr, unsigned long count)
{
 ...
 old_fs = (current_thread_info()->addr_limit);
 set_fs((0x00000000));
...
extern typeof(kernel_read) kernel_read; static const char __kstrtab_kernel_read[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "kernel_read"; static const struct kernel_symbol __ksymtab_kernel_read __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&kernel_read, __kstrtab_kernel_read };
...
static int exec_mmap(struct mm_struct *mm)
{
 ...
 if (old_mm) {
  ...
  if (__builtin_expect(!!(old_mm->core_state), 0)) {
 ...
 tsk->mm = mm;
 tsk->active_mm = mm;
 switch_mm(active_mm, mm, ((void *)0));
 task_unlock(tsk);
 arch_pick_mmap_layout(mm);
 if (old_mm) {
  up_read(&old_mm->mmap_sem);
  do { if (__builtin_expect(!!(active_mm != old_mm), 0)) do { *(int *)0 = 0; } while (1); } while(0);
...
static int de_thread(struct task_struct *tsk)
{
 ...
 while (sig->notify_count) {
  do { (get_current())->state = (2); } while (0);
  spin_unlock_irq(lock);
  schedule();
  spin_lock_irq(lock);
 }
 ...
 if (!(tsk == tsk->group_leader)) {
  ...
  for (;;) {
   ...
   if (__builtin_expect(!!(leader->exit_state), 1))
    break;
   do { (get_current())->state = (2); } while (0);
   _raw_write_unlock_irq(&tasklist_lock);
   schedule();
  }
# 829 "../../fs/exec.c"
  tsk->start_time = leader->start_time;

  do { if (__builtin_expect(!!(!same_thread_group(leader, tsk)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
  do { if (__builtin_expect(!!(has_group_leader_pid(tsk)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
# 845 "../../fs/exec.c"
  ...
  do { if (__builtin_expect(!!(leader->exit_state != 16), 0)) do { *(int *)0 = 0; } while (1); } while(0);
  leader->exit_state = 32;
  _raw_write_unlock_irq(&tasklist_lock);
...
no_thread_group:
 if ((get_current())->mm)
  setmax_mm_hiwater_rss(&sig->maxrss, (get_current())->mm);

 exit_itimers(sig);
 flush_itimer_signals();

 if ((*(volatile int *)&(&oldsighand->count)->counter) != 1) {
  struct sighand_struct *newsighand;




  newsighand = kmem_cache_alloc(sighand_cachep, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
  if (!newsighand)
   return -12;

  (((&newsighand->count)->counter) = (1));
  memcpy(newsighand->action, oldsighand->action,
         sizeof(newsighand->action));

  _raw_write_lock_irq(&tasklist_lock);
  spin_lock(&oldsighand->siglock);
  ({ if (!__builtin_constant_p(newsighand) || ((newsighand) != ((void *)0))) __asm__ __volatile__ ("dmb" : : : "memory"); (tsk->sighand) = (newsighand); });
 ...
 do { if (__builtin_expect(!!(!(tsk == tsk->group_leader)), 0)) do { *(int *)0 = 0; } while (1); } while(0);
 return 0;
}
...
void set_task_comm(struct task_struct *tsk, char *buf)
{
 ...
 ({ void *__p = (tsk->comm); size_t __n = 16; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
...
int flush_old_exec(struct linux_binprm * bprm)
{
 ...
 flush_thread();
 (get_current())->personality &= ~bprm->per_clear;
...
extern typeof(flush_old_exec) flush_old_exec; static const char __kstrtab_flush_old_exec[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "flush_old_exec"; static const struct kernel_symbol __ksymtab_flush_old_exec __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&flush_old_exec, __kstrtab_flush_old_exec };

void setup_new_exec(struct linux_binprm * bprm)
{
 int i, ch;
 char * name;
 char tcomm[sizeof((get_current())->comm)];

 arch_pick_mmap_layout((get_current())->mm);


 (get_current())->sas_ss_sp = (get_current())->sas_ss_size = 0;

 if ((({ (get_current())->cred->euid; })) == (({ (get_current())->cred->uid; })) && (({ (get_current())->cred->egid; })) == (({ (get_current())->cred->gid; })))
  set_dumpable((get_current())->mm, 1);
 else
  set_dumpable((get_current())->mm, suid_dumpable);
 ...
 tcomm[i] = '\0';
 set_task_comm((get_current()), tcomm);





 (get_current())->mm->task_size = ((0xC0000000UL) - (0x01000000UL));


 if (bprm->cred->uid != (({ (get_current())->cred->euid; })) ||
     bprm->cred->gid != (({ (get_current())->cred->egid; }))) {
  (get_current())->pdeath_signal = 0;
 } else if (file_permission(bprm->file, 4) ||
     bprm->interp_flags & (1 << 0)) {
  set_dumpable((get_current())->mm, suid_dumpable);
 }
 ...
 if (!get_dumpable((get_current())->mm))
  perf_event_exit_task((get_current()));




 (get_current())->self_exec_id++;

 flush_signal_handlers((get_current()), 0);
 flush_old_files((get_current())->files);
}
extern typeof(setup_new_exec) setup_new_exec; static const char __kstrtab_setup_new_exec[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "setup_new_exec"; static const struct kernel_symbol __ksymtab_setup_new_exec __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&setup_new_exec, __kstrtab_setup_new_exec };
...
{
 if (mutex_lock_interruptible(&(get_current())->cred_guard_mutex))
  return -513;

 bprm->cred = prepare_exec_creds();
 if (__builtin_expect(!!(bprm->cred), 1))
  return 0;

 mutex_unlock(&(get_current())->cred_guard_mutex);
 return -12;
}
...
extern typeof(install_exec_creds) install_exec_creds; static const char __kstrtab_install_exec_creds[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "install_exec_creds"; static const struct kernel_symbol __ksymtab_install_exec_creds __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&install_exec_creds, __kstrtab_install_exec_creds };
...
{
 struct task_struct *p = (get_current()), *t;
 unsigned n_fs;
 int res = 0;
...
int prepare_binprm(struct linux_binprm *bprm)
{
 ...
 bprm->cred->euid = (({ (get_current())->cred->euid; }));
 bprm->cred->egid = (({ (get_current())->cred->egid; }));

 if (!(bprm->file->f_path.mnt->mnt_flags & 0x01)) {

  if (mode & 0004000) {
   bprm->per_clear |= (READ_IMPLIES_EXEC | ADDR_NO_RANDOMIZE | ADDR_COMPAT_LAYOUT | MMAP_PAGE_ZERO);
  ...
  if ((mode & (0002000 | 00010)) == (0002000 | 00010)) {
   bprm->per_clear |= (READ_IMPLIES_EXEC | ADDR_NO_RANDOMIZE | ADDR_COMPAT_LAYOUT | MMAP_PAGE_ZERO);
 ...
 ({ void *__p = (bprm->buf); size_t __n = 128; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 return kernel_read(bprm->file, 0, bprm->buf, 128);
}

extern typeof(prepare_binprm) prepare_binprm; static const char __kstrtab_prepare_binprm[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "prepare_binprm"; static const struct kernel_symbol __ksymtab_prepare_binprm __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&prepare_binprm, __kstrtab_prepare_binprm };
...
int remove_arg_zero(struct linux_binprm *bprm)
{
 ...
 do {
  ...
  for (; offset < ((1UL) << 12) && kaddr[offset];
    offset++, bprm->p++)
   ;
  ...
  if (offset == ((1UL) << 12))
   free_arg_page(bprm, (bprm->p >> 12) - 1);
 } while (offset == ((1UL) << 12));
...
extern typeof(remove_arg_zero) remove_arg_zero; static const char __kstrtab_remove_arg_zero[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "remove_arg_zero"; static const struct kernel_symbol __ksymtab_remove_arg_zero __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&remove_arg_zero, __kstrtab_remove_arg_zero };
...
int search_binary_handler(struct linux_binprm *bprm,struct pt_regs *regs)
{
 ...
 set_fs(((0xC0000000UL) - (0x01000000UL)));
 ...
 for (try=0; try<2; try++) {
  ...
  for (fmt = ({ const typeof( ((typeof(*fmt) *)0)->lh ) *__mptr = ((&formats)->next); (typeof(*fmt) *)( (char *)__mptr - __builtin_offsetof(typeof(*fmt),lh) );}); prefetch(fmt->lh.next), &fmt->lh != (&formats); fmt = ({ const typeof( ((typeof(*fmt) *)0)->lh ) *__mptr = (fmt->lh.next); (typeof(*fmt) *)( (char *)__mptr - __builtin_offsetof(typeof(*fmt),lh) );})) {
   ...
   if (retval >= 0) {
    ...
    if (bprm->file)
     fput(bprm->file);
    bprm->file = ((void *)0);
    (get_current())->did_exec = 1;
    proc_exec_connector((get_current()));
    return retval;
   }
   _raw_read_lock(&binfmt_lock);
   put_binfmt(fmt);
   if (retval != -8 || bprm->mm == ((void *)0))
    break;
   if (!bprm->file) {
    _raw_read_unlock(&binfmt_lock);
    return retval;
  ...
  if (retval != -8 || bprm->mm == ((void *)0)) {
   ...
   if ((((bprm->buf[0])=='\t') || ((bprm->buf[0])=='\n') || (0x20<=(bprm->buf[0]) && (bprm->buf[0])<=0x7e)) &&
       (((bprm->buf[1])=='\t') || ((bprm->buf[1])=='\n') || (0x20<=(bprm->buf[1]) && (bprm->buf[1])<=0x7e)) &&
       (((bprm->buf[2])=='\t') || ((bprm->buf[2])=='\n') || (0x20<=(bprm->buf[2]) && (bprm->buf[2])<=0x7e)) &&
       (((bprm->buf[3])=='\t') || ((bprm->buf[3])=='\n') || (0x20<=(bprm->buf[3]) && (bprm->buf[3])<=0x7e)))
    break;
   __request_module(true, "binfmt-%04x", *(unsigned short *)(&bprm->buf[2]));
...
extern typeof(search_binary_handler) search_binary_handler; static const char __kstrtab_search_binary_handler[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "search_binary_handler"; static const struct kernel_symbol __ksymtab_search_binary_handler __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&search_binary_handler, __kstrtab_search_binary_handler };
 ...
 struct pt_regs * regs)
{
 ...
 retval = -12;
 bprm = kzalloc(sizeof(*bprm), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!bprm)
  goto out_files;
 ...
 bprm->argc = count(argv, 0x7FFFFFFF);
 if ((retval = bprm->argc) < 0)
  goto out;

 bprm->envc = count(envp, 0x7FFFFFFF);
 ...
 (get_current())->flags &= ~0x00200000;
 retval = search_binary_handler(bprm,regs);
 if (retval < 0)
  goto out;


 (get_current())->fs->in_exec = 0;
 (get_current())->in_execve = 0;
 acct_update_integrals((get_current()));
 free_bprm(bprm);
 if (displaced)
...
extern typeof(set_binfmt) set_binfmt; static const char __kstrtab_set_binfmt[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "set_binfmt"; static const struct kernel_symbol __ksymtab_set_binfmt __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&set_binfmt, __kstrtab_set_binfmt };
...
{
 const struct cred *cred = ((get_current())->cred);
 const char *pat_ptr = core_pattern;
 int ispipe = (*pat_ptr == '|');
 char *out_ptr = corename;
 char *const out_end = corename + 128;
 int rc;
...
static int zap_process(struct task_struct *start, int exit_code)
{
 ...
 start->signal->flags = 0x00000008;
 start->signal->group_exit_code = exit_code;
 start->signal->group_stop_count = 0;

 t = start;
 do {
  if (t != (get_current()) && t->mm) {
   sigaddset(&t->pending.signal, 9);
   signal_wake_up(t, 1);
   nr++;
  }
...
static inline __attribute__((always_inline)) int zap_threads(struct task_struct *tsk, struct mm_struct *mm,
    struct core_state *core_state, int exit_code)
{
 ...
 if ((*(volatile int *)&(&mm->mm_users)->counter) == nr + 1)
  goto done;
# 1635 "../../fs/exec.c"
 rcu_read_lock();
 for (g = &init_task ; (g = ({ const typeof( ((struct task_struct *)0)->tasks ) *__mptr = (({ typeof((g)->tasks.next) _________p1 = (*(volatile typeof((g)->tasks.next) *)&((g)->tasks.next)); do { } while(0); (_________p1); })); (struct task_struct *)( (char *)__mptr - __builtin_offsetof(struct task_struct,tasks) );})) != &init_task ; ) {
  if (g == tsk->group_leader)
   continue;
  if (g->flags & 0x00200000)
   continue;
  p = g;
  do {
   if (p->mm) {
    if (__builtin_expect(!!(p->mm == mm), 0)) {
 ...
 (((&core_state->nr_threads)->counter) = (nr));
 return nr;
}
...
static int coredump_wait(int exit_code, struct core_state *core_state)
{
 ...
 if (__builtin_expect(!!(core_waiters < 0), 0))
  goto fail;
...
void set_dumpable(struct mm_struct *mm, int value)
{
 switch (value) {
 case 0:
  _clear_bit_le(0,&mm->flags);
  __asm__ __volatile__ ("dmb" : : : "memory");
  _clear_bit_le(1,&mm->flags);
  break;
 case 1:
  _set_bit_le(0,&mm->flags);
  __asm__ __volatile__ ("dmb" : : : "memory");
  _clear_bit_le(1,&mm->flags);
  break;
 case 2:
  _set_bit_le(1,&mm->flags);
  __asm__ __volatile__ ("dmb" : : : "memory");
  _set_bit_le(0,&mm->flags);
  break;
 }
...
static void wait_for_dump_helpers(struct file *file)
{
 ...
 while ((pipe->readers > 1) && (!signal_pending((get_current())))) {
  __wake_up_sync((&pipe->wait), 1, 1);
  kill_fasync(&pipe->fasync_readers, 29, ((2 << 16)|1));
  pipe_wait(pipe);
 }
...
static int umh_pipe_setup(struct subprocess_info *info)
{
 ...
 struct files_struct *cf = (get_current())->files;
 ...
 fdt = ((({ typeof(((cf)->fdt)) _________p1 = (*(volatile typeof(((cf)->fdt)) *)&(((cf)->fdt))); do { } while(0); (_________p1); })));
 (((fd_set *)(fdt->open_fds))->fds_bits[(0) >> 5] |= (1<<((0) & 31)));
 (((fd_set *)(fdt->close_on_exec))->fds_bits[(0) >> 5] &= ~(1<<((0) & 31)));
 spin_unlock(&cf->file_lock);


 (get_current())->signal->rlim[4] = (struct rlimit){1, 1};
...
void do_coredump(long signr, int exit_code, struct pt_regs *regs)
{
 ...
 int flag = 0;
 int ispipe;
 static atomic_t core_dump_count = { (0) };
 ...
 clear_ti_thread_flag(current_thread_info(), 0);





  do { _lock_kernel(__func__, "../../fs/exec.c", 1898); } while (0);
 ispipe = format_corename(corename, signr);
 do { _unlock_kernel(__func__, "../../fs/exec.c", 1900); } while (0);
...
# 1921 "../../fs/exec.c"
   printk("<4>"
    ...
    task_tgid_vnr((get_current())), (get_current())->comm);
   printk("<4>" "Aborting core\n");
   goto fail_unlock;
  }
  cprm.limit = (~0UL);

  dump_count = (atomic_add_return(1, &core_dump_count));
  if (core_pipe_limit && (core_pipe_limit < dump_count)) {
   printk("<4>" "Pid %d(%s) over core_pipe_limit\n",
          task_tgid_vnr((get_current())), (get_current())->comm);
   printk("<4>" "Skipping core dump\n");
   goto fail_dropcount;
  }

  helper_argv = argv_split(((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)), corename+1, ((void *)0));
  if (!helper_argv) {
   printk("<4>" "%s failed to allocate memory\n",
          __func__);
   goto fail_dropcount;
  }

  retval = call_usermodehelper_fns(helper_argv[0], helper_argv,
     ((void *)0), UMH_WAIT_EXEC, umh_pipe_setup,
     ((void *)0), &cprm);
  argv_free(helper_argv);
  if (retval) {
    printk("<6>" "Core dump to %s pipe failed\n",
          corename);
   goto close_fail;
   }
 } else {
  ...
  cprm.file = filp_open(corename,
     00000100 | 2 | 0100000 | 0400000 | flag,
     0600);
  if (IS_ERR(cprm.file))
   goto fail_unlock;
  ...
  if (inode->i_uid != (({ (get_current())->cred->fsuid; })))
   goto close_fail;
 ...
 if (retval)
  (get_current())->signal->group_exit_code |= 0x80;

 if (ispipe && core_pipe_limit)
  wait_for_dump_helpers(cprm.file);
close_fail:
 if (cprm.file)
  filp_close(cprm.file, ((void *)0));
fail_dropcount:
 if (ispipe)
  atomic_sub(1, &core_dump_count);
fail_unlock:
 coredump_finish(mm);
