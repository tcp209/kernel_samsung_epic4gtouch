# 1 "../../fs/fat/namei_msdos.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../fs/fat/namei_msdos.c"
# 9 "../../fs/fat/namei_msdos.c"
...
static int msdos_hash(struct dentry *dentry, struct qstr *qstr)
{
 ...
 if (!error)
  qstr->hash = full_name_hash(msdos_name, 11);
 return 0;
}
...
{
 struct fat_mount_options *options = &MSDOS_SB(dentry->d_sb)->options;
 unsigned char a_msdos_name[11], b_msdos_name[11];
 int error;
 ...
 if (error)
  goto old_compare;
 error = memcmp(a_msdos_name, b_msdos_name, 11);
out:
 return error;
...
static int msdos_rmdir(struct inode *dir, struct dentry *dentry)
{
 ...
 clear_nlink(inode);
 inode->i_ctime = ((struct timespec) { get_seconds(), 0 });
 fat_detach(inode);
out:
 unlock_super(sb);
 if (!err)
      ...
      struct dentry *new_dentry, int is_hid)
{
 ...
 if (!err) {
  if (!new_inode) {
   ...
   old_dir->i_version++;
   old_dir->i_ctime = old_dir->i_mtime = ((struct timespec) { get_seconds(), 0 });
   if ((((old_dir)->i_sb->s_flags & (16|128)) || ((old_dir)->i_flags & (1|64))))
   ...
   struct inode *new_dir, struct dentry *new_dentry)
{
 ...
 unsigned char old_msdos_name[11], new_msdos_name[11];
 int err, is_hid;
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) init_msdos_fs(void)
{
 return register_filesystem(&msdos_fs_type);
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) exit_msdos_fs(void)
{
...
static initcall_t __initcall_init_msdos_fs6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = init_msdos_fs;
static exitcall_t __exitcall_exit_msdos_fs __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = exit_msdos_fs;
