...
# 17 "../../drivers/char/mem.c" 2

# 1 "../../include/linux/raw.h" 1
# 9 "../../include/linux/raw.h"
...
# 24 "../../drivers/char/mem.c" 2
# 1 "../../include/linux/crash_dump.h" 1
# 63 "../../include/linux/crash_dump.h"
static inline __attribute__((always_inline)) int is_kdump_kernel(void) { return 0; }


extern unsigned long saved_max_pfn;
# 25 "../../drivers/char/mem.c" 2
...
# 32 "../../drivers/char/mem.c" 2
# 41 "../../drivers/char/mem.c"
static inline __attribute__((always_inline)) unsigned long size_inside_page(unsigned long start,
          unsigned long size)
{
 unsigned long sz;

 sz = ((1UL) << 12) - (start & (((1UL) << 12) - 1));

 return ({ typeof(sz) _min1 = (sz); typeof(size) _min2 = (size); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
}
# 87 "../../drivers/char/mem.c"
static inline __attribute__((always_inline)) int range_is_allowed(unsigned long pfn, unsigned long size)
{
 return 1;
}
   ...
   size_t count, loff_t *ppos)
{
 ...
 if (!valid_phys_addr_range(p, count))
  return -14;
 read = 0;
# 128 "../../drivers/char/mem.c"
 while (count > 0) {
  unsigned long remaining;

  sz = size_inside_page(p, count);

  if (!range_is_allowed(p >> 12, count))
   return -1;
  ...
  ptr = ((void *)(((unsigned long)(p)) - (0x40000000UL) + (0xC0000000UL)));
  if (!ptr)
   return -14;
    ...
    size_t count, loff_t *ppos)
{
 ...
 written = 0;
# 185 "../../drivers/char/mem.c"
 while (count > 0) {
  sz = size_inside_page(p, count);

  if (!range_is_allowed(p >> 12, sz))
   return -1;
...
# 239 "../../drivers/char/mem.c"
static int uncached_access(struct file *file, unsigned long addr)
{
# 260 "../../drivers/char/mem.c"
 if (file->f_flags & 00010000)
  return 1;
 return addr >= (((unsigned long)(high_memory)) - (0xC0000000UL) + (0x40000000UL));
         ...
         unsigned long size, pgprot_t vma_prot)
{
 ...
 unsigned long offset = pfn << 12;

 if (uncached_access(file, offset))
  return (((vma_prot) & ~((0x0f << 2))) | ((0x00 << 2)));

 return vma_prot;
}
# 300 "../../drivers/char/mem.c"
static inline __attribute__((always_inline)) int private_mapping_ok(struct vm_area_struct *vma)
{
 return 1;
}
...
static int mmap_kmem(struct file *file, struct vm_area_struct *vma)
{
 ...
 pfn = (((unsigned long)((u64)vma->vm_pgoff << 12)) - (0xC0000000UL) + (0x40000000UL)) >> 12;
# 362 "../../drivers/char/mem.c"
 if (!pfn_valid(pfn))
  return -5;
...
# 435 "../../drivers/char/mem.c"
 ...
 if (count > 0) {
  kbuf = (char *)__get_free_pages((((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u))), 0);
  if (!kbuf)
   return -12;
  while (count > 0) {
   sz = size_inside_page(p, count);
  ...
  free_pages(((unsigned long)kbuf), 0);
 }
 *ppos = p;
...
static ssize_t do_write_kmem(unsigned long p, const char *buf,
    size_t count, loff_t *ppos)
{
 ssize_t written, sz;
 unsigned long copied;
     ...
     size_t count, loff_t *ppos)
{
 ...
 if (p < (unsigned long) high_memory) {
  unsigned long to_write = ({ unsigned long __min1 = (count); unsigned long __min2 = ((unsigned long)high_memory - p); __min1 < __min2 ? __min1: __min2; });
    ...
    size_t count, loff_t *ppos)
{
 ...
 if (!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (buf), "Ir" (count), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0))
  return -14;
 ...
 while (count) {
  ...
  if (signal_pending((get_current())))
   return written ? written : -512;
  buf += chunk;
  count -= chunk;
  ({ __might_sleep("../../drivers/char/mem.c", 679, 0); _cond_resched(); });
 }
 return written ? written : -14;
}
...
{
 return capable(17) ? 0 : -1;
}
# 763 "../../drivers/char/mem.c"
...
static struct backing_dev_info zero_bdi = {
 .name = "char/mem",
 .capabilities = 0x00000004 | (0x00000002 | 0x00000001 | 0x00000080),
};
     ...
     size_t count, loff_t *ppos)
{
 ...
 tmp = kmalloc(count + 1, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (tmp == ((void *)0))
  return -12;
 ret = -14;
...
static const struct memdev {
 ...
 struct backing_dev_info *dev_info;
  ...
  [3] = { "null", 0666, &null_fops, ((void *)0) },



  [5] = { "zero", 0666, &zero_fops, &zero_bdi },
  [7] = { "full", 0666, &full_fops, ((void *)0) },
  [8] = { "random", 0666, &random_fops, ((void *)0) },
  [9] = { "urandom", 0666, &urandom_fops, ((void *)0) },
 [11] = { "kmsg", 0, &kmsg_fops, ((void *)0) },




 [13] = {"s3c-mem", 00400 | 00200 | 00040 | 00020 | 00004
      | 00002, &s3c_mem_fops},
...
static int memory_open(struct inode *inode, struct file *filp)
{
 ...
 minor = iminor(inode);
 if (minor >= (sizeof(devlist) / sizeof((devlist)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(devlist), typeof(&devlist[0]))); }))))
  return -6;
...
{
 if (mode && devlist[((unsigned int) ((dev->devt) & ((1U << 20) - 1)))].mode)
  *mode = devlist[((unsigned int) ((dev->devt) & ((1U << 20) - 1)))].mode;
 return ((void *)0);
}

static struct class *mem_class;

static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) chr_dev_init(void)
{
 int minor;
 int err;
 ...
 mem_class = ({ static struct lock_class_key __key; __class_create(((struct module *)0), "mem", &__key); });
 if (IS_ERR(mem_class))
  return PTR_ERR(mem_class);

 mem_class->devnode = mem_devnode;
 for (minor = 1; minor < (sizeof(devlist) / sizeof((devlist)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(devlist), typeof(&devlist[0]))); }))); minor++) {
  if (!devlist[minor].name)
   continue;
  device_create(mem_class, ((void *)0), (((1) << 20) | (minor)),
         ((void *)0), devlist[minor].name);
 }

 return tty_init();
}

static initcall_t __initcall_chr_dev_init5 __attribute__((__used__)) __attribute__((__section__(".initcall" "5" ".init"))) = chr_dev_init;
