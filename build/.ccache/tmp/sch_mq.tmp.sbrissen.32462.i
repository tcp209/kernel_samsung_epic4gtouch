# 1 "../../net/sched/sch_mq.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../net/sched/sch_mq.c"
# 11 "../../net/sched/sch_mq.c"
...
# 18 "../../net/sched/sch_mq.c" 2
# 1 "../../include/net/pkt_sched.h" 1
...
static inline __attribute__((always_inline)) spinlock_t *qdisc_root_sleeping_lock(struct Qdisc *qdisc)
{
 ...
 do { if (__builtin_expect(!!(!rtnl_is_locked()), 0)) { printk("<3>" "RTNL: assertion failed at %s (%d)\n", "../../include/net/sch_generic.h", 230); dump_stack(); } } while(0);
 return qdisc_lock(root);
}
...
qdisc_class_find(struct Qdisc_class_hash *hash, u32 id)
{
 ...
 for (n = (&hash->hash[h])->first; n && ({ prefetch(n->next); 1;}) && ({ cl = ({ const typeof( ((typeof(*cl) *)0)->hnode ) *__mptr = (n); (typeof(*cl) *)( (char *)__mptr - __builtin_offsetof(typeof(*cl),hnode) );}); 1;}); n = n->next) {
  if (cl->classid == id)
   return cl;
 }
 return ((void *)0);
...
static inline __attribute__((always_inline)) void qdisc_reset_all_tx_gt(struct net_device *dev, unsigned int i)
{
 struct Qdisc *qdisc;
...
static inline __attribute__((always_inline)) int qdisc_enqueue_root(struct sk_buff *skb, struct Qdisc *sch)
{
 ...
 return qdisc_enqueue(skb, sch) & 0x0f;
}

static inline __attribute__((always_inline)) void __qdisc_update_bstats(struct Qdisc *sch, unsigned int len)
{
...
static inline __attribute__((always_inline)) unsigned int __qdisc_queue_drop_head(struct Qdisc *sch,
           struct sk_buff_head *list)
{
...
static inline __attribute__((always_inline)) unsigned int qdisc_queue_drop_head(struct Qdisc *sch)
{
...
{
 return (char *) q + (((sizeof(struct Qdisc)) + 64 -1) & ~(64 -1));
}
...
{
 return ((ktime_to_ns(ktime_get())) >> 6);
}
...
static inline __attribute__((always_inline)) void qdisc_run(struct Qdisc *q)
{
 if (!_test_and_set_bit_le(__QDISC_STATE_RUNNING,&q->state))
  __qdisc_run(q);
}
...
static int mq_init(struct Qdisc *sch, struct nlattr *opt)
{
 ...
 for (ntx = 0; ntx < dev->num_tx_queues; ntx++) {
  ...
  qdisc = qdisc_create_dflt(dev, dev_queue, &pfifo_fast_ops,
       (((((sch->handle)&(0xFFFF0000U)))&(0xFFFF0000U))|((((ntx + 1)&(0x0000FFFFU)))&(0x0000FFFFU))));
...
static int mq_dump(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 sch->q.qlen = 0;
 ({ void *__p = (&sch->bstats); size_t __n = sizeof(sch->bstats); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 ({ void *__p = (&sch->qstats); size_t __n = sizeof(sch->qstats); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
...
static struct netdev_queue *mq_select_queue(struct Qdisc *sch,
         struct tcmsg *tcm)
{
 unsigned int ntx = ((tcm->tcm_parent)&(0x0000FFFFU));
...
{
 unsigned int ntx = ((classid)&(0x0000FFFFU));
