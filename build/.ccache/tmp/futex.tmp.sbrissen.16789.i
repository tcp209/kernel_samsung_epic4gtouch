...
# 1 "../../arch/arm/include/asm/futex.h" 1
...
# 1 "../../include/asm-generic/futex.h" 1
...
static inline __attribute__((always_inline)) int
futex_atomic_op_inuser (int encoded_op, int *uaddr)
{
 ...
 if (! (({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (uaddr), "Ir" (sizeof(int)), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0))
  return -14;
 ...
 if (!ret) {
  switch (cmp) {
  case 0: ret = (oldval == cmparg); break;
  case 1: ret = (oldval != cmparg); break;
  case 2: ret = (oldval < cmparg); break;
  case 5: ret = (oldval >= cmparg); break;
  case 3: ret = (oldval <= cmparg); break;
  case 4: ret = (oldval > cmparg); break;
  default: ret = -38;
  }
 }
 return ret;
...
static inline __attribute__((always_inline)) int
futex_atomic_cmpxchg_inatomic(int *uaddr, int oldval, int newval)
{
 return -38;
}
# 9 "../../arch/arm/include/asm/futex.h" 2
# 64 "../../kernel/futex.c" 2

# 1 "../../kernel/rtmutex_common.h" 1
# 47 "../../kernel/rtmutex_common.h"
struct rt_mutex_waiter {
...
static inline __attribute__((always_inline)) int rt_mutex_has_waiters(struct rt_mutex *lock)
{
 return !plist_head_empty(&lock->wait_list);
}

static inline __attribute__((always_inline)) struct rt_mutex_waiter *
rt_mutex_top_waiter(struct rt_mutex *lock)
{
 struct rt_mutex_waiter *w;

 w = ({ const typeof( ((struct rt_mutex_waiter *)0)->list_entry ) *__mptr = (plist_first(&lock->wait_list)); (struct rt_mutex_waiter *)( (char *)__mptr - __builtin_offsetof(struct rt_mutex_waiter,list_entry) );});

 do { if (__builtin_expect(!!(w->lock != lock), 0)) do { *(int *)0 = 0; } while (1); } while(0);

 return w;
}

static inline __attribute__((always_inline)) int task_has_pi_waiters(struct task_struct *p)
{
...
{
 return ({ const typeof( ((struct rt_mutex_waiter *)0)->pi_list_entry ) *__mptr = (plist_first(&p->pi_waiters)); (struct rt_mutex_waiter *)( (char *)__mptr - __builtin_offsetof(struct rt_mutex_waiter,pi_list_entry) );});

}
# 98 "../../kernel/rtmutex_common.h"
static inline __attribute__((always_inline)) struct task_struct *rt_mutex_owner(struct rt_mutex *lock)
{
 return (struct task_struct *)
  ((unsigned long)lock->owner & ~3UL);
}

static inline __attribute__((always_inline)) struct task_struct *rt_mutex_real_owner(struct rt_mutex *lock)
{
 return (struct task_struct *)
  ((unsigned long)lock->owner & ~2UL);
}

static inline __attribute__((always_inline)) unsigned long rt_mutex_owner_pending(struct rt_mutex *lock)
{
 return (unsigned long)lock->owner & 1UL;
}
...
# 1 "../../kernel/rtmutex.h" 1
# 136 "../../kernel/rtmutex_common.h" 2
# 66 "../../kernel/futex.c" 2
...
static struct futex_hash_bucket futex_queues[1<<(0 ? 4 : 8)];
...
static struct futex_hash_bucket *hash_futex(union futex_key *key)
{
 u32 hash = jhash2((u32*)&key->both.word,
     ...
     key->both.offset);
 return &futex_queues[hash & ((1 << (0 ? 4 : 8))-1)];
}




static inline __attribute__((always_inline)) int match_futex(union futex_key *key1, union futex_key *key2)
{
 return (key1 && key2
...
static void get_futex_key_refs(union futex_key *key)
{
 ...
 switch (key->both.offset & (1|2)) {
 case 1:
  atomic_add(1, &key->shared.inode->i_count);
  break;
 case 2:
  atomic_add(1, &key->private.mm->mm_count);
  break;
 }
}
...
static void drop_futex_key_refs(union futex_key *key)
{
 if (!key->both.ptr) {
  ...
  ({ static bool __warned; int __ret_warn_once = !!(1); if (__builtin_expect(!!(__ret_warn_once), 0)) if (({ int __ret_warn_on = !!(!__warned); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 187); __builtin_expect(!!(__ret_warn_on), 0); })) __warned = true; __builtin_expect(!!(__ret_warn_once), 0); });
  return;
 }
...
# 216 "../../kernel/futex.c"
static int
get_futex_key(u32 *uaddr, int fshared, union futex_key *key)
{
 ...
 key->both.offset = address % ((1UL) << 12);
 if (__builtin_expect(!!((address % sizeof(u32)) != 0), 0))
  return -22;
 address -= key->both.offset;
# 240 "../../kernel/futex.c"
 if (!fshared) {
  if (__builtin_expect(!!(!(({ unsigned long flag, roksum; (void)0; __asm__("adds %1, %2, %3; sbcccs %1, %1, %0; movcc %0, #0" : "=&r" (flag), "=&r" (roksum) : "r" (uaddr), "Ir" (sizeof(u32)), "0" (current_thread_info()->addr_limit) : "cc"); flag; }) == 0)), 0))
   return -14;
  key->private.mm = mm;
 ...
 if (__builtin_expect(!!(!vma), 0))
  return -14;




 if (__builtin_expect(!!((vma->vm_flags & (0x00004000|0x00000001)) != 0x00000001), 0))
  return (vma->vm_flags & 0x00004000) ? -1 : -13;
# 272 "../../kernel/futex.c"
 if (__builtin_expect(!!(!(vma->vm_flags & 0x00000080)), 1)) {
  key->both.offset |= 2;
  key->private.mm = mm;
  key->private.address = address;
  get_futex_key_refs(key);
...
# 335 "../../kernel/futex.c"
static int fault_in_user_writeable(u32 *uaddr)
{
 struct mm_struct *mm = (get_current())->mm;
 int ret;

 down_read(&mm->mmap_sem);
 ret = get_user_pages((get_current()), mm, (unsigned long)uaddr,
        1, 1, 0, ((void *)0), ((void *)0));
 up_read(&mm->mmap_sem);
     ...
     union futex_key *key)
{
 ...
 for (this = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = ((&(&hb->chain)->node_list)->next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );}); prefetch(this->list.plist.node_list.next), &this->list.plist.node_list != (&(&hb->chain)->node_list); this = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = (this->list.plist.node_list.next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );})) {
...
static u32 cmpxchg_futex_value_locked(u32 *uaddr, u32 uval, u32 newval)
{
 u32 curval;
...
static int get_futex_value_locked(u32 *dest, u32 *from)
{
 int ret;

 pagefault_disable();
 ret = __copy_from_user(dest, from, sizeof(u32));
 pagefault_enable();
...
static int refill_pi_state_cache(void)
{
 ...
 if (__builtin_expect(!!((get_current())->pi_state_cache), 1))
  return 0;

 pi_state = kzalloc(sizeof(*pi_state), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 ...
 (((&pi_state->refcount)->counter) = (1));
 pi_state->key = (union futex_key) { .both = { .ptr = ((void *)0) } };

 (get_current())->pi_state_cache = pi_state;
...
{
 struct futex_pi_state *pi_state = (get_current())->pi_state_cache;

 ({ int __ret_warn_on = !!(!pi_state); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 420); __builtin_expect(!!(__ret_warn_on), 0); });
 (get_current())->pi_state_cache = ((void *)0);
...
{
 if (!(atomic_sub_return(1, &pi_state->refcount) == 0))
  return;
 ...
 if (pi_state->owner) {
  _raw_spin_lock_irq(&pi_state->owner->pi_lock);
  list_del_init(&pi_state->list);
  _raw_spin_unlock_irq(&pi_state->owner->pi_lock);

  rt_mutex_proxy_unlock(&pi_state->pi_mutex, pi_state->owner);
 }

 if ((get_current())->pi_state_cache)
  kfree(pi_state);
 else {
...
void exit_pi_state_list(struct task_struct *curr)
{
 ...
 union futex_key key = (union futex_key) { .both = { .ptr = ((void *)0) } };
 ...
 _raw_spin_lock_irq(&curr->pi_lock);
 while (!list_empty(head)) {

  next = head->next;
  pi_state = ({ const typeof( ((struct futex_pi_state *)0)->list ) *__mptr = (next); (struct futex_pi_state *)( (char *)__mptr - __builtin_offsetof(struct futex_pi_state,list) );});
  key = pi_state->key;
  hb = hash_futex(&key);
  _raw_spin_unlock_irq(&curr->pi_lock);
  ...
  ({ int __ret_warn_on = !!(pi_state->owner != curr); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 515); __builtin_expect(!!(__ret_warn_on), 0); });
  ({ int __ret_warn_on = !!(list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 516); __builtin_expect(!!(__ret_warn_on), 0); });
  list_del_init(&pi_state->list);
  pi_state->owner = ((void *)0);
  _raw_spin_unlock_irq(&curr->pi_lock);
...
{
 struct futex_pi_state *pi_state = ((void *)0);
 struct futex_q *this, *next;
 struct plist_head *head;
 struct task_struct *p;
 pid_t pid = uval & 0x3fffffff;

 head = &hb->chain;

 for (this = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = ((&(head)->node_list)->next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );}), next = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = (this->list.plist.node_list.next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );}); &this->list.plist.node_list != (&(head)->node_list); this = next, next = ({ const typeof( ((typeof(*next) *)0)->list.plist.node_list ) *__mptr = (next->list.plist.node_list.next); (typeof(*next) *)( (char *)__mptr - __builtin_offsetof(typeof(*next),list.plist.node_list) );})) {
  if (match_futex(&this->key, key)) {
   ...
   if (__builtin_expect(!!(!pi_state), 0))
    return -22;

   ({ int __ret_warn_on = !!(!(*(volatile int *)&(&pi_state->refcount)->counter)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 555); __builtin_expect(!!(__ret_warn_on), 0); });
# 567 "../../kernel/futex.c"
   ...
   atomic_add(1, &pi_state->refcount);
   *ps = pi_state;
 ...
 _raw_spin_lock_irq(&p->pi_lock);
 if (__builtin_expect(!!(p->flags & 0x00000004), 0)) {





  int ret = (p->flags & 0x00000008) ? -3 : -11;

  _raw_spin_unlock_irq(&p->pi_lock);
  put_task_struct(p);
  return ret;
 }
 ...
 ({ int __ret_warn_on = !!(!list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 624); __builtin_expect(!!(__ret_warn_on), 0); });
...
# 654 "../../kernel/futex.c"
static int futex_lock_pi_atomic(u32 *uaddr, struct futex_hash_bucket *hb,
    union futex_key *key,
...
retry:
 ...
 if (__builtin_expect(!!(curval == -14), 0))
  return -14;




 if ((__builtin_expect(!!((curval & 0x3fffffff) == task_pid_vnr(task)), 0)))
  return -35;




 if (__builtin_expect(!!(!curval), 0))
  return 1;
 ...
 newval = curval | 0x80000000;
# 707 "../../kernel/futex.c"
 if (__builtin_expect(!!(ownerdied || !(curval & 0x3fffffff)), 0)) {

  newval = (curval & ~0x3fffffff) | task_pid_vnr(task);
  ownerdied = 0;
  lock_taken = 1;
 }
 ...
 if (__builtin_expect(!!(curval == -14), 0))
  return -14;
 if (__builtin_expect(!!(curval != uval), 0))
  goto retry;




 if (__builtin_expect(!!(lock_taken), 0))
  return 1;
...
static int wake_futex_pi(u32 *uaddr, u32 uval, struct futex_q *this)
{
 ...
 if (pi_state->owner != (get_current()))
  return -22;

 _raw_spin_lock(&pi_state->pi_mutex.wait_lock);
 ...
 if (!(uval & 0x40000000)) {
  ...
  newval = 0x80000000 | task_pid_vnr(new_owner);
  ...
  if (curval == -14)
   ...
   ret = -22;
  if (ret) {
   _raw_spin_unlock(&pi_state->pi_mutex.wait_lock);
   return ret;
  }
 }

 _raw_spin_lock_irq(&pi_state->owner->pi_lock);
 ({ int __ret_warn_on = !!(list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 843); __builtin_expect(!!(__ret_warn_on), 0); });
 list_del_init(&pi_state->list);
 _raw_spin_unlock_irq(&pi_state->owner->pi_lock);

 _raw_spin_lock_irq(&new_owner->pi_lock);
 ({ int __ret_warn_on = !!(!list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 848); __builtin_expect(!!(__ret_warn_on), 0); });
 list_add(&pi_state->list, &new_owner->pi_state_list);
 pi_state->owner = new_owner;
 _raw_spin_unlock_irq(&new_owner->pi_lock);
...
static int unlock_futex_pi(u32 *uaddr, u32 uval)
{
 u32 oldval;
...
double_lock_hb(struct futex_hash_bucket *hb1, struct futex_hash_bucket *hb2)
{
 if (hb1 <= hb2) {
  spin_lock(&hb1->lock);
  if (hb1 < hb2)
   do { _raw_spin_lock(spinlock_check(&hb2->lock)); } while (0);
 } else {
  spin_lock(&hb2->lock);
  do { _raw_spin_lock(spinlock_check(&hb1->lock)); } while (0);
 }
}
...
static int futex_wake(u32 *uaddr, int fshared, int nr_wake, u32 bitset)
{
 struct futex_hash_bucket *hb;
 struct futex_q *this, *next;
 struct plist_head *head;
...
static int
futex_wake_op(u32 *uaddr1, int fshared, u32 *uaddr2,
       int nr_wake, int nr_wake2, int op)
{
 union futex_key key1 = (union futex_key) { .both = { .ptr = ((void *)0) } }, key2 = (union futex_key) { .both = { .ptr = ((void *)0) } };
...
retry_private:
 ...
 if (__builtin_expect(!!(op_ret < 0), 0)) {

  double_unlock_hb(hb1, hb2);
# 987 "../../kernel/futex.c"
  if (__builtin_expect(!!(op_ret != -14), 0)) {
   ret = op_ret;
   goto out_put_keys;
  }
     ...
     struct futex_hash_bucket *hb2, union futex_key *key2)
{
 ...
 if (__builtin_expect(!!(&hb1->chain != &hb2->chain), 1)) {
      ...
      struct futex_hash_bucket *hb)
{
 ...
 ({ int __ret_warn_on = !!(plist_node_empty(&q->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1086); __builtin_expect(!!(__ret_warn_on), 0); });
 plist_del(&q->list, &q->list.plist);

 ({ int __ret_warn_on = !!(!q->rt_waiter); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1089); __builtin_expect(!!(__ret_warn_on), 0); });
 q->rt_waiter = ((void *)0);
 ...
 wake_up_state(q->task, (1 | 2));
}
# 1120 "../../kernel/futex.c"
static int futex_proxy_trylock_atomic(u32 *pifutex,
     struct futex_hash_bucket *hb1,
     struct futex_hash_bucket *hb2,
     union futex_key *key1, union futex_key *key2,
     struct futex_pi_state **ps, int set_waiters)
{
 struct futex_q *top_waiter = ((void *)0);
 u32 curval;
 int ret;
...
# 1180 "../../kernel/futex.c"
static int futex_requeue(u32 *uaddr1, int fshared, u32 *uaddr2,
...
retry_private:
 ...
 if (__builtin_expect(!!(cmpval != ((void *)0)), 1)) {
  u32 curval;
  ...
  if (__builtin_expect(!!(ret), 0)) {
   ...
   ret = ({ register const typeof(*(uaddr1)) *__p asm("r0") = (uaddr1); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } curval = (typeof(*(uaddr1))) __r2; __e; });
   if (ret)
    goto out_put_keys;
 ...
 if (requeue_pi && (task_count - nr_wake < nr_requeue)) {
  ...
  if (ret == 1) {
   ({ int __ret_warn_on = !!(pi_state); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1278); __builtin_expect(!!(__ret_warn_on), 0); });
   drop_count++;
   task_count++;
  ...
  case -11:
   ...
   put_futex_key(fshared, &key2);
   put_futex_key(fshared, &key1);
   ({ __might_sleep("../../kernel/futex.c", 1303, 0); _cond_resched(); });
   goto retry;
  default:
   goto out_unlock;
  }
 ...
 head1 = &hb1->chain;
 for (this = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = ((&(head1)->node_list)->next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );}), next = ({ const typeof( ((typeof(*this) *)0)->list.plist.node_list ) *__mptr = (this->list.plist.node_list.next); (typeof(*this) *)( (char *)__mptr - __builtin_offsetof(typeof(*this),list.plist.node_list) );}); &this->list.plist.node_list != (&(head1)->node_list); this = next, next = ({ const typeof( ((typeof(*next) *)0)->list.plist.node_list ) *__mptr = (next->list.plist.node_list.next); (typeof(*next) *)( (char *)__mptr - __builtin_offsetof(typeof(*next),list.plist.node_list) );})) {
...
static inline __attribute__((always_inline)) struct futex_hash_bucket *queue_lock(struct futex_q *q)
{
 struct futex_hash_bucket *hb;
...
# 1423 "../../kernel/futex.c"
static inline __attribute__((always_inline)) void queue_me(struct futex_q *q, struct futex_hash_bucket *hb)
{
 int prio;
# 1435 "../../kernel/futex.c"
 prio = ({ typeof((get_current())->normal_prio) _min1 = ((get_current())->normal_prio); typeof(100) _min2 = (100); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
...
retry:
 ...
 if (lock_ptr != ((void *)0)) {
  spin_lock(lock_ptr);
# 1481 "../../kernel/futex.c"
  if (__builtin_expect(!!(lock_ptr != q->lock_ptr), 0)) {
   spin_unlock(lock_ptr);
   goto retry;
  }
  ({ int __ret_warn_on = !!(plist_node_empty(&q->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1485); __builtin_expect(!!(__ret_warn_on), 0); });
  plist_del(&q->list, &q->list.plist);

  do { if (__builtin_expect(!!(q->pi_state), 0)) do { *(int *)0 = 0; } while (1); } while(0);
...
{
 ({ int __ret_warn_on = !!(plist_node_empty(&q->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1505); __builtin_expect(!!(__ret_warn_on), 0); });
 plist_del(&q->list, &q->list.plist);
...
{
 u32 newtid = task_pid_vnr(newowner) | 0x80000000;
...
retry:
 ...
 if (pi_state->owner != ((void *)0)) {
  _raw_spin_lock_irq(&pi_state->owner->pi_lock);
  ({ int __ret_warn_on = !!(list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1573); __builtin_expect(!!(__ret_warn_on), 0); });
  list_del_init(&pi_state->list);
  _raw_spin_unlock_irq(&pi_state->owner->pi_lock);
 }
 ...
 _raw_spin_lock_irq(&newowner->pi_lock);
 ({ int __ret_warn_on = !!(!list_empty(&pi_state->list)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1581); __builtin_expect(!!(__ret_warn_on), 0); });
 list_add(&pi_state->list, &newowner->pi_state_list);
 _raw_spin_unlock_irq(&newowner->pi_lock);
 return 0;
# 1596 "../../kernel/futex.c"
handle_fault:
 spin_unlock(q->lock_ptr);
...
# 1641 "../../kernel/futex.c"
static int fixup_owner(u32 *uaddr, int fshared, struct futex_q *q,
         int locked)
{
 struct task_struct *owner;
 int ret = 0;
 ...
 if (locked) {
  ...
  if (q->pi_state->owner != (get_current()))
   ret = fixup_pi_state_owner(uaddr, q, (get_current()), fshared);
  goto out;
 }
...
# 1679 "../../kernel/futex.c"
 ...
 if (rt_mutex_owner(&q->pi_state->pi_mutex) == (get_current()))
  printk("<3>" "fixup_owner: ret = %d pi-mutex: %p "
    "pi-state %p\n", ret,
    ...
    struct hrtimer_sleeper *timeout)
{
 ...
 if (__builtin_expect(!!(!plist_node_empty(&q->list)), 1)) {
...
# 1756 "../../kernel/futex.c"
static int futex_wait_setup(u32 *uaddr, u32 val, int fshared,
...
# 1779 "../../kernel/futex.c"
retry:
 q->key = (union futex_key) { .both = { .ptr = ((void *)0) } };
 ...
 *hb = queue_lock(q);
 ...
 if (ret) {
  ...
  ret = ({ register const typeof(*(uaddr)) *__p asm("r0") = (uaddr); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } uval = (typeof(*(uaddr))) __r2; __e; });
  if (ret)
   goto out;
...
static int futex_wait(u32 *uaddr, int fshared,
        u32 val, ktime_t *abs_time, u32 bitset, int clockrt)
{
 ...
 q.bitset = bitset;
 q.rt_waiter = ((void *)0);
 q.requeue_pi_key = ((void *)0);

 if (abs_time) {
  to = &timeout;

  hrtimer_init_on_stack(&to->timer, clockrt ? 0 :
          1, HRTIMER_MODE_ABS);
  hrtimer_init_sleeper(to, (get_current()));
  hrtimer_set_expires_range_ns(&to->timer, *abs_time,
          (get_current())->timer_slack_ns);
 }
...
retry:
 ...
 restart->futex.flags = 0x04;

 if (fshared)
  restart->futex.flags |= 0x01;
 if (clockrt)
  restart->futex.flags |= 0x02;
...
static long futex_wait_restart(struct restart_block *restart)
{
 u32 *uaddr = (u32 *)restart->futex.uaddr;
 int fshared = 0;
 ktime_t t, *tp = ((void *)0);

 if (restart->futex.flags & 0x04) {
  t.tv64 = restart->futex.time;
  tp = &t;
 }
 restart->fn = do_no_restart_syscall;
 if (restart->futex.flags & 0x01)
  fshared = 1;
...
# 1923 "../../kernel/futex.c"
static int futex_lock_pi(u32 *uaddr, int fshared,
    int detect, ktime_t *time, int trylock)
{
 ...
 if (time) {
  to = &timeout;
  hrtimer_init_on_stack(&to->timer, 0,
          HRTIMER_MODE_ABS);
  hrtimer_init_sleeper(to, (get_current()));
...
retry_private:
 ...
 ret = futex_lock_pi_atomic(uaddr, hb, &q.key, &q.pi_state, (get_current()), 0);
 if (__builtin_expect(!!(ret), 0)) {
  switch (ret) {
  case 1:
  ...
  case -11:
   ...
   queue_unlock(&q, hb);
   put_futex_key(fshared, &q.key);
   ({ __might_sleep("../../kernel/futex.c", 1970, 0); _cond_resched(); });
   goto retry;
  default:
   goto out_unlock_put_key;
  }
 ...
 ({ int __ret_warn_on = !!(!q.pi_state); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 1982); __builtin_expect(!!(__ret_warn_on), 0); });
 ...
 if (ret && (rt_mutex_owner(&q.pi_state->pi_mutex) == (get_current())))
...
static int futex_unlock_pi(u32 *uaddr, int fshared)
{
 struct futex_hash_bucket *hb;
 struct futex_q *this, *next;
 u32 uval;
...
retry:
 if (({ register const typeof(*(uaddr)) *__p asm("r0") = (uaddr); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } uval = (typeof(*(uaddr))) __r2; __e; }))
  return -14;



 if ((uval & 0x3fffffff) != task_pid_vnr((get_current())))
  return -1;
 ...
 if (!(uval & 0x40000000))
  uval = cmpxchg_futex_value_locked(uaddr, task_pid_vnr((get_current())), 0);


 if (__builtin_expect(!!(uval == -14), 0))
  goto pi_faulted;




 if (__builtin_expect(!!(uval == task_pid_vnr((get_current()))), 0))
  goto out_unlock;
...
# 2167 "../../kernel/futex.c"
 if (!match_futex(&q->key, key2)) {
  ({ int __ret_warn_on = !!(q->lock_ptr && (&hb->lock != q->lock_ptr)); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 2168); __builtin_expect(!!(__ret_warn_on), 0); });
  ...
  if (timeout && !timeout->task)
   ret = -110;
  else if (signal_pending((get_current())))
   ret = -513;
 }
 return ret;
}
# 2226 "../../kernel/futex.c"
static int futex_wait_requeue_pi(u32 *uaddr, int fshared,
     u32 val, ktime_t *abs_time, u32 bitset,
     int clockrt, u32 *uaddr2)
{
 struct hrtimer_sleeper timeout, *to = ((void *)0);
 struct rt_mutex_waiter rt_waiter;
 struct rt_mutex *pi_mutex = ((void *)0);
 struct futex_hash_bucket *hb;
 ...
 do { } while (0);
 rt_waiter.task = ((void *)0);

 key2 = (union futex_key) { .both = { .ptr = ((void *)0) } };
 ret = get_futex_key(uaddr2, fshared, &key2);
 if (__builtin_expect(!!(ret != 0), 0))
  goto out;
...
# 2294 "../../kernel/futex.c"
 if (!q.rt_waiter) {
  ...
  if (q.pi_state && (q.pi_state->owner != (get_current()))) {
   spin_lock(q.lock_ptr);
   ret = fixup_pi_state_owner(uaddr2, &q, (get_current()),
         fshared);
   spin_unlock(q.lock_ptr);
  }
 } else {
  ...
  ({ int __ret_warn_on = !!(!&q.pi_state); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_slowpath_null("../../kernel/futex.c", 2311); __builtin_expect(!!(__ret_warn_on), 0); });
 ...
 if (ret == -14) {
  if (rt_mutex_owner(pi_mutex) == (get_current()))
...

{
 ...
 if (__builtin_expect(!!(len != sizeof(*head)), 0))
  return -22;

 (get_current())->robust_list = head;
...

{
 ...
 unsigned long ret;
 const struct cred *cred = ((get_current())->cred), *pcred;

 if (!futex_cmpxchg_enabled)
  return -38;

 if (!pid)
  head = (get_current())->robust_list;
 else {
  struct task_struct *p;
  ...
  if (!p)
   goto err_unlock;
  ret = -1;
  pcred = ({ const struct task_struct *__t = (p); ({ typeof(__t->real_cred) _________p1 = (*(volatile typeof(__t->real_cred) *)&(__t->real_cred)); do { } while(0); (_________p1); }); });
 ...
 if (({ register const typeof(*(len_ptr)) __r2 asm("r2") = (sizeof(*head)); register const typeof(*(len_ptr)) *__p asm("r0") = (len_ptr); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; }))
  return -14;
 return ({ register const typeof(*(head_ptr)) __r2 asm("r2") = (head); register const typeof(*(head_ptr)) *__p asm("r0") = (head_ptr); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "1" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "2" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "4" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; case 8: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r2" " ; .err ; .endif\n\t" "bl	__put_user_" "8" : "=&r" (__e) : "0" (__p), "r" (__r2) : "ip", "lr", "cc"); break; default: __e = __put_user_bad(); break; } __e; });
...
retry:
 ...
 if ((uval & 0x3fffffff) == task_pid_vnr(curr)) {
# 2470 "../../kernel/futex.c"
  mval = (uval & 0x80000000) | 0x40000000;
  ...
  if (!pi && (uval & 0x80000000))
   futex_wake(uaddr, 1, 1, 0xffffffff);
 }
 return 0;
}
...
static inline __attribute__((always_inline)) int fetch_robust_entry(struct robust_list **entry,
         struct robust_list * *head,
         int *pi)
{
 unsigned long uentry;

 if (({ register const typeof(*((unsigned long *)head)) *__p asm("r0") = ((unsigned long *)head); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } uentry = (typeof(*((unsigned long *)head))) __r2; __e; }))
  return -14;
...
{
 struct robust_list_head *head = curr->robust_list;
 struct robust_list *entry, *next_entry, *pending;
 ...
 if (({ register const typeof(*(&head->futex_offset)) *__p asm("r0") = (&head->futex_offset); register unsigned long __r2 asm("r2"); register int __e asm("r0"); switch (sizeof(*(__p))) { case 1: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "1" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; case 2: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "2" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "r3", "lr", "cc"); break; case 4: __asm__ __volatile__ ( ".ifnc " "%0" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" "bl	__get_user_" "4" : "=&r" (__e), "=r" (__r2) : "0" (__p) : "lr", "cc"); break; default: __e = __get_user_bad(); break; } futex_offset = (typeof(*(&head->futex_offset))) __r2; __e; }))
  return;
 ...
 while (entry != &head->list) {
  ...
  ({ __might_sleep("../../kernel/futex.c", 2567, 0); _cond_resched(); });
 }

 if (pending)
  handle_futex_death((void *)pending + futex_offset,
       curr, pip);
}
  ...
  u32 *uaddr2, u32 val2, u32 val3)
{
 ...
 case 10:
  ...
  break;
 case 3:
  ret = futex_requeue(uaddr, fshared, uaddr2, val, val2, ((void *)0), 0);
  break;
 case 4:
...

{
 ...
 if (utime && (cmd == 0 || cmd == 6 ||
        cmd == 9 ||
        cmd == 11)) {
  if (copy_from_user(&ts, utime, sizeof(ts)) != 0)
   return -14;
  if (!(((&ts)->tv_sec >= 0) && (((unsigned long) (&ts)->tv_nsec) < 1000000000L)))
   return -22;
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) futex_init(void)
{
 u32 curval;
 int i;
# 2686 "../../kernel/futex.c"
 curval = cmpxchg_futex_value_locked(((void *)0), 0, 0);
 if (curval == -14)
  futex_cmpxchg_enabled = 1;

 for (i = 0; i < (sizeof(futex_queues) / sizeof((futex_queues)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(futex_queues), typeof(&futex_queues[0]))); }))); i++) {
  plist_head_init(&futex_queues[i].chain, &futex_queues[i].lock);
  do { spinlock_check(&futex_queues[i].lock); do { *(&(&futex_queues[i].lock)->rlock) = (raw_spinlock_t) { .raw_lock = { 0 }, }; } while (0); } while (0);
 }

 return 0;
}
static initcall_t __initcall_futex_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = futex_init;
