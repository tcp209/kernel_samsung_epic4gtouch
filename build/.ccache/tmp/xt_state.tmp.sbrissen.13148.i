# 1 "../../net/netfilter/xt_state.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./include/generated/autoconf.h" 1
# 1 "<command-line>" 2
# 1 "../../net/netfilter/xt_state.c"
# 11 "../../net/netfilter/xt_state.c"
...
# 14 "../../net/netfilter/xt_state.c" 2

# 1 "../../include/linux/netfilter/xt_state.h" 1
# 9 "../../include/linux/netfilter/xt_state.h"
struct xt_state_info {
 unsigned int statemask;
};
# 16 "../../net/netfilter/xt_state.c" 2
...
state_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
 ...
 if (nf_ct_is_untracked(skb))
  statebit = (1 << (IP_CT_NUMBER + 1));
 else if (!nf_ct_get(skb, &ctinfo))
  statebit = (1 << 0);
 else
  statebit = (1 << ((ctinfo)%IP_CT_IS_REPLY+1));
...
static int state_mt_check(const struct xt_mtchk_param *par)
{
 ...
 if (ret < 0)
  printk("<6>" "cannot load conntrack support for proto=%u\n", par->family);
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) state_mt_init(void)
{
 return xt_register_match(&state_mt_reg);
}

static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) state_mt_exit(void)
{
 xt_unregister_match(&state_mt_reg);
}

static initcall_t __initcall_state_mt_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = state_mt_init;;
static exitcall_t __exitcall_state_mt_exit __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = state_mt_exit;;
