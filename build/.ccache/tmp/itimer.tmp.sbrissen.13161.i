...
int do_getitimer(int which, struct itimerval *value)
{
 ...
 case 0:
  ...
  break;
 case 1:
  get_cpu_itimer(tsk, 1, value);
  break;
 case 2:
  get_cpu_itimer(tsk, 0, value);
  break;
 default:
  return(-22);
 }
...
enum hrtimer_restart it_real_fn(struct hrtimer *timer)
{
 ...
 trace_itimer_expire(0, sig->leader_pid, 0);
 kill_pid_info(14, ((struct siginfo *) 1), sig->leader_pid);

 return HRTIMER_NORESTART;
}

static inline __attribute__((always_inline)) u32 cputime_sub_ns(cputime_t ct, s64 real_ns)
{
 struct timespec ts;
 s64 cpu_ns;

 jiffies_to_timespec(ct,&ts);
...
again:
  ...
  trace_itimer_state(0, value, 0);
  spin_unlock_irq(&tsk->sighand->siglock);
  break;
 case 1:
  set_cpu_itimer(tsk, 1, value, ovalue);
  break;
 case 2:
  set_cpu_itimer(tsk, 0, value, ovalue);
  break;
 default:
  return -22;
 }
...

{
 ...
 if (value) {
  ...
  ({ void *__p = ((char *) &set_buffer); size_t __n = sizeof(set_buffer); if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
