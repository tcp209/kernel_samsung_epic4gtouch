...
# 60 "../../drivers/scsi/sd.c" 2
# 1 "../../include/scsi/scsi_driver.h" 1
...
static inline __attribute__((always_inline)) int scsi_sense_valid(struct scsi_sense_hdr *sshdr)
{
 if (!sshdr)
  return 0;
...
static inline __attribute__((always_inline)) int scsi_sense_is_deferred(struct scsi_sense_hdr *sshdr)
{
...
{
# 846 "../../include/scsi/scsi_host.h"
 return 0;
}
# 858 "../../include/scsi/scsi_host.h"
...
# 63 "../../drivers/scsi/sd.c" 2
# 1 "../../include/scsi/scsi_ioctl.h" 1
...
static inline __attribute__((always_inline)) struct scsi_disk *scsi_disk(struct gendisk *disk)
{
 return ({ const typeof( ((struct scsi_disk *)0)->driver ) *__mptr = (disk->private_data); (struct scsi_disk *)( (char *)__mptr - __builtin_offsetof(struct scsi_disk,driver) );});
}
# 93 "../../drivers/scsi/sd.h"
...
# 117 "../../drivers/scsi/sd.h"
static inline __attribute__((always_inline)) void sd_dif_config_host(struct scsi_disk *disk)
{
}
static inline __attribute__((always_inline)) int sd_dif_prepare(struct request *rq, sector_t s, unsigned int a)
{
 return 0;
}
static inline __attribute__((always_inline)) void sd_dif_complete(struct scsi_cmnd *cmd, unsigned int a)
{
}
# 67 "../../drivers/scsi/sd.c" 2
# 1 "../../drivers/scsi/scsi_logging.h" 1
# 39 "../../drivers/scsi/scsi_logging.h"
extern unsigned int scsi_logging_level;
# 68 "../../drivers/scsi/sd.c" 2
...
static spinlock_t sd_index_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } };
static struct ida sd_index_ida = { .idr = { .top = ((void *)0), .id_free = ((void *)0), .layers = 0, .id_free_cnt = 0, .lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } }, }, .free_bitmap = ((void *)0), };




static struct mutex sd_ref_mutex = { .count = { (1) } , .wait_lock = (spinlock_t ) { { .rlock = { .raw_lock = { 0 }, } } } , .wait_list = { &(sd_ref_mutex.wait_list), &(sd_ref_mutex.wait_list) } };
      ...
      const char *buf, size_t count)
{
 ...
 struct scsi_disk *sdkp = ({ const typeof( ((struct scsi_disk *)0)->dev ) *__mptr = (dev); (struct scsi_disk *)( (char *)__mptr - __builtin_offsetof(struct scsi_disk,dev) );});
 ...
 for (i = 0; i < (sizeof(sd_cache_types) / sizeof((sd_cache_types)[0]) + (sizeof(struct { int:-!!(__builtin_types_compatible_p(typeof(sd_cache_types), typeof(&sd_cache_types[0]))); }))); i++) {
 ...
 rcd = ct & 0x01 ? 1 : 0;
 wce = ct & 0x02 ? 1 : 0;
 if (scsi_mode_sense(sdp, 0x08, 8, buffer, sizeof(buffer), (30 * 200),
       5, &data, ((void *)0)))
  return -22;
 len = ({ size_t __min1 = (sizeof(buffer)); size_t __min2 = (data.length - data.header_length - data.block_descriptor_length); __min1 < __min2 ? __min1: __min2; });
 ...
 if (scsi_mode_select(sdp, 1, sp, 8, buffer_data, len, (30 * 200),
        5, &data, &sshdr)) {
...
static struct device_attribute sd_disk_attrs[] = {
 { .attr = {.name = "cache_type", .mode = (00400|00040|00004)|00200 }, .show = sd_show_cache_type, .store = sd_store_cache_type, },

 { .attr = {.name = "FUA", .mode = (00400|00040|00004) }, .show = sd_show_fua, .store = ((void *)0), },
 { .attr = {.name = "allow_restart", .mode = (00400|00040|00004)|00200 }, .show = sd_show_allow_restart, .store = sd_store_allow_restart, },

 { .attr = {.name = "manage_start_stop", .mode = (00400|00040|00004)|00200 }, .show = sd_show_manage_start_stop, .store = sd_store_manage_start_stop, },

 { .attr = {.name = "protection_type", .mode = (00400|00040|00004) }, .show = sd_show_protection_type, .store = ((void *)0), },
 { .attr = {.name = "app_tag_own", .mode = (00400|00040|00004) }, .show = sd_show_app_tag_own, .store = ((void *)0), },
 { .attr = {.name = "thin_provisioning", .mode = (00400|00040|00004) }, .show = sd_show_thin_provisioning, .store = ((void *)0), },
 { .attr = { .name = ((void *)0) } },
};
...
# 420 "../../drivers/scsi/sd.c"
static int sd_prepare_discard(struct request *rq)
{
 struct scsi_disk *sdkp = scsi_disk(rq->rq_disk);
 struct bio *bio = rq->bio;
 sector_t sector = bio->bi_sector;
 unsigned int num = ((bio)->bi_size >> 9);
 ...
 ({ void *__p = (rq->cmd); size_t __n = rq->cmd_len; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });

 if (sdkp->unmap) {
  char *buf = kmap_atomic((&((((bio)))->bi_io_vec[(((bio))->bi_idx)]))->bv_page, KM_USER0);
...
static int sd_prep_fn(struct request_queue *q, struct request *rq)
{
 ...
 if (((rq)->cmd_flags & (1 << __REQ_DISCARD)))
 ...
 threshold = get_capacity(disk) - 8 *
  (sdp->sector_size / 512);

 if (__builtin_expect(!!(sdp->last_sector_bug && block + this_count > threshold), 0)) {
  if (block < threshold) {
 ...
 ;;
# 564 "../../drivers/scsi/sd.c"
 if (sdp->sector_size == 1024) {
  if ((block & 1) || (blk_rq_sectors(rq) & 1)) {
   (SCpnt)->request->rq_disk ? printk("<3>" "%s %s: " "[%s] " "Bad block number requested\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev) , (SCpnt)->request->rq_disk->disk_name) : printk("<3>" "%s %s: " "Bad block number requested\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev));
 ...
 if (((rq)->cmd_flags & 1) == 1) {
  ...
  if ((0) &&
      sd_dif_prepare(rq, block, sdp->sector_size) == -5)
   goto out;

 } else if (((rq)->cmd_flags & 1) == 0) {
  SCpnt->cmnd[0] = 0x08;
  SCpnt->sc_data_direction = DMA_FROM_DEVICE;
 } else {
  (SCpnt)->request->rq_disk ? printk("<3>" "%s %s: " "[%s] " "Unknown command %x\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev) , (SCpnt)->request->rq_disk->disk_name, rq->cmd_flags) : printk("<3>" "%s %s: " "Unknown command %x\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev) , rq->cmd_flags);
  goto out;
 }
 ...
 if (host_dif == SD_DIF_TYPE2_PROTECTION) {
  SCpnt->cmnd = mempool_alloc(sd_cdb_pool, ((( gfp_t)0x20u)));

  if (__builtin_expect(!!(SCpnt->cmnd == ((void *)0)), 0)) {
   ret = 2;
   goto out;
  }

  SCpnt->cmd_len = SD_EXT_CDB_SIZE;
  ({ void *__p = (SCpnt->cmnd); size_t __n = SCpnt->cmd_len; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
  SCpnt->cmnd[0] = 0x7f;
  SCpnt->cmnd[7] = 0x18;
  SCpnt->cmnd[9] = (((rq)->cmd_flags & 1) == 0) ? 0x09 : 0x0b;
  SCpnt->cmnd[10] = protect | (((rq)->cmd_flags & (1 << __REQ_FUA)) ? 0x8 : 0);
  ...
  SCpnt->cmnd[0] += 0x88 - 0x08;
  SCpnt->cmnd[1] = protect | (((rq)->cmd_flags & (1 << __REQ_FUA)) ? 0x8 : 0);
  ...
  if (__builtin_expect(!!(((rq)->cmd_flags & (1 << __REQ_FUA))), 0)) {
   ...
   (SCpnt)->request->rq_disk ? printk("<3>" "%s %s: " "[%s] " "FUA write on READ/WRITE(6) drive\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev) , (SCpnt)->request->rq_disk->disk_name) : printk("<3>" "%s %s: " "FUA write on READ/WRITE(6) drive\n" , dev_driver_string(&((SCpnt)->device)->sdev_gendev) , dev_name(&((SCpnt)->device)->sdev_gendev));
...
static int sd_open(struct block_device *bdev, fmode_t mode)
{
 ...
 retval = -123;
 if (sdev->removable && !sdkp->media_present && !(mode & (( fmode_t)0x40)))
  goto error_out;
 ...
 retval = -30;
 if (sdkp->write_prot && (mode & (( fmode_t)0x2)))
  goto error_out;
 ...
 if (!sdkp->openers++ && sdev->removable) {
  if (scsi_block_when_processing_errors(sdev))
   scsi_set_medium_removal(sdev, 1);
 }
...
static int sd_release(struct gendisk *disk, fmode_t mode)
{
 ...
 if (!--sdkp->openers && sdev->removable) {
  if (scsi_block_when_processing_errors(sdev))
   scsi_set_medium_removal(sdev, 0);
 }
...
# 966 "../../drivers/scsi/sd.c"
 retval = -19;

 if (scsi_block_when_processing_errors(sdp)) {
  sshdr = kzalloc(sizeof(*sshdr), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
  retval = scsi_test_unit_ready(sdp, (30 * 200), 5,
           sshdr);
 }
...
out:
 if (retval != sdkp->previous_state)
  sdev_evt_send_simple(sdp, SDEV_EVT_MEDIA_CHANGE, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
...
static unsigned int sd_completed_bytes(struct scsi_cmnd *scmd)
{
 ...
 if (scmd->device->sector_size < 512) {
  ...
  ({ unsigned int __r, __b = (factor); if (!__builtin_constant_p(__b) || __b == 0 || (7 < 4 && (__b & (__b - 1)) != 0)) { __r = ({ register unsigned int __base asm("r4") = __b; register unsigned long long __n asm("r0") = start_lba; register unsigned long long __res asm("r2"); register unsigned int __rem asm("r1"); asm( ".ifnc " "%0" "," "r1" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%3" "," "r4" " ; .err ; .endif\n\t" "bl	__do_div64" : "=r" (__rem), "=r" (__res) : "r" (__n), "r" (__base) : "ip", "lr", "cc"); start_lba = __res; __rem; }); } else if ((__b & (__b - 1)) == 0) { __r = start_lba; __r &= (__b - 1); start_lba /= __b; } else { unsigned long long __res, __x, __t, __m, __n = start_lba; unsigned int __c, __p, __z = 0; __r = __n; __p = 1 << ({ unsigned int __left = (__b), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); __m = (~0ULL / __b) * __p; __m += (((~0ULL % __b + 1) * __p) + __b - 1) / __b; __x = ~0ULL / __b * __b - 1; __res = (__m & 0xffffffff) * (__x & 0xffffffff); __res >>= 32; __res += (__m & 0xffffffff) * (__x >> 32); __t = __res; __res += (__x & 0xffffffff) * (__m >> 32); __t = (__res < __t) ? (1ULL << 32) : 0; __res = (__res >> 32) + __t; __res += (__m >> 32) * (__x >> 32); __res /= __p; if (~0ULL % (__b / (__b & -__b)) == 0) { __n /= (__b & -__b); __m = ~0ULL / (__b / (__b & -__b)); __p = 1; __c = 1; } else if (__res != __x / __b) { __c = 1; __m = (~0ULL / __b) * __p; __m += ((~0ULL % __b + 1) * __p) / __b; } else { unsigned int __bits = -(__m & -__m); __bits |= __m >> 32; __bits = (~__bits) << 1; if (!__bits) { __p /= (__m & -__m); __m /= (__m & -__m); } else { __p >>= ({ unsigned int __left = (__bits), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); __m >>= ({ unsigned int __left = (__bits), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); } __c = 0; } if (!__c) { asm ( "umull	%Q0, %R0, %1, %Q2\n\t" "mov	%Q0, #0" : "=&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else if (!(__m & ((1ULL << 63) | (1ULL << 31)))) { __res = __m; asm ( "umlal	%Q0, %R0, %Q1, %Q2\n\t" "mov	%Q0, #0" : "+&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else { asm ( "umull	%Q0, %R0, %Q1, %Q2\n\t" "cmn	%Q0, %Q1\n\t" "adcs	%R0, %R0, %R1\n\t" "adc	%Q0, %3, #0" : "=&r" (__res) : "r" (__m), "r" (__n), "r" (__z) : "cc" ); } if (!(__m & ((1ULL << 63) | (1ULL << 31)))) { asm ( "umlal	%R0, %Q0, %R1, %Q2\n\t" "umlal	%R0, %Q0, %Q1, %R2\n\t" "mov	%R0, #0\n\t" "umlal	%Q0, %R0, %R1, %R2" : "+&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else { asm ( "umlal	%R0, %Q0, %R2, %Q3\n\t" "umlal	%R0, %1, %Q2, %R3\n\t" "mov	%R0, #0\n\t" "adds	%Q0, %1, %Q0\n\t" "adc	%R0, %R0, #0\n\t" "umlal	%Q0, %R0, %R2, %R3" : "+&r" (__res), "+&r" (__z) : "r" (__m), "r" (__n) : "cc" ); } __res /= __p; { unsigned int __res0 = __res; unsigned int __b0 = __b; __r -= __res0 * __b0; } start_lba = __res; } __r; });
  ({ unsigned int __r, __b = (factor); if (!__builtin_constant_p(__b) || __b == 0 || (7 < 4 && (__b & (__b - 1)) != 0)) { __r = ({ register unsigned int __base asm("r4") = __b; register unsigned long long __n asm("r0") = end_lba; register unsigned long long __res asm("r2"); register unsigned int __rem asm("r1"); asm( ".ifnc " "%0" "," "r1" " ; .err ; .endif\n\t" ".ifnc " "%1" "," "r2" " ; .err ; .endif\n\t" ".ifnc " "%2" "," "r0" " ; .err ; .endif\n\t" ".ifnc " "%3" "," "r4" " ; .err ; .endif\n\t" "bl	__do_div64" : "=r" (__rem), "=r" (__res) : "r" (__n), "r" (__base) : "ip", "lr", "cc"); end_lba = __res; __rem; }); } else if ((__b & (__b - 1)) == 0) { __r = end_lba; __r &= (__b - 1); end_lba /= __b; } else { unsigned long long __res, __x, __t, __m, __n = end_lba; unsigned int __c, __p, __z = 0; __r = __n; __p = 1 << ({ unsigned int __left = (__b), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); __m = (~0ULL / __b) * __p; __m += (((~0ULL % __b + 1) * __p) + __b - 1) / __b; __x = ~0ULL / __b * __b - 1; __res = (__m & 0xffffffff) * (__x & 0xffffffff); __res >>= 32; __res += (__m & 0xffffffff) * (__x >> 32); __t = __res; __res += (__x & 0xffffffff) * (__m >> 32); __t = (__res < __t) ? (1ULL << 32) : 0; __res = (__res >> 32) + __t; __res += (__m >> 32) * (__x >> 32); __res /= __p; if (~0ULL % (__b / (__b & -__b)) == 0) { __n /= (__b & -__b); __m = ~0ULL / (__b / (__b & -__b)); __p = 1; __c = 1; } else if (__res != __x / __b) { __c = 1; __m = (~0ULL / __b) * __p; __m += ((~0ULL % __b + 1) * __p) / __b; } else { unsigned int __bits = -(__m & -__m); __bits |= __m >> 32; __bits = (~__bits) << 1; if (!__bits) { __p /= (__m & -__m); __m /= (__m & -__m); } else { __p >>= ({ unsigned int __left = (__bits), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); __m >>= ({ unsigned int __left = (__bits), __nr = 0; if (__left & 0xffff0000) __nr += 16, __left >>= 16; if (__left & 0x0000ff00) __nr += 8, __left >>= 8; if (__left & 0x000000f0) __nr += 4, __left >>= 4; if (__left & 0x0000000c) __nr += 2, __left >>= 2; if (__left & 0x00000002) __nr += 1; __nr; }); } __c = 0; } if (!__c) { asm ( "umull	%Q0, %R0, %1, %Q2\n\t" "mov	%Q0, #0" : "=&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else if (!(__m & ((1ULL << 63) | (1ULL << 31)))) { __res = __m; asm ( "umlal	%Q0, %R0, %Q1, %Q2\n\t" "mov	%Q0, #0" : "+&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else { asm ( "umull	%Q0, %R0, %Q1, %Q2\n\t" "cmn	%Q0, %Q1\n\t" "adcs	%R0, %R0, %R1\n\t" "adc	%Q0, %3, #0" : "=&r" (__res) : "r" (__m), "r" (__n), "r" (__z) : "cc" ); } if (!(__m & ((1ULL << 63) | (1ULL << 31)))) { asm ( "umlal	%R0, %Q0, %R1, %Q2\n\t" "umlal	%R0, %Q0, %Q1, %R2\n\t" "mov	%R0, #0\n\t" "umlal	%Q0, %R0, %R1, %R2" : "+&r" (__res) : "r" (__m), "r" (__n) : "cc" ); } else { asm ( "umlal	%R0, %Q0, %R2, %Q3\n\t" "umlal	%R0, %1, %Q2, %R3\n\t" "mov	%R0, #0\n\t" "adds	%Q0, %1, %Q0\n\t" "adc	%R0, %R0, #0\n\t" "umlal	%Q0, %R0, %R2, %R3" : "+&r" (__res), "+&r" (__z) : "r" (__m), "r" (__n) : "cc" ); } __res /= __p; { unsigned int __res0 = __res; unsigned int __b0 = __b; __r -= __res0 * __b0; } end_lba = __res; } __r; });
 }
...
# 1182 "../../drivers/scsi/sd.c"
 if ((((result) >> 24) & 0xff) != 0x08 &&
 ...
 case 0x00:
  ...
  SCpnt->result = 0;
  ({ void *__p = (SCpnt->sense_buffer); size_t __n = 96; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
  break;
 case 0x0b:
 case 0x05:
  if (sshdr.asc == 0x10)
 ...
 out:
 if (((SCpnt->request)->cmd_flags & 1) == 0 && scsi_prot_sg_count(SCpnt))
...
sd_spinup_disk(struct scsi_disk *sdkp)
{
 ...
 do {
  ...
  do {
   cmd[0] = 0x00;
   ({ void *__p = ((void *) &cmd[1]); size_t __n = 9; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
   ...
   retries++;
    ...
    (!scsi_status_is_good(the_result) ||
     (((((the_result) >> 24) & 0xff) & 0x08) &&
     sense_valid && sshdr.sense_key == 0x06)));

  if (((((the_result) >> 24) & 0xff) & 0x08) == 0) {


   if(!spintime && !scsi_status_is_good(the_result)) {
    (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Unit Not Ready\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Unit Not Ready\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  ...
  if (sense_valid && sshdr.sense_key == 0x02) {
   ...
   if (!spintime) {
    (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Spinning up disk..." , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Spinning up disk..." , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
    cmd[0] = 0x1b;
    cmd[1] = 1;
    ({ void *__p = ((void *) &cmd[2]); size_t __n = 8; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
    cmd[4] = 1;
    ...
    scsi_execute_req(sdkp->device, cmd, DMA_NONE,
       ...
       (30 * 200), 5,
       ((void *)0));
    spintime_expire = jiffies + 100 * 200;
    spintime = 1;
   }
 ...
 } while (spintime && (({ unsigned long __dummy; typeof(spintime_expire) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(jiffies) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(spintime_expire) - (long)(jiffies) >= 0)));
...
void sd_read_protection_type(struct scsi_disk *sdkp, unsigned char *buffer)
{
 ...
 if (type > SD_DIF_TYPE3_PROTECTION) {
  (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "formatted with unsupported " "protection type %u. Disabling disk!\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, type) : printk("<3>" "%s %s: " "formatted with unsupported " "protection type %u. Disabling disk!\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , type);
 ...
 if (scsi_host_dif_capable(sdp->host, type))
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Enabling DIF Type %u protection\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, type) : printk("<5>" "%s %s: " "Enabling DIF Type %u protection\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , type);

 else
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Disabling DIF Type %u protection\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, type) : printk("<5>" "%s %s: " "Disabling DIF Type %u protection\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , type);
...
{
 sd_print_result(sdkp, the_result);
 if ((((the_result) >> 24) & 0xff) & 0x08)
  sd_print_sense_hdr(sdkp, sshdr);
 else
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Sense not available.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Sense not available.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 ...
 if (sdp->removable &&
     sense_valid && sshdr->sense_key == 0x02)
  sdp->changed = 1;
      ...
      unsigned char *buffer)
{
 ...
 do {
  ({ void *__p = (cmd); size_t __n = 16; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
  cmd[0] = 0x9e;
  cmd[1] = 0x10;
  cmd[13] = 32;
  ({ void *__p = (buffer); size_t __n = 32; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 ...
 if (the_result) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "READ CAPACITY(16) failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "READ CAPACITY(16) failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 ...
 if ((sizeof(sdkp->capacity) == 4) && (lba >= 0xffffffffULL)) {
  (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Too big for this kernel. Use a " "kernel compiled with support for large block " "devices.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Too big for this kernel. Use a " "kernel compiled with support for large block " "devices.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 ...
 if (alignment && sdkp->first_scan)
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "physical block alignment offset: %u\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, alignment) : printk("<5>" "%s %s: " "physical block alignment offset: %u\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , alignment);
      ...
      unsigned char *buffer)
{
 ...
 do {
  cmd[0] = 0x25;
  ({ void *__p = (&cmd[1]); size_t __n = 9; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
  ({ void *__p = (buffer); size_t __n = 8; if ((__n) != 0) { if (__builtin_constant_p((0)) && (0) == 0) __memzero((__p),(__n)); else memset((__p),(0),(__n)); } (__p); });
 ...
 if (the_result) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "READ CAPACITY failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "READ CAPACITY failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  read_capacity_error(sdkp, sdp, &sshdr, sense_valid, the_result);
  return -22;
 }
...
sd_read_capacity(struct scsi_disk *sdkp, unsigned char *buffer)
{
 ...
 if (sd_try_rc16_first(sdp)) {
  ...
  if ((sizeof(sdkp->capacity) > 4) &&
   ...
   (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Very big device. " "Trying to use READ CAPACITY(16).\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Very big device. " "Trying to use READ CAPACITY(16).\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));

   sector_size = read_capacity_16(sdkp, sdp, buffer);
   if (sector_size < 0) {
    (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Using 0xffffffff as device size\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Using 0xffffffff as device size\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
...
# 1657 "../../drivers/scsi/sd.c"
 if (sdp->fix_capacity ||
     (sdp->guess_capacity && (sdkp->capacity & 0x01))) {
  (sdkp)->disk ? printk("<6>" "%s %s: " "[%s] " "Adjusting the sector count " "from its reported value: %llu\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, (unsigned long long) sdkp->capacity) : printk("<6>" "%s %s: " "Adjusting the sector count " "from its reported value: %llu\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (unsigned long long) sdkp->capacity);
...
got_data:
 if (sector_size == 0) {
  sector_size = 512;
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Sector size 0 reported, " "assuming 512.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Sector size 0 reported, " "assuming 512.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 ...
 if (sector_size != 512 &&
     ...
     sector_size != 4096 &&
     sector_size != 256) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Unsupported sector size %d.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, sector_size) : printk("<5>" "%s %s: " "Unsupported sector size %d.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , sector_size);
...

 {
  ...
  u64 sz = (u64)sdkp->capacity << ( __builtin_constant_p(sector_size) ? ( (sector_size) < 1 ? ____ilog2_NaN() : (sector_size) & (1ULL << 63) ? 63 : (sector_size) & (1ULL << 62) ? 62 : (sector_size) & (1ULL << 61) ? 61 : (sector_size) & (1ULL << 60) ? 60 : (sector_size) & (1ULL << 59) ? 59 : (sector_size) & (1ULL << 58) ? 58 : (sector_size) & (1ULL << 57) ? 57 : (sector_size) & (1ULL << 56) ? 56 : (sector_size) & (1ULL << 55) ? 55 : (sector_size) & (1ULL << 54) ? 54 : (sector_size) & (1ULL << 53) ? 53 : (sector_size) & (1ULL << 52) ? 52 : (sector_size) & (1ULL << 51) ? 51 : (sector_size) & (1ULL << 50) ? 50 : (sector_size) & (1ULL << 49) ? 49 : (sector_size) & (1ULL << 48) ? 48 : (sector_size) & (1ULL << 47) ? 47 : (sector_size) & (1ULL << 46) ? 46 : (sector_size) & (1ULL << 45) ? 45 : (sector_size) & (1ULL << 44) ? 44 : (sector_size) & (1ULL << 43) ? 43 : (sector_size) & (1ULL << 42) ? 42 : (sector_size) & (1ULL << 41) ? 41 : (sector_size) & (1ULL << 40) ? 40 : (sector_size) & (1ULL << 39) ? 39 : (sector_size) & (1ULL << 38) ? 38 : (sector_size) & (1ULL << 37) ? 37 : (sector_size) & (1ULL << 36) ? 36 : (sector_size) & (1ULL << 35) ? 35 : (sector_size) & (1ULL << 34) ? 34 : (sector_size) & (1ULL << 33) ? 33 : (sector_size) & (1ULL << 32) ? 32 : (sector_size) & (1ULL << 31) ? 31 : (sector_size) & (1ULL << 30) ? 30 : (sector_size) & (1ULL << 29) ? 29 : (sector_size) & (1ULL << 28) ? 28 : (sector_size) & (1ULL << 27) ? 27 : (sector_size) & (1ULL << 26) ? 26 : (sector_size) & (1ULL << 25) ? 25 : (sector_size) & (1ULL << 24) ? 24 : (sector_size) & (1ULL << 23) ? 23 : (sector_size) & (1ULL << 22) ? 22 : (sector_size) & (1ULL << 21) ? 21 : (sector_size) & (1ULL << 20) ? 20 : (sector_size) & (1ULL << 19) ? 19 : (sector_size) & (1ULL << 18) ? 18 : (sector_size) & (1ULL << 17) ? 17 : (sector_size) & (1ULL << 16) ? 16 : (sector_size) & (1ULL << 15) ? 15 : (sector_size) & (1ULL << 14) ? 14 : (sector_size) & (1ULL << 13) ? 13 : (sector_size) & (1ULL << 12) ? 12 : (sector_size) & (1ULL << 11) ? 11 : (sector_size) & (1ULL << 10) ? 10 : (sector_size) & (1ULL << 9) ? 9 : (sector_size) & (1ULL << 8) ? 8 : (sector_size) & (1ULL << 7) ? 7 : (sector_size) & (1ULL << 6) ? 6 : (sector_size) & (1ULL << 5) ? 5 : (sector_size) & (1ULL << 4) ? 4 : (sector_size) & (1ULL << 3) ? 3 : (sector_size) & (1ULL << 2) ? 2 : (sector_size) & (1ULL << 1) ? 1 : (sector_size) & (1ULL << 0) ? 0 : ____ilog2_NaN() ) : (sizeof(sector_size) <= 4) ? __ilog2_u32(sector_size) : __ilog2_u64(sector_size) );
  ...
  if (sdkp->first_scan || old_capacity != sdkp->capacity) {
   (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "%llu %d-byte logical blocks: (%s/%s)\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, (unsigned long long)sdkp->capacity, sector_size, cap_str_10, cap_str_2) : printk("<5>" "%s %s: " "%llu %d-byte logical blocks: (%s/%s)\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (unsigned long long)sdkp->capacity, sector_size, cap_str_10, cap_str_2);




   if (sdkp->hw_sector_size != sector_size)
    (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "%u-byte physical blocks\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, sdkp->hw_sector_size) : printk("<5>" "%s %s: " "%u-byte physical blocks\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , sdkp->hw_sector_size);
...
sd_read_write_protect_flag(struct scsi_disk *sdkp, unsigned char *buffer)
{
 ...
 if (sdp->skip_ms_page_3f) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Assuming Write Enabled\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Assuming Write Enabled\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  return;
 }
 ...
 if (!scsi_status_is_good(res)) {
  (sdkp)->disk ? printk("<4>" "%s %s: " "[%s] " "Test WP failed, assume Write Enabled\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<4>" "%s %s: " "Test WP failed, assume Write Enabled\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));

 } else {
  sdkp->write_prot = ((data.device_specific & 0x80) != 0);
  set_disk_ro(sdkp->disk, sdkp->write_prot);
  if (sdkp->first_scan || old_wp != sdkp->write_prot) {
   (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Write Protect is %s\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, sdkp->write_prot ? "on" : "off") : printk("<5>" "%s %s: " "Write Protect is %s\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , sdkp->write_prot ? "on" : "off");

   (sdkp)->disk ? printk("<7>" "%s %s: " "[%s] " "Mode Sense: %02x %02x %02x %02x\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, buffer[0], buffer[1], buffer[2], buffer[3]) : printk("<7>" "%s %s: " "Mode Sense: %02x %02x %02x %02x\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , buffer[0], buffer[1], buffer[2], buffer[3]);
...
sd_read_cache_type(struct scsi_disk *sdkp, unsigned char *buffer)
{
 ...
 if (!data.header_length) {
  modepage = 6;
  (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Missing header in MODE_SENSE response\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Missing header in MODE_SENSE response\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 }
 ...
 if (scsi_status_is_good(res)) {
  ...
  if (offset >= 512 - 2) {
   (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Malformed MODE SENSE response\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Malformed MODE SENSE response\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
   goto defaults;
  }

  if ((buffer[offset] & 0x3f) != modepage) {
   (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Got wrong page\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Got wrong page\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
   goto defaults;
  }
  ...
  if (sdkp->DPOFUA && !sdkp->device->use_10_for_rw) {
   (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Uses READ/WRITE(6), disabling FUA\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Uses READ/WRITE(6), disabling FUA\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  ...
  if (sdkp->first_scan || old_wce != sdkp->WCE ||
   ...
   (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Write cache: %s, read cache: %s, %s\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, sdkp->WCE ? "enabled" : "disabled", sdkp->RCD ? "disabled" : "enabled", sdkp->DPOFUA ? "supports DPO and FUA" : "doesn't support DPO or FUA") : printk("<5>" "%s %s: " "Write cache: %s, read cache: %s, %s\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , sdkp->WCE ? "enabled" : "disabled", sdkp->RCD ? "disabled" : "enabled", sdkp->DPOFUA ? "supports DPO and FUA" : "doesn't support DPO or FUA");
...
bad_sense:
 if (scsi_sense_valid(&sshdr) &&
  ...
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Cache data unavailable\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Cache data unavailable\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 else
  (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Asking for cache data failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Asking for cache data failed\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));

defaults:
 (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "Assuming drive cache: write through\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "Assuming drive cache: write through\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 sdkp->WCE = 0;
 sdkp->RCD = 0;
 sdkp->DPOFUA = 0;
}
...
void sd_read_app_tag_own(struct scsi_disk *sdkp, unsigned char *buffer)
{
 ...
 res = scsi_mode_sense(sdp, 1, 0x0a, buffer, 36, (30 * 200),
         5, &data, &sshdr);

 if (!scsi_status_is_good(res) || !data.header_length ||
     data.length < 6) {
  (sdkp)->disk ? printk("<4>" "%s %s: " "[%s] " "getting Control mode page failed, assume no ATO\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<4>" "%s %s: " "getting Control mode page failed, assume no ATO\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
 ...
 if ((buffer[offset] & 0x3f) != 0x0a) {
  (sdkp)->disk ? printk("<3>" "%s %s: " "[%s] " "ATO Got wrong page\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<3>" "%s %s: " "ATO Got wrong page\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  return;
 }
...
static void sd_read_block_limits(struct scsi_disk *sdkp)
{
 ...
 unsigned char *buffer = kmalloc(vpd_len, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
...
static void sd_read_block_characteristics(struct scsi_disk *sdkp)
{
 ...
 buffer = kmalloc(vpd_len, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 ...
 if (rot == 1)
  queue_flag_set_unlocked(14, sdkp->disk->queue);
...
static int sd_revalidate_disk(struct gendisk *disk)
{
 ...
 buffer = kmalloc(512, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!buffer) {
  (sdkp)->disk ? printk("<4>" "%s %s: " "[%s] " "sd_revalidate_disk: Memory " "allocation failure.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<4>" "%s %s: " "sd_revalidate_disk: Memory " "allocation failure.\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
...
static void sd_probe_async(void *data, async_cookie_t cookie)
{
 ...
 (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Attached SCSI %sdisk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name, sdp->removable ? "removable " : "") : printk("<5>" "%s %s: " "Attached SCSI %sdisk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , sdp->removable ? "removable " : "");

 put_device(&sdkp->dev);
}
# 2265 "../../drivers/scsi/sd.c"
static int sd_probe(struct device *dev)
{
 struct scsi_device *sdp = ({ const typeof( ((struct scsi_device *)0)->sdev_gendev ) *__mptr = (dev); (struct scsi_device *)( (char *)__mptr - __builtin_offsetof(struct scsi_device,sdev_gendev) );});
 struct scsi_disk *sdkp;
 struct gendisk *gd;
 u32 index;
 int error;
 ...
 error = -19;
 if (sdp->type != 0x00 && sdp->type != 0x07 && sdp->type != 0x0e)
  goto out;
 ...
 error = -12;
 sdkp = kzalloc(sizeof(*sdkp), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!sdkp)
  goto out;
 ...
 do {
  if (!ida_pre_get(&sd_index_ida, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u))))
   goto out_put;
 ...
 error = sd_format_disk_name("sd", index, gd->disk_name, 32);
 if (error)
  goto out_free_index;
 ...
 if (!sdp->request_queue->rq_timeout) {
  if (sdp->type != 0x07)
   blk_queue_rq_timeout(sdp->request_queue, (30 * 200));
  else
...
static int sd_start_stop_device(struct scsi_disk *sdkp, int start)
{
 ...
 res = scsi_execute_req(sdp, cmd, DMA_NONE, ((void *)0), 0, &sshdr,
          (30 * 200), 5, ((void *)0));
 if (res) {
  (sdkp)->disk ? printk("<4>" "%s %s: " "[%s] " "START_STOP FAILED\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<4>" "%s %s: " "START_STOP FAILED\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  sd_print_result(sdkp, res);
  if ((((res) >> 24) & 0xff) & 0x08)
   sd_print_sense_hdr(sdkp, &sshdr);
 }
...
static void sd_shutdown(struct device *dev)
{
 ...
 if (sdkp->WCE) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Synchronizing SCSI cache\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Synchronizing SCSI cache\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
  sd_sync_cache(sdkp);
 }

 if (system_state != SYSTEM_RESTART && sdkp->device->manage_start_stop) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Stopping disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Stopping disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
...
static int sd_suspend(struct device *dev, pm_message_t mesg)
{
 ...
 if ((mesg.event & (0x0002 | 0x0004)) && sdkp->device->manage_start_stop) {
  (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Stopping disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Stopping disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
...
static int sd_resume(struct device *dev)
{
 ...
 (sdkp)->disk ? printk("<5>" "%s %s: " "[%s] " "Starting disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<5>" "%s %s: " "Starting disk\n" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
...
static int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) __attribute__((no_instrument_function)) init_sd(void)
{
 int majors = 0, i, err;
 ...
 err = ({ static struct lock_class_key __key; __class_register(&sd_disk_class, &__key); });
 if (err)
  goto err_out;
 ...
 sd_cdb_cache = kmem_cache_create("sd_ext_cdb", SD_EXT_CDB_SIZE,
      0, 0, ((void *)0));
 if (!sd_cdb_cache) {
  printk("<3>" "sd: can't init extended cdb cache\n");
  goto err_out_class;
 }

 sd_cdb_pool = mempool_create_slab_pool(SD_MEMPOOL_SIZE, sd_cdb_cache);
 if (!sd_cdb_pool) {
  printk("<3>" "sd: can't init extended cdb pool\n");
  goto err_out_cache;
 }
...
static void __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) exit_sd(void)
{
 int i;
 ...
 driver_unregister(&sd_template.gendrv);;
 class_unregister(&sd_disk_class);
...
static initcall_t __initcall_init_sd6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init"))) = init_sd;;
static exitcall_t __exitcall_exit_sd __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit"))) = exit_sd;;

static void sd_print_sense_hdr(struct scsi_disk *sdkp,
          struct scsi_sense_hdr *sshdr)
{
 (sdkp)->disk ? printk("<6>" "%s %s: " "[%s] " "" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev) , (sdkp)->disk->disk_name) : printk("<6>" "%s %s: " "" , dev_driver_string(&((sdkp)->device)->sdev_gendev) , dev_name(&((sdkp)->device)->sdev_gendev));
